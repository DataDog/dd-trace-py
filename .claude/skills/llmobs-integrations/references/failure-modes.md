...
- Stream-manager `__enter__` retains the child wrapper for the `with` body; `on_stream_created` runs before retain and finalizes on callback failure because `__exit__` will not run
