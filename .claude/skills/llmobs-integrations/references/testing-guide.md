...
Use this pattern when the SDK can call the provider normally and vcrpy can record/replay the HTTP layer. Add `vcrpy` to `riotfile.py` only for suites that need it, and follow nearby integrations for version constraints.
...
Use the **run-tests** skill (`scripts/run-tests`). Never invoke `pytest` directly.
