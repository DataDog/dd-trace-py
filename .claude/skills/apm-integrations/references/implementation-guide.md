...
Key rules:
- A variant's `name` is the exact environment name passed to `scripts/test-env lock` or `refresh`; use the
  integration name as its base and add a qualifier when needed
