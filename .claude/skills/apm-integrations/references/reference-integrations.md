...
|----------|-----------|-----------|-------|
| cache | `redis/patch.py` | `pymemcache/patch.py` | Key-value stores, `cache.*` span tags, uses `context_with_data` via redis_utils (no Pin) |
...
| http-client | `httpx/patch.py` | `requests/connection.py` | Outbound HTTP, `http.*` span tags. httpx uses `context_with_event` (new preferred pattern); requests uses Pin + `tracer.trace` |
| http-server | `flask/patch.py` | `django/patch.py` | Web frameworks, request/response spans, `context_with_data` (no Pin) |
...
| orchestration | `celery/patch.py` | -- | Task orchestration, distributed tracing, Pin + signals |
| rpc | `grpc/patch.py` | -- | RPC frameworks, client + server interceptors, `tracer.start_span()` directly |
