...
FORMATTER_SCRIPT_PATH = SCRIPT_DIR / "format_integration_registry.py"
# --- End Configuration ---
    ...
    try:
        with open(filepath, 'r', newline='', encoding='utf-8') as csvfile:
            ...
            if not (col_integration and col_min and col_max):
                print(f"Error: Missing expected columns ('integration', 'minimum...', 'max...') in {filepath}. Found: {header}", file=sys.stderr)
...
def main():
    ...
    for entry in original_integrations_list:
        ...
        if not integration_name: continue # Skip entries without a name
        ...
        if is_external:
            ...
            if new_version_info:
                ...
                if removed:
                     ...
                     print(f"  Info: Removed version info for '{integration_name}' as it was not found in the new support table.")
    ...
    try:
        ...
        if FORMATTER_SCRIPT_PATH.is_file():
            ...
            print(f"Warning: Formatter script not found at {FORMATTER_SCRIPT_PATH}. Skipping formatting.", file=sys.stderr)
