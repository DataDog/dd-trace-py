...
then
    ...
    for file in $(find "$THIS_DIR" -name '*.[c|cpp|h]' | grep -v '.riot/' | grep -v 'ddtrace/vendor/' | grep -v 'ddtrace/appsec/iast/_taint_tracking/cmake-build-debug/' | grep -v '^ddtrace/appsec/iast/_taint_tracking/vendor/')
    do
    clang-format --style="{BasedOnStyle: Mozilla, IndentWidth: 4, ColumnLimit: 120}" -i "$file"
    done
else
  git ls-files '*.c' '*.cpp' '*.h' | grep -v '^ddtrace/vendor/' | grep -v '^ddtrace/appsec/iast/_taint_tracking/vendor/'  | while read filename
  do
    CFORMAT_TMP=`mktemp`
