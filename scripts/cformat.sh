...
then
    ...
    for file in $(find "$THIS_DIR" -name '*.[c|cpp|h]' | grep -v '.riot/' | grep -v 'src/ddtrace/vendor/' | grep -v 'src/ddtrace/appsec/iast/_taint_tracking/cmake-build-debug/' | grep -v '^src/ddtrace/appsec/iast/_taint_tracking/_vendor/')
    do
    clang-format --style="{BasedOnStyle: Mozilla, IndentWidth: 4, ColumnLimit: 120}" -i "$file"
    done
else
  git ls-files '*.c' '*.cpp' '*.h' | grep -v '^src/ddtrace/vendor/' | grep -v '^src/ddtrace/appsec/iast/_taint_tracking/_vendor/'  | while read filename
  do
    CFORMAT_TMP=`mktemp`
