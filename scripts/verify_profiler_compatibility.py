...
Usage:
    ...
    # Stub a new MAJOR.MINOR in the version registry + print open checklist
...
def _suite_asyncio_guards() -> dict[str, Any]:
    ...
    # Use importlib so mypy does not follow ddtrace package imports (ambient).
    import asyncio
    import importlib

    _asyncio_mod: Any = importlib.import_module("ddtrace.profiling._asyncio")
...
def _suite_profiler_samples(tmpdir: str) -> dict[str, Any]:
    ...
    ddup: Any = importlib.import_module("ddtrace.internal.datadog.profiling.ddup")
    _stack_ext: Any = importlib.import_module("ddtrace.internal.datadog.profiling.stack")
    stack_collector: Any = importlib.import_module("ddtrace.profiling.collector.stack")
    ...
    importlib.import_module("ddtrace.profiling._asyncio")
    ...
    try:
        ...
        _fail_unparsed_pprof(result, files, exc)

    return result


def _fail_unparsed_pprof(
    result: dict[str, Any],
    pprof_files: list[str],
    parse_exc: BaseException,
) -> dict[str, Any]:
    """Fail when pprof cannot be parsed — wall-sample contract is unchecked.

    File existence alone is not a pass. Missing files or missing
    zstandard/protobuf must not report success.
    """
    result["pprof_written"] = bool(pprof_files)
    result["pprof_parse_skipped"] = True
    result["passed"] = False
    if not pprof_files:
        result["error"] = (
            f"No pprof file written and profile parse unavailable ({parse_exc}). "
            "Wall-time sample contract not met. Install zstandard and protobuf."
        )
    else:
        result["error"] = (
            f"pprof written but content not validated ({parse_exc}). "
            "Wall-time sample contract not checked. Install zstandard and protobuf."
        )
    result["pprof_parse_skip_reason"] = result["error"]
    return result
...
def _hex_for_minor(major: int, minor: int) -> str:
    return f"0x{major:02x}{minor:02x}0000"


def _scaffold_version(version: str) -> None:
    """Stub a MAJOR.MINOR entry in the registry and print open checklist rows."""
    parts: list[str] = version.split(".")
    if len(parts) != 2 or not all(p.isdigit() for p in parts):
        raise SystemExit(f"--scaffold expects MAJOR.MINOR (got {version!r})")
    major: int = int(parts[0])
    minor: int = int(parts[1])
    key: str = f"{major}.{minor}"

    registry: dict[str, Any] = _load_version_registry()
    versions: dict[str, Any]
    raw_versions: object = registry.get("versions", {})
    ...
    if key in versions:
        print(f"Registry already has {key}:")
        print(json.dumps(versions[key], indent=2))
    else:
        template: object = registry.get("scaffold_template", {})
        entry: dict[str, Any] = copy.deepcopy(template) if isinstance(template, dict) else {}
        entry["hex"] = _hex_for_minor(major, minor)
        versions[key] = entry
        registry["versions"] = versions
        registry["default_target"] = key
        _save_version_registry(registry)
        print(f"Scaffolded {key} → {_VERSION_REGISTRY_FILE}")
        print(f"  hex={entry['hex']}")
        print(f"  default_target set to {key}")

    entry_now: dict[str, Any] = versions[key]
    open_rows: object = entry_now.get("checklist_open", [])
    print("\nOpen checklist (from registry):")
    if isinstance(open_rows, list) and open_rows:
        for row in open_rows:
            print(f"  [ ] {row}")
    else:
        print("  (none — update checklist_open as work proceeds)")
    print("\nNext: migrate-profiling-new-cpython skill + docs/contributing-profiling-new-cpython.rst")
    baselines: dict[str, Any] = _load_baselines()
    if key not in baselines:
        print(f"No baseline yet for {key}. After a green run: --python {key} --baseline")
...
def main() -> None:
    ...
    parser.add_argument(
        "--scaffold",
        metavar="MAJOR.MINOR",
        help="Stub a new version in scripts/profiles/profiling_versions.json and print open checklist rows.",
    )
    ...
    if args.baseline:
        if not all_passed:
            ...
            min_samples: int = _min_wall_samples_for(baseline_key)
