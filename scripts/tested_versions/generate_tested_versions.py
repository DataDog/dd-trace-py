...
def get_tracer_version() -> str:
    """Read the tracer version from the project's pyproject.toml."""
    ...
    if match:
        return re.sub(r"\.?rc\d+$", "", match.group(1))

    raise ValueError(f"Could not find project version in {PYPROJECT_TOML_PATH}")


TRACER_VERSION = get_tracer_version()
...
def to_json_data(tested_versions_per_integration: dict[str, dict[str, list[TestedVersion]]]) -> dict[str, object]:
    """Build the JSON payload for tested_versions.json."""
    entries = []
    ...
    return {
        ...
        "entries": sorted(entries, key=lambda entry: (entry["dependency_name"], entry["integration_name"])),
    }
