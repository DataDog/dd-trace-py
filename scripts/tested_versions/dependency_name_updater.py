...
class DependencyNameUpdater:
    ...
    def get_missing_tested_versions(self, observed_versions: set[tuple[str, str, str, str]]) -> list[dict[str, str]]:
        """Return tested versions from this session that are missing from tested_versions.json."""
        ...
        # this compare the difference between existing versions and observed versions during tests
        return [
            {
            ...
            for integration_name, dependency_name, version, python_version in sorted(
                observed_versions - current_tested_versions
            )
        ]
    ...
    def _get_packages_distributions(self) -> Mapping[str, list[str]]:
        """Return a cached mapping of import package names to distribution names."""
    ...
    def _iter_patched_dependencies(self):
        """Yield patched object, integration name, and distribution name triples."""
    ...
    def _is_valid_patch_call(self, tb_string: str) -> bool:
        """Return whether the traceback came from a ddtrace internal patch module."""
        return any(
            "ddtrace/contrib/internal" in line and "/patch.py" in line for line in reversed(tb_string.splitlines())
        )

    def _get_integration_name_from_traceback(self, tb_string: str) -> Optional[str]:
        """Extract the integration name from a ddtrace internal patch traceback."""
    ...
    def _get_full_module_from_object(self, obj) -> Optional[str]:
        """Return the full module name for a patched object."""
    ...
    def _distribution_contains_module(self, distribution_name: str, full_module_name: str) -> bool:
        """Return whether a distribution includes the patched module path."""
        try:
