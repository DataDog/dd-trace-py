        ...
        if integration_name in suites:
            ...
            if not candidates:
                candidates = sorted(
                    ...
                    if _suite_pattern_matches_integration(integration_name, suite_config)
                )
            if not candidates:
                raise ValueError(
...
def _suite_pattern_matches_integration(integration_name: str, suite_config: dict) -> bool:
    pattern = suite_config.get("pattern")
    if not isinstance(pattern, str):
        return False
    return re.fullmatch(pattern, integration_name) is not None
