...
def _parse_target(raw_item: object, *, integration_name: str) -> TargetSpec:
...
def _merge_effective_pkgs(
    suite_name: str, target: TargetSpec, base_pkgs: dict[str, PackageSpecValue]
) -> dict[str, PackageSpecValue] | None:
    effective = dict(base_pkgs)
    target_package = target.package or suite_name
...
def _normalize_target(integration_name: str, suite: RiotVenv, target: TargetSpec) -> _NormalizedTarget:
    ...
    return _NormalizedTarget(
        ...
        pkgs=_merge_effective_pkgs(integration_name, target, base_pkgs),
...
def _parse_integration(raw_item: object, *, suites: dict[str, RiotVenv]) -> IntegrationSpec | FallbackSpec:
    ...
    parsed_targets = [_parse_target(raw_target, integration_name=name) for raw_target in raw_targets]
