...
# Try to import the component hash generator
try:
...
class BuildCache:
    ...
    def __init__(self, base_dir: Optional[Path] = None):
        ...
        self.component_cache_dirs = {
            'cmake_iast': self.cache_dir / f"py{self.python_version}" / "cmake_iast",
            'cmake_ddup': self.cache_dir / f"py{self.python_version}" / "cmake_ddup", 
            'cmake_crashtracker': self.cache_dir / f"py{self.python_version}" / "cmake_crashtracker",
            'cmake_stack_v2': self.cache_dir / f"py{self.python_version}" / "cmake_stack_v2",
            'rust_native': self.cache_dir / f"py{self.python_version}" / "rust_native",
            'cython_extensions': self.cache_dir / f"py{self.python_version}" / "cython",
            'c_extensions': self.cache_dir / f"py{self.python_version}" / "c_extensions",
            'vendor_extensions': self.cache_dir / f"py{self.python_version}" / "vendor",
        }
        
        # Extension to component mapping
        self.extension_to_component = {
    ...
    def cache_artifacts(self, component: str, source_paths: List[Path], 
    ...
    def should_rebuild_extension(self, extension_name: str) -> bool:
        ...
        """
        component = self.extension_to_component.get(extension_name)
        if not component:
    ...
    def get_extension_component(self, extension_name: str) -> Optional[str]:
        ...
        return self.extension_to_component.get(extension_name)
    ...
    def get_cache_status(self) -> Dict[str, Dict]:
        ...
        for component in self.component_cache_dirs:
            ...
            status[component] = {
                ...
                'cached_python_version': cache_info.get('additional_info', {}).get('python_version') if cache_info else None,
            }
    ...
    def print_cache_status(self) -> None:
        ...
        for component, info in status.items():
            ...
            if info['cached_python_version']:
                ...
                print(f"  Python Version: Unknown")
...
def main():
    ...
    clean_parser.add_argument('--all-python-versions', action='store_true', 
                             help='Clean caches for all Python versions')
    ...
    if args.command == 'status':
        ...
        cache.clean_cache(args.components if args.components else None, 
                         all_python_versions=args.all_python_versions)
