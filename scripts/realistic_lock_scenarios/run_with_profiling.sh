...
#   logs      - Logs pipeline simulator
#   agent     - Agent checks simulator
...
configure_service() {
    ...
    case $scenario in
        ...
        profiler)
            ...
            ;;
        logs)
            export DD_SERVICE="logs-pipeline-simulator"
            export DD_ENV="profiling-test"
            export DD_VERSION="1.0.0"
            ;;
        agent)
            export DD_SERVICE="agent-checks-simulator"
            export DD_ENV="profiling-test"
            export DD_VERSION="1.0.0"
            ;;
    esac
...
run_scenario() {
    ...
    case $scenario in
        ...
        profiler)
            ...
            ;;
        logs)
            ddtrace-run python "${SCRIPT_DIR}/logs_pipeline_simulator.py" \
                --eps 1000 \
                --duration "$duration" \
                --workers 4
            ;;
        agent)
            ddtrace-run python "${SCRIPT_DIR}/agent_checks_simulator.py" \
                --checks 20 \
                --duration "$duration"
            ;;
        *)
...
case $SCENARIO in
    all)
        ...
        echo ""
        run_scenario "logs" "$DURATION"
        echo ""
        run_scenario "agent" "$DURATION"
        ;;
    dogweb|trace|profiler|logs|agent)
        run_scenario "$SCENARIO" "$DURATION"
        ;;
    *)
        echo -e "${RED}Unknown scenario: ${SCENARIO}${NC}"
        echo ""
        echo "Usage: $0 [dogweb|trace|profiler|logs|agent|all] [duration_seconds]"
        exit 1
        ;;
esac
...
echo "  - logs-pipeline-simulator"
echo "  - agent-checks-simulator"
