#!/usr/bin/env python3
"""
RLock Debug Stress Test

This is a diagnostic version that writes BOTH local pprof files AND uploads to production.
Use this to debug discrepancies between what the collector captures vs what appears in UI.

This script will:
1. Write local pprof files to /tmp
2. Upload to Datadog production/agent
3. Parse the local pprof files and print lock names found
4. Allow comparison between local output and UI

Usage:
    # With agent
    export DD_AGENT_HOST="localhost"
    python scripts/rlock_debug_stress_test.py
...
os.environ["DD_SERVICE"] = "rlock-debug-stress-test"
...
os.environ["DD_PROFILING_OUTPUT_PPROF"] = "/tmp/rlock_debug_stress_test"

print("\n" + "="*80)
print("RLock Debug Stress Test - Local + Production")
print("="*80)
...
try:
    ...
    print("  Local pprof analysis will not be available")
...
def analyze_local_pprof():
    ...
    newest_file = pprof_files[-1]
    print(f"\nAnalyzing: {newest_file}")
    print("-" * 80)
    
    try:
        profile = pprof_utils.parse_profile(newest_file)
        ...
        if lock_names:
            ...
            if bad_names:
                ...
                for name in bad_names[:10]:  # Show first 10
                    print(f"  • {name}")
                if len(bad_names) > 10:
        ...
        if len(good_names) > 0:
            ...
            print("  3. Check if variables are being optimized away")
            print("  4. Compare with unit test environment")
...
def main():
    ...
    print("  • Parse the local pprof files")
    print("  • Show what lock names were captured")
    print("  • Compare with what you see in the UI")
    print("\n" + "="*80)
    ...
    # Wait a bit for final profile write
    print("\nWaiting 5 seconds for final profile write...")
    time.sleep(5)
    ...
    print("\n6. Compare lock names in UI with the analysis above")
