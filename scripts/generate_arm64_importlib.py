"""Generate ARM64 import libraries for Python DLLs when cross-compiling to win-arm64.
...
x64 host Python, because the host Python ships x64 import libraries, but the MSVC
cross-linker (HostX64/arm64) requires ARM64-tagged import libraries.
"""
...
def run_lib_exe(def_path, out_path, machine="ARM64"):
    """Run lib.exe to produce an import library from a .def file."""
    subprocess.run(
        [
            "lib",
            f"/def:{def_path}",
...
def generate_arm64_importlib(dll_path, lib_output_path):
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        run_lib_exe(def_path, lib_output_path)
