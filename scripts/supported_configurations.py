...
def check_registry(data: dict) -> int:
    ...
    if missing:
        print(
            ...
            f"  3. Stage the updated files and commit\n"
            f"\n"
            f"If the var is reported via telemetry, also register it in dd-go (prod branch):\n"
            f"  trace/apps/tracer-telemetry-intake/telemetry-payload/static/python_config_rules.json\n"
            f"Then sync system-tests: ./utils/telemetry/intake/update.sh\n"
            f"\n"
            f"Unregistered vars:"
        )
