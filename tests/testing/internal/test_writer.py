...
class TestGetAsyncFlushEvents:
    ...
    def test_reads_private_partial_flush_env_var(self) -> None:
        with patch.dict("os.environ", {"_DD_CIVISIBILITY_PARTIAL_FLUSH_MIN_SPANS": "25"}):
            assert _get_async_flush_events(selected_tests_count=20000) == 25

    def test_reads_legacy_partial_flush_env_var(self) -> None:
        with patch.dict("os.environ", {"DD_TRACE_PARTIAL_FLUSH_MIN_SPANS": "5"}):
            assert _get_async_flush_events() == 5
    ...
    def test_private_partial_flush_env_var_takes_priority(self) -> None:
        with patch.dict(
            ...
            assert _get_async_flush_events() == 3

    def test_invalid_value_disables(self) -> None:
