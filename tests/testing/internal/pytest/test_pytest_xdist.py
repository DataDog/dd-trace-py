...
class TestXdistPartialFlush:
    """Verify that _DD_TRACE_PARTIAL_FLUSH_MIN_SPANS mitigates crash data loss.
    ...
    def test_partial_flush_before_and_after(self, test_project: Path) -> None:
        """Same crash scenario, two runs: without and with _DD_TRACE_PARTIAL_FLUSH_MIN_SPANS=1.
        ...
        with MockCIVisibilityServer() as server_with:
            ...
            env = _make_env(server_with.url, extra={"_DD_TRACE_PARTIAL_FLUSH_MIN_SPANS": "1"})
            _run_pytest_subprocess(test_project, "-n", "1", "--max-worker-restart", "1", "-p", "no:randomly", env=env)
        ...
        assert "test_passes_before_crash" in names_with, (
            f"With _DD_TRACE_PARTIAL_FLUSH_MIN_SPANS=1, the event should be saved. Got: {names_with}"
        )
    ...
    def test_partial_flush_preserves_all_healthy_tests_with_crashes(
        ...
        env = _make_env(mock_server.url, extra={"_DD_TRACE_PARTIAL_FLUSH_MIN_SPANS": "1"})
