...
async def rag(req: RagRequest, request: Request):
    ...
    # If the client provided trace headers, activate them so vLLM spans
    # become children of the client span without creating a server span.
    try:
        ...
        if ctx:
            ...
            print(f"ddtracer.context_provider.active(): {ddtracer.context_provider.active()}")
    except Exception:
