...
class TestTracedAsyncCursor(TracerTestCase):
    def setUp(self):
    ...
    async def test_execute_wrapped_is_called_and_returned(self):
        ...
        traced_cursor = await TracedAsyncCursor(cursor, pin, {})
    ...
    async def test_dbm_propagation_not_supported(self):
        ...
        traced_cursor = await TracedAsyncCursor(cursor, Pin("dbapi_service", tracer=self.tracer), cfg)
    ...
    async def test_cursor_execute_with_dbm_injection(self):
        ...
        traced_cursor = await TracedAsyncCursor(cursor, Pin(service="orders-db", tracer=self.tracer), cfg)
    ...
    async def test_cfg_service(self):
        ...
        traced_cursor = await TracedAsyncCursor(cursor, pin, cfg)
...
class TestFetchTracedAsyncCursor(TracerTestCase):
    def setUp(self):
    ...
    async def test_execute_wrapped_is_called_and_returned(self):
        ...
        pin = Pin("pin_name", tracer=self.tracer)
        traced_cursor = await FetchTracedAsyncCursor(cursor, pin, {})
        assert "__result__" == await traced_cursor.execute("__query__", "arg_1", kwarg1="kwarg1")
        cursor.execute.assert_called_once_with("__query__", "arg_1", kwarg1="kwarg1")
    ...
    async def test_cursor_execute_fetch_with_dbm_injection(self):
        ...
        traced_cursor = await FetchTracedAsyncCursor(cursor, Pin("dbapi_service", tracer=self.tracer), cfg)
...
class TestTracedAsyncConnection(TracerTestCase):
    def setUp(self):
    ...
    async def test_cursor_class(self):
        ...
        # Default
        traced_connection = await TracedAsyncConnection(self.connection, pin=pin)
        ...
        with self.override_config("dbapi2", dict(trace_fetch_methods=True)):
            traced_connection = await TracedAsyncConnection(self.connection, pin=pin, cursor_cls=TracedAsyncCursor)
            self.assertTrue(traced_connection._self_cursor_cls is TracedAsyncCursor)
    ...
    async def test_commit_is_traced(self):
        ...
        pin = Pin("pin_name", tracer=tracer)
        traced_connection = await TracedAsyncConnection(connection, pin)
    ...
    async def test_connection_context_manager(self):
        ...
        conn = await TracedAsyncConnection(ConnectionConnection(), pin)
        with conn as conn2:
            await conn2.commit()
        ...
        class ConnectionTracedAsyncCursor(object):
            ...
            async def cursor(self):
                return await TracedAsyncCursor(Cursor(), pin, {})
        ...
        conn = await TracedAsyncConnection(ConnectionDifferentConnection(), pin)
        with conn as conn2:
            await conn2.commit()
        spans = self.pop_spans()
        assert len(spans) == 0
        ...
        conn = await TracedAsyncConnection(ConnectionNoCtx(), pin)
