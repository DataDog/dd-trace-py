...
class MySQLTest(unittest.TestCase):
    ...
    def test_connection(self):
        ...
        self.conn = MySQL(**MYSQL_CONFIG)
        assert_is_not_none(self.conn)
    ...
    def test_cursor_buffered_raw(self):
        ...
        for buffered in (None, False, True):
            for raw in (None, False, True):
                cursor = self.conn.cursor(buffered=buffered, raw=raw)
                if buffered:
                    if raw:
    ...
    def test_connection_buffered_raw(self):
        ...
        for buffered in (None, False, True):
            for raw in (None, False, True):
                self.conn = MySQL(buffered=buffered, raw=raw, **MYSQL_CONFIG)
                cursor = self.conn.cursor()
                if buffered:
                    if raw:
                        eq_(cursor._datadog_baseclass_name, "MySQLCursorBufferedRaw")
