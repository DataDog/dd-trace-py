...
from tests.utils import SubprocessTestCase
...
class TestLLMObsOpenAI(SubprocessTestCase):
    def setUp(self):
        self.mock_tracer = DummyTracer()
        self.patcher = mock.patch("ddtrace.llmobs._llmobs.LLMObsSpanWriter").start()
        self.mock_llmobs_writer = mock.MagicMock()
        self.patcher.return_value = self.mock_llmobs_writer

        LLMObs.enable(ml_app="<ml-app-name>", _tracer=self.mock_tracer, integrations=["openai"])
        LLMObs._instance._llmobs_span_writer = self.mock_llmobs_writer

        import openai

        self.openai = openai
        pin = Pin.get_from(self.openai)
        pin.override(self.openai, tracer=self.mock_tracer)

    def tearDown(self):
        self.mock_llmobs_writer.reset_mock()
        LLMObs.disable()
        self.patcher.stop()

    @run_in_subprocess
    def test_llmobs_completion(self):
        ...
        assert self.mock_llmobs_writer.enqueue.call_count == 1
        self.mock_llmobs_writer.enqueue.assert_called_with(
    ...
    @run_in_subprocess
    def test_llmobs_completion_stream(self):
        with get_openai_vcr(subdirectory_name="v0").use_cassette("completion_streamed.yaml"):
            model = "ada"
            expected_completion = '! ... A page layouts page drawer? ... Interesting. The "Tools" is'
            resp = self.openai.Completion.create(model=model, prompt="Hello world", stream=True)
            for _ in resp:
                pass
        span = self.mock_tracer.pop_traces()[0][0]
        assert self.mock_llmobs_writer.enqueue.call_count == 1
    ...
    @run_in_subprocess
    def test_llmobs_chat_completion_stream(self):
        """Ensure llmobs records are emitted for chat completion endpoints when configured.
    ...
    @run_in_subprocess
    def test_llmobs_chat_completion_function_call(self):
    ...
    @run_in_subprocess
    def test_llmobs_chat_completion_function_call_stream(self):
        """Test that function call chat completion calls are recorded as LLMObs events correctly."""
        if not hasattr(self.openai, "ChatCompletion"):
            pytest.skip("ChatCompletion not supported for this version of openai")
    ...
    @run_in_subprocess
    def test_llmobs_chat_completion_tool_call(self):
        if not hasattr(self.openai, "ChatCompletion"):
            pytest.skip("ChatCompletion not supported for this version of openai")
    ...
    @run_in_subprocess
    def test_llmobs_completion_error(self):
    ...
    @run_in_subprocess
    def test_llmobs_chat_completion_error(self):
