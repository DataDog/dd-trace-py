...
def test_translated_step_events_share_apm_trace_id_with_root(bedrock_agent_client, request_vcr, llmobs_events):
...
def test_translate_bedrock_traces_finishes_orphaned_step_spans(bedrock_llmobs, llmobs_events, tracer):
    """A pending modelInvocationInput without a matching output event must still be finished
    by the safety net in ``BedrockIntegration._finalize_bedrock_step_spans``.
    """
    ...
    assert integration._step_spans_by_step_id == {}
    assert integration._active_span_by_step_id == {}
    assert integration._inner_spans_by_step_id == {}
