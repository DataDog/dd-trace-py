...
class BotocoreTest(TracerTestCase):
    ...
    def setUp(self):
        ...
        Pin(service=self.TEST_SERVICE, tracer=self.tracer)._onto(botocore.parsers.ResponseParser)
    ...
    def test_dynamodb_put_get(self):
        ...
        Pin(service=self.TEST_SERVICE, tracer=self.tracer)._onto(ddb)
    ...
    def _test_sqs_client(self):
        ...
        Pin(service=self.TEST_SERVICE, tracer=self.tracer)._onto(self.sqs_client)
        self.sqs_test_queue = self.sqs_client.create_queue(QueueName=self.queue_name)
    ...
    def _test_distributed_tracing_sns_to_sqs(self, raw_message_delivery):
        with self.override_config("botocore", dict(distributed_tracing=True, propagation_enabled=True)):
            with mock.patch("time.time") as mt:
                ...
                Pin.get_from(sns)._clone(tracer=self.tracer)._onto(sns)
                Pin.get_from(self.sqs_client)._clone(tracer=self.tracer)._onto(self.sqs_client)
    ...
    def test_stepfunctions_send_start_execution_trace_injection(self):
        ...
        Pin(service=self.TEST_SERVICE, tracer=self.tracer)._onto(sf)
        sf.start_execution(
    ...
    def test_unpatch(self):
        ...
        Pin(service=self.TEST_SERVICE, tracer=self.tracer)._onto(kinesis)
    ...
    def test_eventbridge_single_entry_trace_injection(self):
        ...
        Pin(service=self.TEST_SERVICE, tracer=self.tracer)._onto(bridge)
    ...
    def test_kms_client(self):
        ...
        Pin(service=self.TEST_SERVICE, tracer=self.tracer)._onto(kms)
    ...
    def test_schematized_kms_client_default(self):
        kms = self.session.create_client("kms", region_name="us-east-1")
        Pin.get_from(kms)._clone(tracer=self.tracer)._onto(kms)
    ...
    def test_firehose_no_records_arg(self):
        ...
        Pin(service=self.TEST_SERVICE, tracer=self.tracer)._onto(firehose)
    ...
    def _test_sns(self, use_default_tracer=False):
        ...
        if use_default_tracer:
            ...
            Pin(service=self.TEST_SERVICE, tracer=self.tracer)._onto(sns)
    ...
    def test_sns_send_message_batch_trace_injection_with_no_message_attributes(self):
        with self.override_config("botocore", dict(distributed_tracing=True, propagation_enabled=True)):
            ...
            Pin(service=self.TEST_SERVICE, tracer=self.tracer)._onto(sns)
            Pin.get_from(sns)._clone(tracer=self.tracer)._onto(self.sqs_client)
            entries = [
                {
                    "Id": "1",
                    "Message": "ironmaiden",
