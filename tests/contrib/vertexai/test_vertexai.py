...
def test_global_tags(vertexai, mock_client, mock_tracer):
    ...
    """
    with patch.object(
        vertexai.generative_models.GenerativeModel, "_prediction_client", new_callable=PropertyMock
...
async def test_vertexai_completion_async(vertexai, mock_async_client):
    with patch.object(
        vertexai.generative_models.GenerativeModel, "_prediction_async_client", new_callable=PropertyMock
    ) as mock_client_property:
