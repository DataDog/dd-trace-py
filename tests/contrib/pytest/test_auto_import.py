...
class TestAutoImport:
    ...
    def test_with_real_pytest_ini(self, tmp_path):
        ...
        shutil.copy(os.path.join(os.path.dirname(__file__), "fixtures", "test_auto_import.py"), test_file)
        shutil.copy(os.path.join(os.path.dirname(__file__), "fixtures", "pytest.ini"), test_dir / "pytest.ini")
        ...
        result = subprocess.run(
            [sys.executable, "-m", "pytest", str(test_file)], capture_output=True, text=True, cwd=str(test_dir), env=env
        )
