...
class TestAutoImport:
    ...
    def test_ddtrace_run_imports_sitecustomize(self):
        ...
        result = subprocess.run(
            [sys.executable, '-m', 'ddtrace.run', test_script],
            capture_output=True,
            text=True
        )
    ...
    def test_pytest_plugin_imports_sitecustomize(self, pytest_plugin_enabled):
        """Test that the pytest plugin imports sitecustomize when enabled."""
        with mock.patch("ddtrace.bootstrap.sitecustomize") as mock_sitecustomize, \
             mock.patch("ddtrace.contrib.internal.pytest.plugin.is_enabled", return_value=pytest_plugin_enabled):
            
            # Import the pytest plugin module
            import ddtrace.contrib.internal.pytest.plugin  # noqa: F401
            
            # Verify sitecustomize was imported only if plugin is enabled
