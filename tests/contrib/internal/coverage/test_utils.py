...
class TestRemapLcovPaths:
    ...
    def test_remap_not_poisoned_by_canonical_filename_cache(self, tmp_path, monkeypatch):
        ...
        coverage.py's ``PathAliases.map`` calls ``canonical_filename`` which
        searches ``cwd`` and ``sys.path`` and caches the result globally.
        When the checkout sources are later removed (e.g. wheel-only CI), a
        stale cache entry can resolve the mapped path back to an installed
        package location, defeating the alias.  This test verifies that
        ``_remap_lcov_paths`` applies aliases directly without relying on
        ``canonical_filename``.
        """
        import re
        ...
        # Poison the canonical_filename cache by populating it from the
        # current cwd (simulating a stale entry from a previous cwd or
        # sys.path lookup that resolves to a different directory).
        canonical_filename("ddtrace/__init__.py")  # populate cache
