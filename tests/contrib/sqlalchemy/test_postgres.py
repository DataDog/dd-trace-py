...
class PostgresTestCase(SQLAlchemyTestMixin, TracerTestCase):
    ...
    def check_meta(self, span):
        ...
        self.assertEqual(span._get_str_attribute("out.host"), POSTGRES_CONFIG["host"])
        self.assertEqual(span._get_numeric_attribute("network.destination.port"), POSTGRES_CONFIG["port"])
    ...
    def test_engine_execute_errors(self):
        ...
        self.assertTrue('relation "a_wrong_table" does not exist' in span._get_str_attribute(ERROR_MSG))
        assert "psycopg2.errors.UndefinedTable" in span._get_str_attribute("error.type")
        assert 'UndefinedTable: relation "a_wrong_table" does not exist' in span._get_str_attribute("error.stack")
