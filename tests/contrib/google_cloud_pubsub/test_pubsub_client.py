...
def _assert_admin_span(span, method, resource_path, project_id="test-project"):
    ...
    assert span.resource == f"{method} {resource_path}"
...
def _assert_create_delete_spans(test_spans, topic_name):
    create_span = test_spans.find_span(name="gcp.pubsub.request", resource=f"createTopic {topic_name}")
    _assert_admin_span(create_span, "createTopic", topic_name)

    delete_span = test_spans.find_span(name="gcp.pubsub.request", resource=f"deleteTopic {topic_name}")
    _assert_admin_span(delete_span, "deleteTopic", topic_name)


class TestAdminOperations:
    def test_flat_kwargs(self, publisher, test_spans):
        topic_name = f"{PROJECT_PATH}/topics/admin-test-flat-kwargs"
    ...
    def test_positional_request_arg(self, publisher, test_spans):
        topic_name = f"{PROJECT_PATH}/topics/admin-test-positional"
    ...
    def test_keyword_request_arg(self, publisher, test_spans):
        topic_name = f"{PROJECT_PATH}/topics/admin-test-keyword"
...
class TestErrorHandling:
    def test_error_records_on_span(self, publisher, test_spans):
        with pytest.raises(Exception):
            publisher.get_topic(topic=f"{PROJECT_PATH}/topics/nonexistent-topic-xyz")
