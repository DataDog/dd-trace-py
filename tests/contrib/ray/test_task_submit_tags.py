"""Unit tests for Enrichment Task 8: ray.task.submit span scheduling-hint tags.

These tests verify that ``RaySubmissionEvent`` correctly carries the new
scheduling-hint fields and that ``RaySubmissionSubscriber.on_started`` stamps
...
def _make_ctx_with_span(event):
    """Create a mock context object that holds the given event and a live span.

    We can't easily exercise the full `core.context_with_event` lifecycle in
    unit tests without the full subscriber registration chain being active.
    Instead, we create a minimal context-like object so we can call
    `RaySubmissionSubscriber.on_started` directly and inspect the span.
    """
