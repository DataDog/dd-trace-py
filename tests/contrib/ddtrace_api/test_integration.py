...
class DDTraceAPITestCase(TracerTestCase):
    ...
    def test_set_traceback(self):
        ...
        assert spans[0]._get_attribute("error.stack") is not None
    ...
    def test_set_exc_info(self):
        ...
        spans = self._assert_real_spans()
        assert spans[0]._get_attribute("error.message") is not None
        assert spans[0]._get_attribute("error.stack") is not None
        assert spans[0]._get_attribute("error.type") is not None
    ...
    def test_set_tags(self):
        ...
        spans = self._assert_real_spans()
        assert spans[0]._get_attribute("tag1") == "value1", "Tag set via API should be applied to the real spans"
        assert spans[0]._get_attribute("tag2") == "value2", "Tag set via API should be applied to the real spans"
