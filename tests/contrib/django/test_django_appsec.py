...
def test_django_simple_attack(client, test_spans, tracer):
    ...
    assert root_span._store.kept_addresses["server.request.uri.raw"] == "http://testserver/.git?q=1"
    assert root_span._store.kept_addresses["server.request.query"]["q"] == "1"
    assert "Cookie" not in root_span._store.kept_addresses["server.request.headers.no_cookies"]
