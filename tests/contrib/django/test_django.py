...
def test_middleware_trace_error_500(client, test_spans):
    ...
    with modify_settings(**(
            ...
            dict(MIDDLEWARE_CLASSES={'append': 'tests.contrib.django.middleware.CatchExceptionMiddleware'})
    )):
...
def test_middleware_handled_view_exception_success(client, test_spans):
    ...
    with modify_settings(**(
            ...
            if django.VERSION >= (2, 0, 0) else
            dict(MIDDLEWARE_CLASSES={'append': 'tests.contrib.django.middleware.HandleErrorMiddlewareSuccess'})
    )):
