...
class TestRqTracingSync(TracerTestCase):
    ...
    def test_enqueue(self):
        ...
        assert span.get_tag("job.func_name") == "tests.contrib.rq.test_rq.job1"
...
class TestRqTracingAsync(TracerTestCase):
    def setUp(self):
    ...
    def test_simple_job_run(self):
        ...
        # The spans generated by the worker (which forks off to run the task)
        ...
        self.assert_structure(
            ...
            (
                dict(
                    ...
                    service="rq",
                    resource="tests.contrib.rq.test_rq.job1",
                    meta={
                        "queue.name": "test-queue",
                        "job.id": job.id,
                        "job.func_name": "tests.contrib.rq.test_rq.job1",
                    },
                ),
                dict(
                    name="rq.job.fetch",
        ...
        self.assert_structure(
            ...
            (
                dict(
                    name="rq.queue.enqueue_job",
                    service="rq",
                    resource="tests.contrib.rq.test_rq.job2",
                    meta={
                        "queue.name": "test-queue",
                        "job.id": job.id,
                        "job.func_name": "tests.contrib.rq.test_rq.job2",
                    },
                ),
                dict(
                    name="rq.job.fetch",
                ...
                (
                    ...
                    (dict(name="rq.job.fetch", service="rq-worker", meta={"job.id": job.id, "job.status": "failed"}),),
                ),
            ),
        )
