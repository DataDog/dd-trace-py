...
class CeleryOldStyleTaskTest(CeleryBaseTestCase):
    ...
    def test_apply_async_previous_style_tasks(self):
        ...
        assert run_span._get_str_attribute("celery.id") == res.task_id
        assert run_span._get_str_attribute("celery.action") == "run"
        assert run_span._get_str_attribute("celery.state") == "SUCCESS"
        ...
        assert apply_span._get_str_attribute("celery.action") == "apply_async"
        assert apply_span._get_str_attribute("celery.routing_key") == "celery"
        assert apply_span._get_str_attribute("component") == "celery"
        assert apply_span._get_str_attribute("span.kind") == "producer"
