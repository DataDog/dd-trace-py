...
class CeleryIntegrationTask(CeleryTestCase):
    ...
    def test_fn_task(self):
        ...
        eq_(1, len(traces))
        eq_(2, len(traces[0]))
        eq_('celery.task.run', traces[0][0].name)
        eq_('celery.task.apply', traces[0][1].name)
    ...
    def test_fn_exception(self):
        ...
        eq_('celery', traces[0][0].service)
        eq_('celery', traces[0][1].service)
        eq_('FAILURE', traces[0][1].get_tag('state'))
        eq_(1, traces[0][0].error)
        eq_('Task class is failing', traces[0][0].get_tag('error.msg'))
        ok_('Traceback (most recent call last)' in traces[0][0].get_tag('error.stack'))
