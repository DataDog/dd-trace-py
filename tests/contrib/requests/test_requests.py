...
class TestRequests(BaseRequestTestCase, TracerTestCase):
    ...
    def test_user_set_service_name(self):
        ...
        with self.override_config("requests", dict(service_name="clients")):
            out = self.session.get(URL_200)
            assert out.status_code == 200
            spans = self.pop_spans()
            assert len(spans) == 1
    ...
    def test_split_by_domain_with_ampersat(self):
        ...
        with self.override_config("requests", dict(split_by_domain=True, service_name="monkey_service")):
    ...
    def test_split_by_domain_precedence(self):
        ...
        with self.override_config("requests", dict(split_by_domain=True, service_name="intake")):
            out = self.session.get(URL_200)
            assert out.status_code == 200
    ...
    def test_global_config_service_env_precedence(self):
        ...
        with self.override_config("requests", dict(service="override2")):
            out = self.session.get(URL_200)
            assert out.status_code == 200
            spans = self.pop_spans()
