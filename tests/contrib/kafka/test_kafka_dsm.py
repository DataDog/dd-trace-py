...
class KafkaConsumerPollFilter(TraceFilter):
    def process_trace(self, trace):
        spans = []
        for span in trace:
            if span.name == "kafka.consume" and span.get_tag("kafka.message_offset") is None:
                continue
            spans.append(span)
        return spans
...
def get_topic_name(topic_name):
    return "{}_{}".format(topic_name, get_original_test_name())


# DEV: create_topics takes quite some time
def create_topic(name):
    """Create a kafka topic if not already created"""
    admin = kafka_admin.AdminClient({"bootstrap.servers": BOOTSTRAP_SERVERS})
    topics = [kafka_admin.NewTopic(name, num_partitions=1)]
