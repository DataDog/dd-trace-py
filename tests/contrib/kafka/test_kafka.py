...
def dummy_tracer():
    ...
    t._span_aggregagtor.writer._send_payload_with_backoff = t._span_aggregagtor.writer._send_payload
    yield t
    unpatch()
...
def tracer(should_filter_empty_polls):
    ...
    # disable backoff because it makes these tests less reliable
    previous_backoff = ddtracer._span_aggregagtor.writer._send_payload_with_backoff
    ddtracer._span_aggregagtor.writer._send_payload_with_backoff = ddtracer._span_aggregagtor.writer._send_payload
    try:
        yield ddtracer
    finally:
        ddtracer.flush()
        ddtracer._span_aggregagtor.writer._send_payload_with_backoff = previous_backoff
        unpatch()
...
def _generate_in_subprocess(random_topic):
    ...
    # disable backoff because it makes these tests less reliable
    ddtrace.tracer._span_aggregagtor.writer._send_payload_with_backoff = (
        ddtrace.tracer._span_aggregagtor.writer._send_payload
    )
    patch()
