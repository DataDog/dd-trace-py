...
class FlaskStaticFileTestCase(BaseFlaskTestCase):
    def test_serve_static_file(self):
        ...
        self.assertEqual(req_span._get_str_attribute("flask.endpoint"), "static")
        self.assertEqual(req_span._get_str_attribute("flask.url_rule"), "/static/<path:filename>")
        self.assertEqual(req_span._get_str_attribute("flask.view_args.filename"), "test.txt")
        assert_span_http_status_code(req_span, 200)
        self.assertEqual(req_span._get_str_attribute(http.URL), "http://localhost/static/test.txt")
    ...
    def test_serve_static_file_404(self):
        ...
        self.assertEqual(req_span._get_str_attribute("flask.endpoint"), "static")
        self.assertEqual(req_span._get_str_attribute("flask.url_rule"), "/static/<path:filename>")
        self.assertEqual(req_span._get_str_attribute("flask.view_args.filename"), "unknown-file")
        assert_span_http_status_code(req_span, 404)
        self.assertEqual(req_span._get_str_attribute(http.URL), "http://localhost/static/unknown-file")
        self.assertEqual(req_span._get_str_attribute("http.method"), "GET")
        ...
        self.assertEqual(send_file_span.name, "flask.send_static_file")
        self.assertEqual(send_file_span.resource, "flask.send_static_file")
        self.assertTrue(send_file_span._get_str_attribute(ERROR_MSG).startswith("404 Not Found"))
        self.assertTrue(send_file_span._get_str_attribute("error.stack").startswith("Traceback"))
        self.assertEqual(send_file_span._get_str_attribute("error.type"), "werkzeug.exceptions.NotFound")
