...
class PsycopgCore(TracerTestCase):
    ...
    def _get_conn(self, service=None):
        ...
        if pin:
            pin._clone(service=service, tracer=self.tracer)._onto(conn)
    ...
    def test_psycopg3_connection_with_string(self):
        ...
        Pin.get_from(conn)._clone(service="postgres", tracer=self.tracer)._onto(conn)
    ...
    def test_connection_instance_method_patch(self):
        ...
        if pin:
            pin._clone(service="postgres", tracer=self.tracer)._onto(connection)
