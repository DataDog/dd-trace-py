...
TEST_CASES = [
    pytest.param({"redis": "3.5.3"}, {"DD_TRACE_REDIS_ENABLED": "false"}, id="redis_incompatible"),  # Below >=4.0.0
    pytest.param({"redis": "4.6.0"}, {}, id="redis_compatible"),  # At/Above >=4.0.0
    pytest.param(
        {"requests": "0.2.0"}, {"DD_TRACE_REQUESTS_ENABLED": "false"}, id="requests_incompatible"  # Below >=2.0.0
    ),
    pytest.param({"requests": "2.28.1"}, {}, id="requests_compatible"),  # At/Above >=2.0.0
    pytest.param({"flask": "0.12.5"}, {"DD_TRACE_FLASK_ENABLED": "false"}, id="flask_incompatible"),  # Below >=1.0.0
    pytest.param({"flask": "2.0.0"}, {}, id="flask_compatible"),  # At/Above >=1.0.0
    pytest.param({"pymysql": "0.9.0"}, {}, id="pymysql_old_compatible"),  # Old version, but >=0.0.0 supported
    pytest.param({"pymysql": "1.1.0"}, {}, id="pymysql_new_compatible"),  # Newer version, >=0.0.0 supported
    pytest.param(
    ...
    pytest.param({"flask": "2.0.0", "bottle": "0.12.25"}, {}, id="mixed_flask_bottle_comp"),  # both compatible
    pytest.param(
    ...
    pytest.param({}, {}, id="no_relevant_packages"),  # No relevant packages installed
    pytest.param(
        {"click": "0.1"}, {}, id="unrelated_package"  # Use a real package ('click') that we don't integrate with
    ),
]
...
def test_integration_compatibility_guardrail(test_venv, packages_to_install, expected_disabled_vars):
    ...
    try:
        ...
        assert (
            output_env_vars.get("_DD_INJECT_WAS_ATTEMPTED") == "true"
