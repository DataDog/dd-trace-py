r"""Lifecycle stress / fuzzing tests for PeriodicThread.
...
- PR 17707 / 18040 - PeriodicThread.awake() blocked forever after stop().

If you are modifying ddtrace/internal/_threads.cpp (or the lifecycle of
ddtrace/internal/periodic.py), run this file with a larger budget before
opening a PR:

    # ~30s soak, fresh random seed:
    DD_STRESS_ITERS=10000 \\
        scripts/run-tests -- -- tests/internal/test_periodic_stress.py

    # Time-bounded soak (recommended under ASan/TSan):
    DD_STRESS_SECONDS=120 \\
        scripts/run-tests -- -- tests/internal/test_periodic_stress.py

    # Re-run a failing seed (the seed is printed to stderr by every run):
    DD_STRESS_SEED=12345 \\
        scripts/run-tests -- -- tests/internal/test_periodic_stress.py

Reproducibility knobs:
- DD_STRESS_SEED       - pin the RNG seed (default: random, printed to stderr)
- DD_STRESS_ITERS      - iteration count (default: 200)
- DD_STRESS_SECONDS    - wall-clock budget; overrides iters when > 0
- DD_STRESS_POOL       - pool size of concurrent PeriodicThread objects
- DD_STRESS_FORK_EVERY - force a fork every N iterations (default: 50)
- DD_STRESS_TRACE_FILE - log every op to this file (useful for hang triage)

The default configuration (200 iters, ~4 forks) is kept short enough for
normal CI - usually well under 10 seconds.
...
def test_periodic_thread_lifecycle_stress():
    ...
    def _do_fork():
        ...
        if pid == 0:
            ...
            assert os.WEXITSTATUS(status_) == 0, "child %d exit=%d; seed=%d, child_seed=%d" % (
                pid,
                os.WEXITSTATUS(status_),
                seed,
                child_seed,
            )
    ...
    while True:
        ...
        try:
            ...
            sys.stderr.write("[stress] step=%d op=%s idx=%d raised %s: %s\n" % (step, op, idx, type(e).__name__, e))
            raise
    ...
    sys.stderr.write("[stress] done: seed=%d steps=%d forks=%d elapsed=%.2fs\n" % (seed, step, forks_done[0], elapsed))
