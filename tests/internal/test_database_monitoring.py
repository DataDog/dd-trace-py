...
def test_injection_mode_configuration():
    ...
    with override_env(dict(DD_TRACE_SQL_COMMENT_PROPAGATION_MODE="service")):
    ...
    with override_env(dict(DD_TRACE_SQL_COMMENT_PROPAGATION_MODE="full")):
        config = _database_monitoring.DatabaseMonitoringConfig()
        assert config.injection_mode == "full"

    # Ensure an invalid injection mode raises a ValueError
    with override_env(dict(DD_TRACE_SQL_COMMENT_PROPAGATION_MODE="notaninjectionmode")):
        with pytest.raises(ValueError) as excinfo:
            _database_monitoring.DatabaseMonitoringConfig()
    assert (
        excinfo.value.args[0] == "Invalid value for environment variable DD_TRACE_SQL_COMMENT_PROPAGATION_MODE: "
        "value must be one of ['disabled', 'full', 'service']"
    )


@pytest.mark.subprocess(env=dict(DD_TRACE_SQL_COMMENT_PROPAGATION_MODE="disabled"))
...
@pytest.mark.subprocess(
    env=dict(
        DD_TRACE_SQL_COMMENT_PROPAGATION_MODE="full",
        DD_SERVICE="orders-app",
        DD_ENV="staging",
        DD_VERSION="v7343437-d7ac743",
    )
