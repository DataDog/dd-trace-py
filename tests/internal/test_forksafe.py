...
def test_service_lock_held_by_forking_thread_unwinds_in_child():
    ...
    class TestService(service.Service):
        child_pid: typing.Optional[int] = None
        start_count = 0

        def __init__(self):
            super().__init__()
            self.child_hook_acquired_lock = False
            self.competing_thread_acquired_lock = False
            self.post_unwind_thread_acquired_lock = False
            self.competing_thread: typing.Optional[threading.Thread] = None
            self.post_unwind_thread: typing.Optional[threading.Thread] = None
    ...
    if test_service.child_pid == 0:
        ...
        child_ok = (
            ...
            and test_service.competing_thread is not None
    ...
    assert test_service.child_pid is not None
