...
def test_periodic_thread_naming():
    ...
    # On Linux (15 char limit), should keep "StackCollectorThread" -> truncated to "StackCollector"
    ...
    if native_name[0] is not None:
        if system == "Linux":
            ...
            assert native_name[0] == "VeryLongThread", f"Expected 'VeryLongThread' on Linux, got '{native_name[0]}'"
        elif system == "Darwin":
    ...
    if native_name[0] is not None:
        # On all platforms, the class name should be preferred
        assert native_name[0].endswith("ClassNameFit"), f"Expected 'ClassNameFit', got '{native_name[0]}'"
