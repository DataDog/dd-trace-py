...
from ddtrace.instrument import (
    InstallException,
    install,
    install_all,
    integration_modname,
...
@mock.patch('ddtrace.instrument._BASE_MODULENAME', 'ddtrace.contrib')
class TestInstall(object):
    ...
    def test_install_integration_does_not_exist(self):
        ...
        When raise_errors is True
            ``install`` should raise an InstallException
        """
        with mock.patch('ddtrace.instrument.log') as log_mock:
            install('nonexistent_integration')
            log_mock.error.assert_called_once_with(
                'install: integration nonexistent_integration not found'
            )

        with pytest.raises(InstallException) as error:
    ...
    @mock.patch('ddtrace.instrument.DEFAULT_INTEGRATIONS', {
        'integration1': True,
        'integration2': True,
        'integration3': True,
    })
    ...
    def test_install_all_integration_does_not_exist(self):
        ...
        When raise_errors is True
            ``install_all`` should raise an InstallException
        """
        with mock.patch('ddtrace.instrument.log') as log_mock:
            install_all()
            calls = [
        ...
        with pytest.raises(InstallException):
    ...
    def test_patch_integration_does_not_exist(self):
        ...
        When raise_errors is False
            ``patch`` should not raise an InstallException
        When raise_errors is not defined
