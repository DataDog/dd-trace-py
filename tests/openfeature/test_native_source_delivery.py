...
class FixtureHandler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        if not self.headers.get("DD-API-KEY"):
            self._send(401, b"")
            return
        if self.state.fixture == "malformed":
            self._send(200, b'{"flags": ', {"ETag": '"bad-ufc"'})
            return
        if self.state.fixture == "unchanged" and self.headers.get("If-None-Match") == self.state.etag:
            self._send(304, b"")
            return
...
def _native_config(url, api_key="test-api-key"):
...
def _set_native_cdn_env(monkeypatch, url, api_key="test-api-key"):
    monkeypatch.setenv("DD_FLAGGING_SOURCE_MODE", "cdn")
    monkeypatch.setenv("DD_FLAGGING_CDN_DELIVERY", "native")
...
def test_python_glue_passes_explicit_config_to_native(monkeypatch):
    captured = {}

    class FakeNativeSourceDelivery:
        def __init__(
            self,
            base_url,
            api_key,
    ...
    NativeOwnedCdnSource(
        _native_config("http://127.0.0.1:8123/mock/ufc/config", api_key="explicit-key"),
        delivery_cls=FakeNativeSourceDelivery,
    )
...
def test_native_source_delivery_provider_ready_evaluates_and_shutdown_is_visible(monkeypatch, fixture_server):
    state, url = fixture_server
    _set_native_cdn_env(monkeypatch, url)
    ...
    assert isinstance(source, NativeOwnedCdnSource)
    ...
    assert source.is_started is False


def test_native_source_delivery_maps_cold_failures_to_not_ready(monkeypatch, fixture_server):
    _, url = fixture_server
    _set_native_cdn_env(monkeypatch, url, api_key=None)
...
def test_native_source_delivery_preserves_lkg_on_malformed_warm_and_uses_etag(monkeypatch, fixture_server):
    state, url = fixture_server
    _set_native_cdn_env(monkeypatch, url)
