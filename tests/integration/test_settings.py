...
assert span.get_metric("_dd.rule_psr") is None, "(second time) unsetting remote config trace sample rate"
    ...
    events = test_agent_session.get_events(subprocess=True)
    events_trace_sample_rate = _get_telemetry_config_items(events, "_DD_TRACE_SAMPLE_RATE")
    assert {"name": "_DD_TRACE_SAMPLE_RATE", "value": 1.0, "origin": "default"} in events_trace_sample_rate
