...
def _get_telemetry_config_items(events, item_name):
    items = []
    for event in events:
        for item in event.get("payload", {}).get("configuration", []):
            if item_name == item["name"]:
                items.append(item)
    # reverse items, to get the more recent items first
    return list(reversed(items)) or None
...
assert span.get_metric("_dd.rule_psr") is None, "(second time) unsetting remote config trace sample rate"
    ...
    events = test_agent_session.get_events()
    events_trace_sample_rate = _get_telemetry_config_items(events, "DD_TRACE_SAMPLE_RATE")
    assert {"name": "DD_TRACE_SAMPLE_RATE", "value": "None", "origin": "remote_config"} in events_trace_sample_rate
