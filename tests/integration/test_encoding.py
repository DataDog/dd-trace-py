...
def _assert_bad_trace_refused_by_agent(mock_log):
...
def test_simple_trace_accepted_by_agent():
    tracer = Tracer()
    tracer.configure(writer=AgentWriter(agent.get_trace_url()))
...
def test_wrong_span_name_type_refused_by_agent():
    ...
    _assert_bad_trace_refused_by_agent(log)
...
def test_trace_with_meta_accepted_by_agent(tags):
...
def test_trace_with_metrics_accepted_by_agent(metrics):
...
def test_trace_with_wrong_meta_types_refused_by_agent(meta):
    tracer = Tracer()
    tracer.configure(writer=AgentWriter(agent.get_trace_url()))
    with mock.patch("ddtrace.internal.writer.log") as log:
        with tracer.trace("root") as root:
...
def test_trace_with_wrong_metrics_types_refused_by_agent(metrics):
    tracer = Tracer()
    tracer.configure(writer=AgentWriter(agent.get_trace_url()))
    with mock.patch("ddtrace.internal.writer.log") as log:
        with tracer.trace("root") as root:
