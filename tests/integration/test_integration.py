...
def test_tracer_flare(ddtrace_run_python_code_in_subprocess):
    import tarfile

    from ddtrace.internal.utils.flare import Flare
...
"""
    ...
    try:
        env = os.environ.copy()
        env["DD_TRACE_FLARE_ENABLED"] = "true"
        _, err, status, _ = ddtrace_run_python_code_in_subprocess(code, env=env)
        assert status == 0, err
        assert err == b""

        # assert tar file was generated when the global tracer is initialized
        tar_path = Flare.get_tar_path()
        assert os.path.exists(tar_path)

        # assert tar file contains tracer flare file
        with tarfile.open(tar_path, "r:bz2") as t:
            assert TracerFlare.FLARE_FILE in t.getnames()
        # ensure tracer flare file was deleted when the file was moved to tar
        assert not os.path.exists(TracerFlare.FLARE_FILE)
    finally:
        # delete tar generated by this test (if it exists)
        tar_path = Flare.get_tar_path()
        if os.path.exists(tar_path):
            os.remove(tar_path)
        # delete tracer flare generated by this test (if it wasn't already removed)
        if os.path.exists(TracerFlare.FLARE_FILE):
            os.remove(TracerFlare.FLARE_FILE)
