...
def test_payload_too_large():
    ...
    encoding = os.environ["DD_TRACE_API_VERSION"]
    assert t._span_aggregagtor.writer._max_payload_size == FOUR_KB
    assert t._span_aggregagtor.writer._buffer_size == FOUR_KB
    with mock.patch("ddtrace.internal.writer.writer.log") as log:
...
def test_resource_name_too_large():
    ...
    try:
        ...
        pytest.fail()
    encoded_spans, size = t._span_aggregagtor.writer._encoder.encode()
    assert size == 1
...
def test_metrics():
    ...
    assert t._span_aggregagtor.partial_flush_min_spans == 300

    with override_global_config(dict(_health_metrics_enabled=True)):
        statsd_mock = mock.Mock()
        t._span_aggregagtor.writer.dogstatsd = statsd_mock
        with mock.patch("ddtrace.internal.writer.writer.log") as log:
            for _ in range(2):
                spans = []
                for _ in range(600):
...
def test_single_trace_too_large():
    ...
    long_string = "a" * 250
    assert t._span_aggregagtor.partial_flush_enabled is True
...
def test_validate_headers_in_payload_to_intake():
    ...
    t._span_aggregagtor.writer._put = mock.Mock(wraps=t._span_aggregagtor.writer._put)
    t.trace("op").finish()
    t.shutdown()
    assert t._span_aggregagtor.writer._put.call_count == 1
    headers = t._span_aggregagtor.writer._put.call_args[0][1]
...
def test_trace_with_invalid_client_endpoint_generates_error_log():
    ...
    calls = [
        mock.call(
            ...
            "/bad",
            404,
            t._span_aggregagtor.writer.agent_url,
        )
    ]
    log.error.assert_has_calls(calls)
...
def test_api_version_downgrade_generates_no_warning_logs():
    ...
    t._span_aggregagtor.writer.api_version = "v0.5"
    t._span_aggregagtor.writer._downgrade(Response(status=404), t._span_aggregagtor.writer._clients[0])
    assert t._span_aggregagtor.writer._endpoint == "v0.4/traces"
    with mock.patch("ddtrace.internal.writer.writer.log") as log:
...
def test_writer_configured_correctly_from_env():
    ...
    assert ddtrace.tracer._span_aggregagtor.writer._encoder.max_size == 1000
    assert ddtrace.tracer._span_aggregagtor.writer._encoder.max_item_size == 1000
    assert ddtrace.tracer._span_aggregagtor.writer._interval == 5.0
...
def test_writer_configured_correctly_from_env_defaults():
    ...
    assert ddtrace.tracer._span_aggregagtor.writer._encoder.max_size == 20 << 20
    assert ddtrace.tracer._span_aggregagtor.writer._encoder.max_item_size == 20 << 20
    assert ddtrace.tracer._span_aggregagtor.writer._interval == 1.0
