...
@pytest.mark.subprocess(env=dict(DD_API_KEY="foobar.baz", DD_SITE="foo.bar", DD_CIVISIBILITY_AGENTLESS_ENABLED="0"))
def test_civisibility_intake_with_evp_available():
    ...
    CIVisibility.enable(tracer=ddtrace.tracer)
    ...
    assert CIVisibility._instance.tracer._writer._headers[EVP_SUBDOMAIN_HEADER_NAME] == EVP_SUBDOMAIN_HEADER_EVENT_VALUE
...
@pytest.mark.subprocess(env=dict(DD_API_KEY="foobar.baz", DD_SITE="foo.bar", DD_CIVISIBILITY_AGENTLESS_ENABLED="1"))
...
@pytest.mark.subprocess(env=dict(DD_API_KEY="foobar.baz"))
