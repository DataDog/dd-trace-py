...
On 3.15+ the monitoring path swaps in a cloned code object while wrapped but restores the
exact original ``__code__`` object on unwrap, so identity is preserved.
...
@pytest.mark.xfail(
    strict=True,
    condition=sys.version_info < NEXT_MAX_PY,
