...
def test_evaluate_method(
    mock_execute_request, ai_guard_client, tracer, action, reason, blocking, suite, target, messages
):
    ...
    if should_block:
        with pytest.raises(AIGuardAbortError, check=lambda e: e.action == action and e.reason == reason):
...
def test_evaluate_http_error(mock_execute_request, ai_guard_client):
    ...
    with pytest.raises(AIGuardClientError, check=lambda e: e.status == 500 and e.errors == errors):
...
@patch("ddtrace.appsec.ai_guard._api_client.AIGuardClient._execute_request")
def test_span_meta_messages_truncation(mock_execute_request, ai_guard_client, tracer):
