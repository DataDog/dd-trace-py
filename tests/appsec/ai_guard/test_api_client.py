...
from ddtrace.appsec.ai_guard import AIGuardClient, AIGuardClientError, AIGuardAbortError, AIGuardWorkflow
...
def _assert_mock_evaluate_call(mock_post, ai_guard_client: AIGuardClient, history: List[Evaluation],
                               current: Evaluation):
...
def test_evaluate_tool_allow(mock_post, ai_guard_client, tracer):
    ...
    assert result == True
    _assert_ai_guard_span(tracer,
                          {"ai_guard.target": "tool", "ai_guard.action": "ALLOW", "ai_guard.tool_name": "shell"})
    _assert_mock_evaluate_call(mock_post, ai_guard_client,
...
def test_evaluate_tool_abort(mock_post, ai_guard_client, tracer):
    ...
    _assert_ai_guard_span(tracer, {"ai_guard.target": "tool", "ai_guard.action": "ABORT",
...
def test_evaluate_prompt_abort(mock_post, ai_guard_client, tracer):
    ...
    _assert_ai_guard_span(tracer, {"ai_guard.target": "prompt", "ai_guard.action": "ABORT",
...
def test_evaluate_http_error(mock_post, ai_guard_client, tracer):
    ...
    with pytest.raises(AIGuardClientError, match="AI Guard service returned an unexpected error"):
        ...
        workflow.evaluate_tool("shell", {"cmd": ["sh", "-c", "rm --rf /"]})
        _assert_ai_guard_span(tracer,
                              {"ai_guard.target": "tool", "ai_guard.action": "ERROR", "ai_guard.tool_name": "shell"})
