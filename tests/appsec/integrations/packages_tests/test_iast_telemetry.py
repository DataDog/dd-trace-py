...
def test_metric_instrumented_sqli_sqlite3(test_agent_session, telemetry_writer):
    sqli_sqlite_unpatch()
    with override_global_config(dict(_iast_enabled=True, _iast_telemetry_report_lvl=TELEMETRY_INFORMATION_NAME)):
        sqli_sqlite3_patch()

    _assert_instrumented_sink(test_agent_session, telemetry_writer, VULN_SQL_INJECTION)


def test_metric_instrumented_sqli_sqlalchemy_patch(test_agent_session, telemetry_writer):
    sqlalchemy_unpatch()
    with override_global_config(dict(_iast_enabled=True, _iast_telemetry_report_lvl=TELEMETRY_INFORMATION_NAME)):
