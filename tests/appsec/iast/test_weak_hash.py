...
def test_weak_hash_hashlib_no_digest(iast_span_md5_and_sha1_configured, hash_func):
    ...
    span_report = _context.get_item(IAST_CONTEXT_KEY, span=iast_span_md5_and_sha1_configured)
...
def test_weak_hash_new_with_child_span(tracer, iast_span_defaults):
    ...
    assert list(span_report1.vulnerabilities)[0].location.path.endswith("tests/appsec/iast/fixtures/weak_algorithms.py")
    ...
    assert list(span_report2.vulnerabilities)[0].location.path.endswith("tests/appsec/iast/fixtures/weak_algorithms.py")
...
def test_weak_hash_md5_builtin_py3_only_md4_configured(iast_span_only_md4):
    ...
    m.update(b" the spammish repetition")
    m.digest()
    span_report = _context.get_item(IAST_CONTEXT_KEY, span=iast_span_only_md4)
...
def test_weak_hash_md5_builtin_py3_only_md5_configured(iast_span_only_md5):
    ...
    m.update(b" the spammish repetition")
    m.digest()
    span_report = _context.get_item(IAST_CONTEXT_KEY, span=iast_span_only_md5)
...
def test_weak_hash_md5_builtin_py3_only_sha1_configured(iast_span_only_sha1):
    ...
    m.update(b" the spammish repetition")
    m.digest()
    span_report = _context.get_item(IAST_CONTEXT_KEY, span=iast_span_only_sha1)
