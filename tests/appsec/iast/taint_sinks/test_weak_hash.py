...
def test_weak_hash_new_with_child_span(tracer, iast_span_defaults):
    with tracer.trace("test_child"):
        hashlib_new()
        span_report1 = core.get_item(IAST.CONTEXT_KEY)

    span_report2 = core.get_item(IAST.CONTEXT_KEY)
...
def test_weak_hash_deduplication_expired_cache(
    ...
    for i in range(10):
        with iast_context_span_deduplication_enabled(deduplication_enabled, time_lapse):
            time.sleep(0.002)
            m = hashlib.new("md5")
