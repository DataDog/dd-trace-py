...
class Contrib_TestClass_For_Threats(_Contrib_TestClass_Base):
    ...
    def test_exploit_prevention(
        ...
        with (
            ...
            mock_patch.object(ddtrace.internal.telemetry.telemetry_writer, "_add_metric_point", MagicMock()) as mocked,
        ):
            self.update_tracer(interface)
    ...
    def test_auto_user_events_sdk_v2(
        ...
        with (
            ...
            mock_patch.object(
                ddtrace.internal.telemetry.telemetry_writer, "_add_metric_point", MagicMock()
            ) as telemetry_mock,
        ):
            self.update_tracer(interface)
            metadata = json.dumps(
