...
from ddtrace.aiguard.integrations._openai_chat import _convert_openai_messages
from ddtrace.aiguard.integrations._openai_chat import _convert_openai_response
...
def reset_ai_guard_loaded():
    ...
    ``load_ai_guard()`` flips this flag to ``False`` after its first successful
...
def test_chat_sync_block_is_openai_compatible_exception(mock_execute_request, openai_client):
    ...
    assert type(err).__module__ == "ddtrace.aiguard.integrations.openai"
...
def test_block_error_class_identity_under_concurrent_lazy_import():
    ...
    module_name = "ddtrace.aiguard.integrations._openai_errors"
...
# ---------------------------------------------------------------------------
# DD_AI_GUARD_ENABLED — load_ai_guard() registration gating
#
# The flag is evaluated ONCE at ``load_ai_guard()`` time (lazy, idempotent via
...
def test_init_ai_guard_disabled_does_not_register_listeners(reset_ai_guard_loaded):
    ...
    with patch("ddtrace.aiguard._listener.ai_guard_listen") as mock_listen:
        with override_ai_guard_config(dict(_ai_guard_enabled=False)):
            ai_guard_mod.load_ai_guard()
...
def test_chat_sync_block_server_side_disabled_wins(openai_client):
    ...
    """
    with patch("ddtrace.aiguard._api_client.AIGuardClient._execute_request") as mock_execute_request:
