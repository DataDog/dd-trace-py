...
@pytest.mark.subprocess(err=None)
@pytest.mark.parametrize("auto_enable_crashtracking", [False])
def test_crashtracker_identity_refresh_reconfigure_errors_are_best_effort():
    from contextlib import ExitStack
    ...
    with ExitStack() as stack:
        ...
        stack.enter_context(mock.patch.object(crashtracking, "CrashtrackerStatus", status, create=True))
        stack.enter_context(
            mock.patch.object(crashtracking, "_get_args", mock.Mock(return_value=(object(), object(), object())))
        )
        stack.enter_context(
            mock.patch.object(crashtracking, "crashtracker_status", mock.Mock(return_value=initialized), create=True)
        )
        stack.enter_context(
            mock.patch.object(
                crashtracking, "crashtracker_reconfigure", mock.Mock(side_effect=RuntimeError("boom")), create=True
            )
        )

        crashtracking._reconfigure_for_identity_refresh({"service": "identity-refresh"})


@pytest.mark.skipif(not sys.platform.startswith("linux"), reason="Linux only")
@pytest.mark.subprocess(err=None)
@pytest.mark.parametrize("auto_enable_crashtracking", [False])
def test_crashtracker_identity_refresh_uses_initial_additional_tags():
    from contextlib import ExitStack
    ...
    def get_args(additional_tags):
        seen_tags.append(dict(additional_tags))
        return args
    ...
    with ExitStack() as stack:
        ...
        stack.enter_context(mock.patch.object(crashtracking, "_identity_refresh_additional_tags", None))
        stack.enter_context(mock.patch.object(crashtracking, "_get_args", get_args))
        stack.enter_context(mock.patch.object(crashtracking, "crashtracker_init", mock.Mock(), create=True))
        stack.enter_context(mock.patch.object(crashtracking, "crashtracker_reconfigure", mock.Mock(), create=True))
        stack.enter_context(
            mock.patch.object(crashtracking, "crashtracker_status", mock.Mock(return_value=initialized), create=True)
        )
    ...
    assert seen_tags == [{"service": "identity-refresh"}, {"service": "identity-refresh"}]


@pytest.mark.skipif(not sys.platform.startswith("linux"), reason="Linux only")
@pytest.mark.subprocess(err=None)
@pytest.mark.parametrize("auto_enable_crashtracking", [False])
def test_crashtracker_identity_refresh_skips_first_new_fork_generation():
    from contextlib import ExitStack
    ...
    with ExitStack() as stack:
        ...
        stack.enter_context(
            mock.patch.object(crashtracking.forksafe, "get_generation", mock.Mock(side_effect=[0, 1, 1]))
        )

        assert crashtracking.start({"service": "identity-refresh"})
        runtime.refresh_identity()
        runtime.refresh_identity()
