...
@pytest.mark.parametrize(
    ...
    [
        # All match
        (
            create_span(name="test.span", service="my-service",),
            SamplingRule(sample_rate=1, name="test.span", service=re.compile(r"^my-"),),
            True,
        ),
        ...
        (
            create_span(name="test.span", service="my-service",),
            SamplingRule(sample_rate=0, name="test.span", service=re.compile(r"^my-"),),
            True,
        ),
        # Name doesn't match
        (
            create_span(name="test.span", service="my-service",),
            SamplingRule(sample_rate=1, name="test_span", service=re.compile(r"^my-"),),
            False,
        ),
        # Service doesn't match
        (
            create_span(name="test.span", service="my-service",),
            SamplingRule(sample_rate=1, name="test.span", service=re.compile(r"^service-"),),
            False,
        ),
    ],
)
...
def test_sampling_rule_matches_exception():
    ...
    with mock.patch("ddtrace.sampler.log") as mock_log:
        ...
        mock_log.warning.assert_called_once_with(
            "%r pattern %r failed with %r", rule, pattern, "test.span", exc_info=True,
        )
