...
def print_tracemalloc_stats(interval):
    while True:
        time.sleep(interval)
        snapshot = tracemalloc.take_snapshot()
        total_allocations = sum(stat.count for stat in snapshot.statistics('lineno'))
        total_size = sum(stat.size for stat in snapshot.statistics('lineno'))

        print(f"Total allocations: {total_allocations}")
        print(f"Total size: {total_size} bytes")
        print(f"Current size: {tracemalloc.get_traced_memory()[0]} bytes, Total allocated size: {tracemalloc.get_traced_memory()[1]} bytes")
...
# Start a separate thread for printing tracemalloc statistics every 60 seconds
stats_thread = threading.Thread(target=print_tracemalloc_stats, args=(60,))
stats_thread.daemon = True  # Mark the thread as daemon so it exits when the main program exits
stats_thread.start()
