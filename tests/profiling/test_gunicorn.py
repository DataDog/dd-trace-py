...
def _run_gunicorn(*args: str) -> subprocess.Popen[bytes]:
    print("Running gunicorn with command: ", args)
    cmd = (
        [
            "ddtrace-run",
            "gunicorn",
...
def _test_gunicorn(
    ...
    *args: str,
    ...
    # # Print all pprof files in
    # debug_print(f"Pprof files: {glob.glob(f'{filename}.*.pprof', recursive=True)}")

    # # Print the contents of each pprof
    # for pprof_file in sorted(glob.glob(f"{filename}.*.pprof", recursive=True)):
    #     print("Reading pprof", pprof_file)
    #     import zstandard as zstd

    #     with open(pprof_file, "rb") as fp:
