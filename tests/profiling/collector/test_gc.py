...
def test_on_gc_records_pause_gctime() -> None:
    ...
    config_handle.push_gctime.assert_called_once_with(0, 0)
    config_handle.push_walltime.assert_not_called()
...
def test_snapshot_emits_config_sample() -> None:
    ...
    # during snapshot() would also call SampleHandle.push_gctime.
    col: GCCollector = _make_isolated_collector()
    ...
    mock_handle.push_gctime.assert_called_once_with(0, 2)
    mock_handle.push_walltime.assert_not_called()
...
def _has_gc_collect_frame(profile: pprof_pb2.Profile, sample: pprof_pb2.Sample) -> bool:
    return any(
        "gc.collect" in pprof_utils.get_location_from_id(profile, loc_id).function_name for loc_id in sample.location_id
    )
