...
def test_native_heap_ownership_handoff_end_to_end() -> None:
    ...
    # Sanity: prove patched GOT hooks intercept malloc before we assert on
    # managed-OBJ allocations. Use the gotter cdylib's own malloc probe (not
    # ctypes.CDLL, which dlsym-resolves libc and bypasses patched GOT slots).
    probe_before: "int | None" = heap_gotter.test_hook_hits()
    probe_ptr: "int | None" = heap_gotter.test_malloc_probe(64)
    assert probe_ptr, "gotter malloc probe failed after install"
    ...
    assert probe_after > probe_before, (
        f"gotter install must intercept malloc (probe hook-hit delta {probe_after - probe_before})"
    )
    heap_gotter.test_free_probe(probe_ptr)
