...
def test_coverage_id_clash_does_not_affect_ddtrace():
    ...
    ddtrace uses its own tool slot (_DD_TOOL_ID=4) and never touches COVERAGE_ID, so a
...
def test_dd_tool_slot_clash_causes_graceful_degradation():
    """If _DD_TOOL_ID (slot 4) is already taken, ddtrace silently skips coverage collection."""
    import os
    from pathlib import Path
    import sys
    ...
    # Claim ddtrace's tool slot before install — should cause graceful degradation
    sys.monitoring.use_tool_id(_DD_TOOL_ID, "something_else")
