...
def test_exact_match(key):
    ...
    m = matcher.match_levhenstein(key, 0)
...
def test_exact_match_fail(key):
    ...
    assert matcher.match_levhenstein(key, 0) is None


@pytest.mark.parametrize(
    "key,expected,distance", [("fooo", "foo", 1), ("bor", "bar", 1), ("az", "baz", 1), ("fooba", "foo", 2)]
)
def test_match_fuzzy(key, expected, distance):
    ...
    m = matcher.match_levhenstein(key)
    assert m is not None
...
def test_match_fuzzy_under_max_dist(key, expected, max_distance):
    ...
    m = matcher.match_levhenstein(key, max_distance)
    assert m is not None
    assert m.match == expected
