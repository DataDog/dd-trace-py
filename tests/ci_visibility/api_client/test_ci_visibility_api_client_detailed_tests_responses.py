...
from tests.ci_visibility.api_client._util import _get_detailed_tests_api_response
...
class TestTestVisibilityAPIClientDetailedTestResponses(TestTestVisibilityAPIClientBase):
    ...
    @pytest.mark.parametrize(
        ...
        [
            # Defaults
            (_get_detailed_tests_api_response({}), set()),
            # Single item
            (
                _get_detailed_tests_api_response({"module1": {"suite1.py": ["test1"]}}),
    ...
    def test_civisibility_api_client_unique_tests_parsed(self, unique_test_response, expected_tests):
        ...
        with mock.patch.object(client, "_do_request", return_value=unique_test_response):
            assert client.fetch_test_details().unique_test_ids() == expected_tests
