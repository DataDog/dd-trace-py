...
def writer():
    ...
    w = RuntimeCoverageWriter(processing_interval=0.1, timeout=2.0)
    w.start()
    yield w
    w.stop()
...
class TestRuntimeCoverageWriterInitialization:
    ...
    def test_writer_initialization_evp_mode(self):
        ...
        writer = RuntimeCoverageWriter(use_evp=True)

        assert writer._use_evp is True
        assert writer.RETRY_ATTEMPTS == 3
        assert writer.HTTP_METHOD == "POST"
        assert writer.STATSD_NAMESPACE == "runtime_coverage.writer"
        # Verify coverage client was created
        assert len(writer._clients) == 1
    ...
    def test_writer_initialization_agentless_mode(self):
        ...
        with override_global_config({"_ci_visibility_agentless_url": "https://citestcov-intake.datadoghq.com"}):
            writer = RuntimeCoverageWriter(use_evp=False)
    ...
    def test_writer_initialization_custom_interval(self):
        ...
        writer = RuntimeCoverageWriter(processing_interval=5.0)

        assert writer._interval == 5.0

    def test_writer_initialization_custom_timeout(self):
        """Test writer respects custom timeout."""
        writer = RuntimeCoverageWriter(timeout=10.0)

        assert writer._timeout == 10.0

    @mock.patch("ddtrace.internal.ci_visibility.runtime_coverage_writer._create_coverage_client")
    def test_writer_creates_coverage_client(self, mock_create_client):
        ...
        _writer = RuntimeCoverageWriter()

        # Verify coverage client was created with correct parameters
        mock_create_client.assert_called_once()
        call_kwargs = mock_create_client.call_args[1]
        assert "use_evp" in call_kwargs
        assert "intake_url" in call_kwargs
        assert "itr_suite_skipping_mode" in call_kwargs
        assert call_kwargs["itr_suite_skipping_mode"] is True
