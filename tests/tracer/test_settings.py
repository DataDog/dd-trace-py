...
class TestHttpConfig(BaseTestCase):
    def test_trace_headers(self):
        ...
        assert "some_header" in http_config.traced_headers

    def test_trace_headers_whitelist_case_insensitive(self):
        http_config = HttpConfig()
        http_config.trace_headers(["some_header", "Some_Header"])
        assert "some_header" in http_config.traced_headers
        assert "Some_Header" not in http_config.traced_headers

    def test_trace_multiple_headers(self):
        http_config = HttpConfig()
        http_config.trace_headers(["some_header_1", "some_header_2"])
        assert "some_header_1" in http_config.traced_headers
        assert "some_header_2" in http_config.traced_headers
        assert "some_header_3" not in http_config.traced_headers
...
class TestIntegrationConfig(BaseTestCase):
    ...
    def test_allow_configuring_http(self):
        ...
        assert "integration_header" in self.integration_config.http.traced_headers
        assert "other_header" not in self.integration_config.http.traced_headers

    def test_allow_exist_both_global_and_integration_config(self):
        self.config.trace_headers("global_header")
        assert "global_header" in self.integration_config.traced_headers

        self.integration_config.http.trace_headers("integration_header")
        assert "integration_header" in self.integration_config.traced_headers

        assert "global_header" not in self.integration_config.http.traced_headers
        assert "integration_header" not in self.config.http.traced_headers
