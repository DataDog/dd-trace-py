...
class TracerTestCases(TracerTestCase):
    ...
    def test_adding_services(self):
        ...
        self.start_span("child", service="two", child_of=root)
...
def test_ctx_distributed():
    ...
    tracer.activate(ctx)
    assert tracer.active_span() is None
    assert tracer.active() == Context(span_id=1234, trace_id=4321)

    with tracer.trace("test") as s1:
        assert tracer.active_span() == s1
        assert tracer.active_root_span() == s1
        assert tracer.get_call_context().trace_id == s1.trace_id == 4321
        assert tracer.get_call_context().span_id == s1.span_id
        assert s1.parent_id == 1234

    trace = tracer.writer.pop_traces()
    assert len(trace) == 1
    assert s1.metrics[SAMPLING_PRIORITY_KEY] == 2
    assert s1.meta[ORIGIN_KEY] == "somewhere"
