...
def test_native_logger_configured(mock_configure):
    import ddtrace

    ddtrace.trace.Tracer()

    mock_configure.assert_called_once_with(output="file", path="native.log")
    ddtrace.trace.tracer = None


@pytest.mark.subprocess(env={"DD_NATIVE_LOGGING_ENABLED": "0"})
def test_native_logger_disabled():
    import mock
    import ddtrace
