...
def test_datadog_sampler_init():
    ...
    assert isinstance(sampler._legacy_sampler, RateByServiceSampler)
    assert sampler._default_rule is None
    ...
    assert sampler.limiter.rate_limit == DatadogSampler.DEFAULT_RATE_LIMIT
    assert isinstance(sampler._default_rule, SamplingRule)
    assert sampler._legacy_sampler is None
    assert sampler._default_rule.sample_rate == 0.5
    ...
    with override_env(dict(DD_TRACE_SAMPLE_RATE="0")):
        ...
        assert isinstance(sampler._default_rule, SamplingRule)
        assert sampler._legacy_sampler is None
        assert sampler._default_rule.sample_rate == 0
...
def test_datadog_sampler_update_rate_by_service_sample_rates(dummy_tracer):
    ...
    for case in cases:
        ...
        rates = {}
        for k, v in iteritems(sampler._legacy_sampler._by_service_samplers):
            rates[k] = v.sample_rate
        assert case == rates, "%s != %s" % (case, rates)
