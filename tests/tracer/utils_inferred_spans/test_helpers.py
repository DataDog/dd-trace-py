...
def assert_web_and_inferred_aws_api_gateway_span_data(
    ...
    assert web_span._get_str_attribute("span.kind") == "server"
    assert web_span._get_str_attribute("component") == web_span_component
    assert web_span._get_numeric_attribute("_dd.inferred_span") is None
    ...
    assert aws_gateway_span._get_str_attribute("component") == "aws-apigateway"
    assert aws_gateway_span._get_numeric_attribute("_dd.inferred_span") == 1

    # assert api gateway http meta
    assert aws_gateway_span._get_str_attribute("http.method") == method
    assert int(aws_gateway_span._get_str_attribute("http.status_code")) == int(status_code)
    assert aws_gateway_span._get_str_attribute("http.url") == url

    # Assert that the http method and status code match
    assert aws_gateway_span._get_str_attribute("http.method") == web_span._get_str_attribute("http.method")
    assert aws_gateway_span._get_str_attribute("http.status_code") == web_span._get_str_attribute("http.status_code")

    # Assert errors (or lack of them) are correctly passed on
    assert aws_gateway_span.error == web_span.error
    assert web_span._get_str_attribute(ERROR_MSG) == aws_gateway_span._get_str_attribute(ERROR_MSG)
    assert web_span._get_str_attribute(ERROR_TYPE) == aws_gateway_span._get_str_attribute(ERROR_TYPE)
    assert web_span._get_str_attribute(ERROR_STACK) == aws_gateway_span._get_str_attribute(ERROR_STACK)

    if is_distributed:
        assert aws_gateway_span.trace_id == distributed_trace_id
        assert aws_gateway_span.parent_id == distributed_parent_id
        assert aws_gateway_span._get_numeric_attribute(_SAMPLING_PRIORITY_KEY) == distributed_sampling_priority
