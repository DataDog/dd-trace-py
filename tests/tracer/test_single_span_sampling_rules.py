...
def test_sampling_rule_init_via_env():
    ...
    with override_env(
        ...
        sampling_rules = SpanSamplingRules().get_single_span_sampling_rules()
    ...
    with override_env(dict(DD_SPAN_SAMPLING_RULES='[{"service":"xyz"}]')):
        ...
        assert sampling_rules[0]._sample_rate == 1.0
        assert sampling_rules[0]._service_matcher.pattern == "xyz"
        # The default pattern for name is * so it matches anything if not specified
        assert sampling_rules[0]._name_matcher.pattern == "*"
        assert sampling_rules[0]._max_per_second is None
        assert len(sampling_rules) == 1
    ...
    with override_env(dict(DD_SPAN_SAMPLING_RULES='[{"name":"xyz"}]')):
        ...
        assert sampling_rules[0]._sample_rate == 1.0
        # The default pattern for name is * so it matches anything if not specified
        assert sampling_rules[0]._service_matcher.pattern == "*"
