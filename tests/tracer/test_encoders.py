...
@pytest.mark.parametrize(
    ...
    [
        {"trace_id": "trace_id"},
        # {"span_id": "span_id"},  # Now handled gracefully by Rust (silently ignored if invalid type)
        {"parent_id": "parent_id"},
...
def test_encoding_invalid_data_raises(data):
    ...
    Note: name, service, resource, and span_id are now validated at the Rust layer and
    handle invalid types gracefully, so they no longer raise during encoding.
    """
