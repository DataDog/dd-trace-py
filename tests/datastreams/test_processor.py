...
def test_data_streams_processor():
    ...
    assert abs(processor._buckets[bucket_time_ns][aggr_key_1].full_pathway_latency.get_quantile_value(1) - 4) <= 4*0.008 # relative accuracy of 0.00775
    assert abs(processor._buckets[bucket_time_ns][aggr_key_2].full_pathway_latency.get_quantile_value(1) - 2) <= 2*0.008 # relative accuracy of 0.00775
