...
def _force_standalone(processor):
    """Force the standalone condition: APM tracing off plus a product's apm_opt_out.

    The processor reads DD_APM_TRACING_ENABLED once, at construction, so the flag is set on the
    instance rather than patched on the config object.
    """
    processor._apm_tracing_enabled = False
    with mock.patch("ddtrace.llmobs._processor.asm_config") as config:
        config._apm_opt_out = True
        yield
...
class TestStandaloneKeepsAPMTrace:
    ...
    def test_drop_decision_respects_apm_opt_out(self, apm_tracing_enabled, tracer_enabled, apm_opt_out, expect_dropped):
        ...
        processor._apm_tracing_enabled = apm_tracing_enabled
        ...
        with mock.patch("ddtrace.llmobs._processor.asm_config") as config:
            config._apm_opt_out = apm_opt_out
            result = processor.process_trace(trace)
...
class TestStandaloneKeepsLLMObsRoutingIntact:
    ...
    def test_trace_kept_and_llmobs_event_enqueued_exactly_once(self, llmobs_standalone, tracer):
        ...
        with _force_standalone(tracer._span_aggregator.llmobs_processor):
            with tracer.trace("llm-span", span_type=SpanTypes.LLM) as span:
                _annotate_llm_span(span)
