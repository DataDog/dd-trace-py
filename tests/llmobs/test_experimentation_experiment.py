...
# --- VCR setup reused from dataset tests ---
...
def init_llmobs(experiments_vcr):
    ...
    try:
        # Use a fixed project name for consistency in VCR cassettes
...
def synced_dataset(experiments_vcr, simple_dataset_data):
    ...
    """
    # Use a consistent name for VCR matching
...
class TestExperimentInitialization:
    ...
    def test_init_success_with_config(self, simple_local_dataset, valid_config):
        ...
        exp = dne.Experiment(
            ...
            task=task_with_config, # Task accepts config
            dataset=simple_local_dataset,
            evaluators=[],
            config=valid_config
        )
        assert exp.name == "test-init-config"
        assert exp.config == valid_config # Check config is stored
    ...
    def test_init_invalid_config_model_structure(self, simple_local_dataset):
        ...
        invalid_model_config = {"models": [{"name": 123}]} # Name must be str
...
class TestExperimentRunTask:
    ...
    # Helper to check output structure
    ...
    def test_run_task_success_datadog(self, synced_dataset, experiments_vcr):
        ...
        exp = dne.Experiment(
            ...
            task=simple_identity_task,
            dataset=synced_dataset,
            evaluators=[simple_match_evaluator] # Note: evaluators not used by run_task itself
        )
    ...
    def test_run_task_sample_size(self, synced_dataset, experiments_vcr):
        ...
        assert exp.has_run
        assert len(exp.outputs) == sample_size # Check only sample_size outputs generated
        self._check_output_structure(exp.outputs[0])
