...
def test_truncating_oversized_events(mock_writer_logs, mock_http_writer_send_payload_response):
    with override_global_config(dict(_dd_api_key="foobar.baz", _dd_site=DATADOG_SITE)):
        ...
        mock_writer_logs.warning.assert_has_calls(
            [
                mock.call("truncating event because its size (%d) exceeds the event size limit", 1400708),
                mock.call("truncating event because its size (%d) exceeds the event size limit", 1400448),
                mock.call("truncating event because its size (%d) exceeds the event size limit", 1400429),
            ]
        )
