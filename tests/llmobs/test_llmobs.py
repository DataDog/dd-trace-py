...
class TestLLMIOProcessing:
    ...
    def test_input_output_processor_remove(self, llmobs, llmobs_enable_opts, llmobs_events):
        ...
        assert llmobs_events[0]["meta"]["input"] == {"messages": [{"content": ""}]}
        assert llmobs_events[0]["meta"]["output"] == {"messages": [{"content": ""}]}
    ...
    def test_input_output_processor_mutate(self, llmobs, llmobs_enable_opts, llmobs_events):
        ...
        assert llmobs_events[0]["meta"]["input"] == {"messages": [{"content": "value processed"}]}
        assert llmobs_events[0]["meta"]["output"] == {
            "messages": [{"content": "value processed"}, {"content": "value2 processed"}]
        }
    ...
    def test_annotated_span(self, llmobs, llmobs_enable_opts, llmobs_events):
        with llmobs.annotation_context(tags={"scrub_values": "1"}):
            with llmobs.llm() as llm_span:
                llmobs.annotate(llm_span, input_data="value", output_data="value")
        assert llmobs_events[0]["meta"]["input"] == {"messages": [{"content": "redacted"}]}
        assert llmobs_events[0]["meta"]["output"] == {"messages": [{"content": "redacted"}]}
    ...
    def test_processor_error_is_logged(self, ddtrace_run_python_code_in_subprocess):
        ...
        out, err, status, _ = ddtrace_run_python_code_in_subprocess(
            ...
            LLMObs.enable(span_processor=span_processor)
