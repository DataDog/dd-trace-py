...
@pytest.mark.parametrize("ddtrace_global_config", [dict(_llmobs_sample_rate=5.0)])
@pytest.mark.parametrize("llmobs_enable_opts", [dict(sample_rate=-0.4)])
def test_enable_invalid_arg_and_invalid_env_falls_back(
    ...
    mock_llmobs_logs.warning.assert_any_call(
        "Invalid LLMObs sample rate argument (%r outside valid range [0.0, 1.0]); ignoring it.", -0.4
    )
    mock_llmobs_logs.warning.assert_any_call(
...
@pytest.mark.parametrize("ddtrace_global_config", [dict(_llmobs_sample_rate=0.8)])
@pytest.mark.parametrize("llmobs_enable_opts", [dict(sample_rate=0.0)])
def test_enable_sample_rate_zero_is_respected(llmobs, ddtrace_global_config, llmobs_enable_opts):
...
@pytest.mark.parametrize("ddtrace_global_config", [dict(_llmobs_sample_rate=0.8)])
@pytest.mark.parametrize("llmobs_enable_opts", [dict(sample_rate=0.3)])
def test_enable_sample_rate_arg_takes_precedence_over_env(llmobs, ddtrace_global_config, llmobs_enable_opts):
    """LLMObs.enable(sample_rate=...) takes precedence over DD_LLMOBS_SAMPLE_RATE."""
    assert llmobs._instance._sampler.sample_rate == 0.3


@pytest.mark.parametrize("ddtrace_global_config", [dict(_llmobs_sample_rate=0.4)])
def test_enable_keeps_valid_env_sample_rate(llmobs, ddtrace_global_config):
    """A valid DD_LLMOBS_SAMPLE_RATE (config value) is kept when no sample_rate is passed to enable()."""
