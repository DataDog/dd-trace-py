...
@pytest.mark.skipif(not RAGAS_AVAILABLE, reason="Tests require ragas to be available on user env")
class TestRagasFaithfulnessEvaluator:
    def test_ragas_evaluator_init(self, ragas, LLMObs):
    ...
    def test_ragas_faithfulness_returns_none_if_inputs_extraction_fails(
        self, ragas, mock_llmobs_submit_evaluation, LLMObs
    ):
        rf_evaluator = RagasFaithfulnessEvaluator(LLMObs)
    ...
    def test_ragas_faithfulness_has_modified_faithfulness_instance(
        self, ragas, mock_llmobs_submit_evaluation, reset_ragas_faithfulness_llm, LLMObs
    ):
    ...
    def test_ragas_faithfulness_submits_evaluation(self, ragas, LLMObs, mock_llmobs_submit_evaluation):
    ...
    def test_ragas_faithfulness_submits_evaluation_on_span_with_question_in_messages(
        self, ragas, LLMObs, mock_llmobs_submit_evaluation
    ):
    ...
    @pytest.mark.vcr_logs
    def test_ragas_faithfulness_submits_evaluation_on_span_with_custom_keys(
        self, ragas, LLMObs, mock_llmobs_submit_evaluation
    ):
        """Test that evaluation is submitted for a valid llm span where the last message content is the question"""
        rf_evaluator = RagasFaithfulnessEvaluator(LLMObs)
    ...
    @pytest.mark.vcr_logs
    def test_ragas_faithfulness_emits_traces(self, ragas, llmobs, llmobs_events):
    ...
    def test_llmobs_with_faithfulness_emits_traces_and_evals_on_exit(
        self, mock_writer_logs, run_python_code_in_subprocess
    ):
        env = os.environ.copy()
...
ctx = logs_vcr.use_cassette(
    "tests.llmobs.test_llmobs_ragas_faithfulness_evaluator.TestRagasFaithfulnessEvaluator.emits_traces_and_evaluations_on_exit.yaml"
)
ctx.__enter__()
