...
class TestAlignmentConfig:
    ...
    def test_custom_labels(self):
        ac = AlignmentConfig(output_type="score", alignment_thresholds=[5], class_labels=["excellent", "poor"])
...
class TestJudgeConfig:
    def _make_config(self, prompt="Rate this: {{output_data}}", training_examples=None):
        return JudgeConfig(
            ...
            structured_output=ScoreStructuredOutput(description="quality", min_score=1, max_score=10),
    ...
    def test_categorical_output_serialization(self):
        config = JudgeConfig(
            ...
            structured_output=CategoricalStructuredOutput(categories=["positive", "negative", "neutral"]),
        )
        d = config.to_dict()
