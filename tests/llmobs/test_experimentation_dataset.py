...
def sample_data_diverse():
    ...
    return [
        ...
        },
        {
            ...
            "expected_output": {
                "entities": [{"name": "Alice", "type": "PERSON"}, {"name": "Paris", "type": "LOCATION"}],
                "score": None,
            },
...
def meals_workouts_data():
    ...
    return [
        # --- Note: Formatting input/output/metadata structure ---
        {
            ...
            "expected_output": {
                "response": [
                    {
                        "type": "meal",
                        "description": "1 medium size pizza",
                        "calories": 2300,
                        "protein": 20,
                        "carbs": 20,
                        "fat": 20,
        ...
        },
        {
            ...
            "expected_output": {
                "response": [
                    {
                        "type": "workout",
                        "description": "3x10 bench press 225lbs",
        ...
        },
        {
            ...
            "expected_output": {
                "response": [
                    {
                        "type": "workout",
                        "description": "I ran 5km",
                        "exercise": "running",
                        "distance_km": 5,
                    },  # Represent distance explicitly if possible
                    {
                        "type": "meal",
                        "description": "3 hard boiled eggs",
                        "calories": 200,
                        "protein": 20,
                        "carbs": 2,
                        "fat": 14,
        ...
        {
            "input": {
                "prompt": "During the morning I had a bacon-egg-and-cheese, in the afternoon I don't remember, and now I'm having a small dish of lasagna."
            },
            "expected_output": {
                "response": [
                    {
                    ...
                    {
                        "type": "meal",
                        "description": "a small dish of lasagna",
                        "calories": 1000,
                        "protein": 100,
                        "carbs": 100,
                        "fat": 100,
        ...
        {
            "input": {
                "prompt": "Started my day with overnight oats with berries, had a protein shake after my workout, and finished with grilled salmon and quinoa for dinner"
            },
            "expected_output": {
                "response": [
                    {
                    ...
                    {
                        "type": "meal",
                        "description": "grilled salmon and quinoa",
                        "calories": 650,
                        "protein": 45,
                        "carbs": 45,
                        "fat": 28,
        ...
        },
        {
            ...
            "expected_output": {
                "response": [
                    ...
                    {
                        "type": "meal",
                        "description": "2 mango sticky rice",
                        "calories": 700,
                        "protein": 8,
                        "carbs": 140,
                        "fat": 16,
        ...
        },
        {
            ...
            "expected_output": {
                "response": [
                    {
                        "type": "workout",
                        "description": "45min HIIT session",
                        "exercise": "HIIT",
                        "duration_seconds": 2700,
                    },
                    {
                        "type": "meal",
                        "description": "2 protein bars",
                        "calories": 440,
                        "protein": 40,
                        "carbs": 44,
                        "fat": 16,
        ...
        {
            "input": {
                "prompt": "Morning routine: 5k run, then 4x12 squats at 185lbs, finished with 10 minutes of stretching"
            },
            "expected_output": {
                "response": [
                    ...
                    {
                        "type": "workout",
                        "description": "4x12 squats at 185lbs",
                        "exercise": "squats",
                        "series": 4,
                        "reps": 12,
                        "weight_kg": 84,
    ...
    with experiments_vcr.use_cassette("test_dataset_meals_workouts_setup.yaml"):
        try:
            ...
            if "not found" in str(e):
                ...
                print(
                    f"Successfully created and pushed dataset '{dataset_name}' version {dataset._datadog_dataset_version}"
                )
            else:
...
class TestDatasetInitialization:
    ...
    def test_init_invalid_data_inconsistent_keys(self):
        ...
        expected_message_regex = (
            rf"Inconsistent keys in data. Expected {expected_keys_str_escaped}, " rf"got {got_keys_str_escaped}"
        )
    ...
    def test_init_implicit_pull_existing(self, experiments_vcr, meals_workouts_dataset):
        ...
        assert_dataset_synced(
            dataset, expected_len=len(meals_workouts_dataset), expected_version=dataset._datadog_dataset_version
        )
...
class TestDatasetModification:
    ...
    def test_update_method(self, local_dataset_dict):
        ...
        new_record_data = {
            "input": {"prompt": "Updated prompt 2"},
            "expected_output": {"response": "Updated response 2"},
        }
...
class TestDatasetPull:
    ...
    def test_pull_specific_version(self, experiments_vcr, meals_workouts_dataset):
        ...
        with experiments_vcr.use_cassette(
            "test_dataset_pull_specific_version_meals_workouts.yaml"
        ):  # Updated cassette name
    ...
    def test_pull_nonexistent_version(self, experiments_vcr, meals_workouts_dataset):
        ...
        with experiments_vcr.use_cassette(
            "test_dataset_pull_nonexistent_version_meals_workouts.yaml"
        ):  # Updated cassette name
            # Match needs to check for the specific version not found message
            with pytest.raises(
                ValueError, match=f"Version {non_existent_version} not found for dataset '{dataset_name}'"
            ):
...
class TestDatasetPush:
    ...
    def test_push_new_local_dataset(self, experiments_vcr, local_dataset_simple):
        ...
        assert_dataset_synced(
            local_dataset_simple, expected_len=2, expected_version=local_dataset_simple._datadog_dataset_version
        )
    ...
    def synced_dataset(self, experiments_vcr, meals_workouts_dataset):
        ...
        dataset_name = meals_workouts_dataset.name
        with experiments_vcr.use_cassette(
            "test_dataset_pull_for_sync_tests.yaml"
        ):  # Cassette for pulling within sync tests
            dataset = dne.Dataset.pull(dataset_name)
        assert dataset._synced is True
        assert dataset._datadog_dataset_id is not None
        return dataset
    ...
    def test_push_synced_no_changes(self, experiments_vcr, synced_dataset, capsys):
        ...
        assert (
            f"Dataset '{synced_dataset.name}' (v{initial_version}) is already synced and has no pending changes"
            in captured.out
        )
    ...
    def test_push_synced_with_deletes(self, experiments_vcr, synced_dataset):
        ...
        assert_dataset_unsynced(
            synced_dataset, expected_len=initial_len - 1, expected_version=initial_version, deleted=1
        )
    ...
    def test_push_synced_with_updates(self, experiments_vcr, synced_dataset):
        ...
        updated_record_data = {
            ...
            "expected_output": {"response": "updated response"},
        }
    ...
    def test_push_synced_mixed_changes(self, experiments_vcr, synced_dataset):
        ...
        updated_record_data = {
            "input": {"prompt": "mixed update"},
            "expected_output": {"response": "mixed update resp"},
        }
        synced_dataset[0] = updated_record_data
        ...
        assert_dataset_unsynced(
            synced_dataset, expected_len=initial_len, expected_version=initial_version, added=1, deleted=1, updated=1
        )
    ...
    def test_push_local_collision_overwrite(self, experiments_vcr, local_colliding_dataset):
        ...
        assert_dataset_synced(
            local_colliding_dataset, expected_len=original_local_data_count, expected_version=final_version
        )
...
class TestDatasetFromCSV:
    ...
    def test_from_csv_header_only_file(self, csv_file_header_only):
        ...
        with pytest.raises(ValueError, match="CSV file is empty"):
            dne.Dataset.from_csv(
                csv_file_header_only, name="bad", input_columns=["col1"], expected_output_columns=["col2"]
            )
    ...
    def test_from_csv_missing_input_column(self, csv_file_simple):
        ...
        with pytest.raises(ValueError, match="Input columns not found in CSV header: \\['missing_input'\\]"):
            dne.Dataset.from_csv(
                csv_file_simple, name="bad", input_columns=["missing_input"], expected_output_columns=["answer"]
            )
    ...
    def test_from_csv_missing_output_column(self, csv_file_simple):
        ...
        with pytest.raises(ValueError, match="Expected output columns not found in CSV header: \\['missing_output'\\]"):
            dne.Dataset.from_csv(
                csv_file_simple, name="bad", input_columns=["question"], expected_output_columns=["missing_output"]
            )
    ...
    def test_from_csv_malformed_file(self, csv_file_malformed):
        ...
        with pytest.raises(DatasetFileError, match="Error parsing CSV file"):
            dne.Dataset.from_csv(
                csv_file_malformed, name="bad", input_columns=["head1"], expected_output_columns=["head2"]
            )
...
class TestDatasetRepr:
    ...
    def test_repr_synced_no_changes(self, experiments_vcr, meals_workouts_dataset):
        ...
        assert (
            dataset._datadog_dataset_id in rep_clean
        )  # Check if the raw ID exists within the cleaned repr (likely in the encoded URL)
    ...
    def test_repr_unsynced_changes(self, experiments_vcr, meals_workouts_dataset):
        ...
        expected_changes = []
        if dataset._changes["added"]:
            expected_changes.append(f"+{len(dataset._changes['added'])} added")
        if dataset._changes["deleted"]:
            expected_changes.append(f"-{len(dataset._changes['deleted'])} deleted")
        if dataset._changes["updated"]:
            expected_changes.append(f"~{len(dataset._changes['updated'])} updated")
