...
@pytest.mark.skipif(sys.version_info < (3, 3), reason="skip for older versions of Python")
...
def test_dir_filtering():
    ...
    # DEV: We removed the functionality of `hide_from_dir`, it should still be here
    assert "my_stuff" in dir(str)
...
def test_dir_without_args_returns_names_in_local_scope():
    ...
    # DEV: We don't use __filtered__dir__ anymore, so sorting isn't needed
