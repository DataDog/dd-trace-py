...
**not** a pytest run. The cases live here; the runner lives in
...
| [`suites/python-tracer-agent-v0.1.yaml`](./suites/python-tracer-agent-v0.1.yaml) | Cases (one file only — the CLI errors if `suites/` has more than one YAML) |

## Run locally (Docker)

From the **`dd-trace-py` repo root**:
...
# Cheap real smoke — still the minimum preset
...
# CI default (`full` = every case in suites/)
docker run --rm -e LLMVAL_AUTH_HEADER -v "$PWD:/repo" "$LLMVAL_IMAGE" \
  --repo /repo --base-sha main --level full --runs 1
...
`full` = every case including the skill starter). `--runs` only repeats those cases.
Needs `ddtool` on the host for a real (non-`--fake`) run.
