...
def _ensure_meson_ninja_patch():
    """Create a .pth file that patches MesonpyMetaFinder._rebuild before sitecustomize runs.
    ...
    plus _ddtrace_meson_ninja_fix.py in the same site-packages directory.  Both
    files are safe to delete; this function recreates them on next pytest run.

    This is a no-op when ninja is available or when the files already exist.
    """
    # Find the site-packages directory that hosts _ddtrace_editable_loader.py.
    site_packages_dir = None
    for p in sys.path:
        p = p or os.getcwd()
        if "site-packages" not in p:
            continue
        if os.path.isfile(os.path.join(p, "_ddtrace_editable_loader.py")):
            site_packages_dir = p
            break
    ...
    # patches _rebuild() to skip ninja when the binary is absent.
    patch_code = '''\
# AIDEV-NOTE: Patches MesonpyMetaFinder._rebuild to skip ninja if binary is
# missing (e.g., deleted by pip build isolation cleanup).  Generated by
# conftest.py _ensure_meson_ninja_patch().  Safe to delete; conftest recreates
# it on the next pytest run.
try:
...
'''
    ...
    try:
        ...
        with open(pth_path, "w", encoding="utf-8") as f:
            f.write("import _ddtrace_meson_ninja_fix\n")
    except OSError:
