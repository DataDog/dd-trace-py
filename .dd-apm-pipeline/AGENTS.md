...
Execute the `execute` workflow for `pyramid` in `dd_trace_py`. This directory is a
portable control bundle; do not launch `dd-apm` as a nested agent runner.

1. Read `TASK.md`, `specs/version-update.md`, `PIPELINE.md`, then only the current numbered stage and its references.
2. Act as orchestrator. The main agent coordinates and integrates; focused subagents perform the
   substantial analysis, implementation, testing, and review work. Inspect their work before advancing.
