...
# 6. feature branch with GitLab MR target var -> fast path, no curl.
...
# 8. feature branch, no CI target vars -> GitHub API lookup.
if command -v jq > /dev/null 2>&1; then
  body='[{"base":{"ref":"4.10"}}]'
  check "$(FAKE_CURL_BODY="${body}" bash "${TARGET}" backport-x-to-4.10 2>/dev/null)" \
