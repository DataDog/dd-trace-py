...
then
  ...
  if python3.12 -m pip download --no-index --no-deps --find-links "${S3_INDEX_URL}" --pre ddtrace 2>/dev/null; then
    ...
    mkdir ./tmp
    PYO3_PYTHON=python3.12 CIBW_BUILD=1 python3.12 -m pip wheel --no-deps -w ./tmp/ ./
