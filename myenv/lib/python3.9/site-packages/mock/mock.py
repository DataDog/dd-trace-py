...
class NonCallableMock(Base):
    ...
    def __dir__(self):
        ...
        from_child_mocks = [m_name for m_name, m_value in self._mock_children.items() if m_value is not _deleted]
    ...
    def assert_has_calls(self, calls, any_order=False):
        ...
        if not any_order:
            if expected not in all_calls:
                ...
                raise AssertionError(
                    f"{problem}\n" f"Expected: {_CallList(calls)}" f'{self._calls_repr(prefix="Actual").rstrip(".")}'
                ) from cause
            return
        ...
        if not_found:
            raise AssertionError(
                ...
                "found %r instead" % (self._mock_name or "mock", tuple(not_found), all_calls)
            ) from cause
    ...
    def _get_child_mock(self, **kw):
        ...
        if issubclass(_type, MagicMock) and _new_name in _async_method_magics:
            ...
            if _new_name in _all_sync_magics or self._mock_methods and _new_name in self._mock_methods:
...
_ATTRIB_DENY_LIST = frozenset(
    {removeprefix(name, "assert_") for name in dir(NonCallableMock) if name.startswith("assert_")}
)
...
class _patch(object):
    ...
    def __init__(self, getter, attribute, new, spec, create, spec_set, autospec, new_callable, kwargs, *, unsafe=False):
        ...
        if _is_instance_mock(spec):
            raise InvalidSpecError(
                f"Cannot spec attr {attribute!r} as the spec " f"has already been mocked out. [spec={spec!r}]"
            )
    ...
    def __exit__(self, *exc_info):
        ...
        if self.is_local and self.temp_original is not DEFAULT:
            ...
            if not self.create and (
                ...
                or self.attribute in ("__doc__", "__module__", "__defaults__", "__annotations__", "__kwdefaults__")
            ):
...
_side_effect_methods = {"__eq__": _get_eq, "__ne__": _get_ne, "__iter__": _get_iter, "__aiter__": _get_async_iter}
...
class AsyncMockMixin(Base):
    ...
    def assert_awaited_once(_mock_self):
        ...
        if not self.await_count == 1:
            msg = (
                f"Expected {self._mock_name or 'mock'} to have been awaited once." f" Awaited {self.await_count} times."
            )
            raise AssertionError(msg)
    ...
    def assert_not_awaited(_mock_self):
        ...
        if self.await_count != 0:
            msg = (
                f"Expected {self._mock_name or 'mock'} to not have been awaited." f" Awaited {self.await_count} times."
            )
            raise AssertionError(msg)
...
def create_autospec(spec, spec_set=False, instance=False, _parent=None, _name=None, *, unsafe=False, **kwargs):
    ...
    if inspect.isdatadescriptor(spec):
        ...
        if instance:
            raise RuntimeError("Instance can not be True when create_autospec " "is mocking an async function")
        Klass = AsyncMock
    ...
    for entry in dir(spec):
        ...
        if not isinstance(original, FunctionTypes):
            ...
            new = child_klass(parent=parent, name=entry, _new_name=entry, _new_parent=parent, **kwargs)
            mock._mock_children[entry] = new
