...
from ._compat import MutableSequence
from ._compat import OriginAbstractSet
from ._compat import OriginMutableSet
from ._compat import Sequence
from ._compat import Set
from ._compat import fields
from ._compat import get_final_base
from ._compat import get_newtype_base
...
from ._compat import is_counter
from ._compat import is_deque
from ._compat import is_frozenset
from ._compat import is_generic
from ._compat import is_generic_attrs
from ._compat import is_hetero_tuple
from ._compat import is_literal
from ._compat import is_mapping
from ._compat import is_mutable_set
from ._compat import is_protocol
from ._compat import is_sequence
from ._compat import is_tuple
from ._compat import is_typeddict
from ._compat import is_union_type
...
from .gen import DictStructureFn
from .gen import HeteroTupleUnstructureFn
from .gen import IterableUnstructureFn
from .gen import MappingStructureFn
from .gen import MappingUnstructureFn
from .gen import make_dict_structure_fn
from .gen import make_dict_unstructure_fn
from .gen import make_hetero_tuple_unstructure_fn
from .gen import make_iterable_unstructure_fn
from .gen import make_mapping_structure_fn
from .gen import make_mapping_unstructure_fn
...
def is_attrs_union_or_none(typ: Type) -> bool:
    return is_union_type(typ) and all(e is NoneType or has(get_origin(e) or e) for e in typ.__args__)
...
class BaseConverter:
    ...
    def unstructure(self, obj: Any, unstructure_as: Any = None) -> Any:
        return self._unstructure_func.dispatch(obj.__class__ if unstructure_as is None else unstructure_as)(obj)
    ...
    def register_unstructure_hook_func(self, check_func: Callable[[Any], bool], func: Callable[[Any], Any]) -> None:
    ...
    def register_structure_hook(self, cl: Any, func: Callable[[Any, Type[T]], T]) -> None:
    ...
    def _gen_attrs_union_structure(self, cl: Any) -> Callable[[Any, Type[T]], Optional[Type[T]]]:
            ...
            if self.detailed_validation:
                ...
                for e in obj:
                    try:
                        ...
                        msg = IterableValidationNote(f"Structuring {cl} @ index {ix}", ix, elem_type)
    ...
    def _structure_set(self, obj: Iterable[T], cl: Any, structure_to: type = set) -> Set[T]:
    ...
    def _structure_frozenset(self, obj: Iterable[T], cl: Any) -> FrozenSetSubscriptable[T]:
            ...
            if self.detailed_validation:
                ...
                for e in obj:
                    try:
                        ...
                        msg = IterableValidationNote(f"Structuring {tup} @ index {ix}", ix, tup_type)
                        exc.__notes__ = getattr(e, "__notes__", []) + [msg]
                        errors.append(exc)
                    finally:
                        ix += 1
            ...
            if self.detailed_validation:
                ...
                for ix, (t, e) in enumerate(zip(tup_params, obj)):
                    try:
                        ...
                        msg = IterableValidationNote(f"Structuring {tup} @ index {ix}", ix, t)
                        exc.__notes__ = getattr(e, "__notes__", []) + [msg]
                        errors.append(exc)
                if len(res) < exp_len:
                ...
                res = tuple([self._structure_func.dispatch(t)(e, t) for t, e in zip(tup_params, obj)])
                if len(res) < exp_len:
                    problem = "Not enough" if len(res) < len(tup_params) else "Too many"
    ...
    def _get_dis_func(union: Any) -> Callable[[Any], Type]:
        ...
        if not all(has(get_origin(e) or e) for e in union_types):
            raise StructureHandlerNotFoundError(
                "Only unions of attrs classes supported " "currently. Register a loads hook manually.",
                type_=union,
            )
    ...
    def copy(
        ...
        res = self.__class__(
            ...
            prefer_attrib_converters if prefer_attrib_converters is not None else self._prefer_attrib_converters,
            detailed_validation if detailed_validation is not None else self.detailed_validation,
        )
...
class Converter(BaseConverter):
    ...
    def __init__(
        ...
        unstruct_collection_overrides = {get_origin(k) or k: v for k, v in unstruct_collection_overrides.items()}
        ...
        if unstruct_strat is UnstructureStrategy.AS_DICT:
            ...
            self.register_unstructure_hook_factory(has_with_generic, self.gen_unstructure_attrs_fromdict)
            self.register_structure_hook_factory(has_with_generic, self.gen_structure_attrs_fromdict)
        self.register_unstructure_hook_factory(is_annotated, self.gen_unstructure_annotated)
        self.register_unstructure_hook_factory(is_hetero_tuple, self.gen_unstructure_hetero_tuple)
        self.register_unstructure_hook_factory(is_sequence, self.gen_unstructure_iterable)
        ...
        self.register_unstructure_hook_factory(is_typeddict, self.gen_unstructure_typeddict)
        self.register_unstructure_hook_factory(
        ...
        self.register_structure_hook_factory(lambda t: get_newtype_base(t) is not None, self.get_structure_newtype)
    ...
    def gen_unstructure_attrs_fromdict(self, cl: Type[T]) -> Callable[[T], Dict[str, Any]]:
        origin = get_origin(cl)
        attribs = fields(origin or cl)
        if attrs_has(cl) and any(isinstance(a.type, str) for a in attribs):
            # PEP 563 annotations - need to be resolved.
            resolve_types(cl)
        attrib_overrides = {a.name: self.type_overrides[a.type] for a in attribs if a.type in self.type_overrides}

        h = make_dict_unstructure_fn(cl, self, _cattrs_omit_if_default=self.omit_if_default, **attrib_overrides)
        return h
    ...
    def gen_structure_typeddict(self, cl: Any) -> Callable[[Dict], Dict]:
        ...
        Also apply converter-scored modifications.
        """
        return make_typeddict_dict_struct_fn(cl, self, _cattrs_detailed_validation=self.detailed_validation)

    def gen_structure_attrs_fromdict(self, cl: Type[T]) -> Callable[[Mapping[str, Any], Any], T]:
    ...
    def gen_unstructure_iterable(self, cl: Any, unstructure_to: Any = None) -> IterableUnstructureFn:
        unstructure_to = self._unstruct_collection_overrides.get(get_origin(cl) or cl, unstructure_to or list)
    ...
    def gen_unstructure_hetero_tuple(self, cl: Any, unstructure_to: Any = None) -> HeteroTupleUnstructureFn:
        unstructure_to = self._unstruct_collection_overrides.get(get_origin(cl) or cl, unstructure_to or list)
    ...
    def gen_unstructure_mapping(
        ...
        unstructure_to = self._unstruct_collection_overrides.get(get_origin(cl) or cl, unstructure_to or dict)
        h = make_mapping_unstructure_fn(cl, self, unstructure_to=unstructure_to, key_handler=key_handler)
        self._unstructure_func.register_cls_list([(cl, h)], direct=True)
        return h
    ...
    def gen_structure_mapping(self, cl: Any) -> MappingStructureFn[T]:
        h = make_mapping_structure_fn(cl, self, detailed_validation=self.detailed_validation)
        self._structure_func.register_cls_list([(cl, h)], direct=True)
        return h
    ...
    def copy(
        ...
        res = self.__class__(
            ...
            forbid_extra_keys if forbid_extra_keys is not None else self.forbid_extra_keys,
        ...
        self._unstructure_func.copy_to(res._unstructure_func, skip=self._unstruct_copy_skip)
