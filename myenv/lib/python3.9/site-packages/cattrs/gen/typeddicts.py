...
from .._compat import get_notrequired_base
...
def make_dict_unstructure_fn(
    ...
    **kwargs: AttributeOverride,
    ...
    try:
        ...
        total_lines = (
            [f"def {fn_name}(instance{internal_arg_line}):"] + ["  res = instance.copy()"] + lines + ["  return res"]
        )
        script = "\n".join(total_lines)
...
def make_dict_structure_fn(
    ...
    **kwargs: AttributeOverride,
        ...
        for a in attrs:
            ...
            if handler:
                if handler == converter._structure_call:
                    ...
                    lines.append(f"{i}res['{an}'] = {struct_handler_name}(o['{kn}'], {type_name})")
            else:
        ...
        for a in attrs:
            ...
            if handler:
                ...
                if handler == converter._structure_call:
                    ...
                    type_name = f"__c_type_{an}"
                    internal_arg_parts[type_name] = t
                    invocation_line = f"  res['{an}'] = {struct_handler_name}(o['{kn}'], {type_name})"
            else:
    ...
    total_lines = [f"def {fn_name}(o, _, *, {internal_arg_line}):"] + lines + post_lines + ["  return res"]
...
def _adapted_fields(cls: Any) -> List[Attribute]:
    ...
    return [Attribute(n, NOTHING, None, False, False, False, False, False, type=a) for n, a in annotations.items()]


def _is_extensions_typeddict(cls) -> bool:
    if _TypedDictMeta is None:
        return False
    return cls.__class__ is _TypedDictMeta or (is_generic(cls) and (cls.__origin__.__class__ is _TypedDictMeta))
