...
from .._compat import _adapted_fields
from .._compat import get_args
from .._compat import get_origin
from .._compat import is_annotated
from .._compat import is_bare
from .._compat import is_bare_final
from .._compat import is_generic
from .._generics import deep_copy_with
from ..errors import AttributeValidationNote
from ..errors import ClassValidationError
from ..errors import ForbiddenExtraKeysError
from ..errors import IterableValidationError
from ..errors import IterableValidationNote
from ..errors import StructureHandlerNotFoundError
from ._consts import AttributeOverride
from ._consts import already_generating
from ._consts import neutral
...
def make_dict_unstructure_fn(
    ...
    **kwargs: AttributeOverride,
    ...
    try:
        for a in attrs:
            ...
            if override.unstruct_hook is not None:
                ...
                if a.type is not None:
                    ...
                    if handler is None:
                        if is_bare_final(t) and a.default is not NOTHING and not isinstance(a.default, attr.Factory):
            ...
            if d is not attr.NOTHING and (
                (_cattrs_omit_if_default and override.omit_if_default is not False) or override.omit_if_default
            ):
        ...
        fname = generate_unique_filename(cl, "unstructure", reserve=_cattrs_use_linecache)
...
def make_dict_structure_fn(
    ...
    **kwargs: AttributeOverride,
    ...
    if _cattrs_detailed_validation:
        ...
        for a in attrs:
            ...
            if override.struct_hook is not None:
                ...
                handler = find_structure_handler(a, t, converter, _cattrs_prefer_attrib_converters)
            ...
            if handler:
                if handler == converter._structure_call:
                    ...
                    type_name = f"__c_type_{an}"
                    internal_arg_parts[type_name] = t
                    lines.append(f"{i}res['{ian}'] = {struct_handler_name}(o['{kn}'], {type_name})")
            else:
        ...
        post_lines.append(f"  if errors: raise __c_cve('While structuring ' + {cl.__name__!r}, errors, __cl)")
        instantiation_lines = (
            ["  try:"]
            + ["    return __cl("]
            + [f"      {line}" for line in invocation_lines]
        ...
        if non_required:
            ...
            for a in non_required:
                ...
                if handler:
                    if handler == converter._structure_call:
                        internal_arg_parts[struct_handler_name] = t
                        post_lines.append(f"    res['{ian}'] = {struct_handler_name}(o['{kn}'])")
                    else:
                        type_name = f"__c_type_{an}"
                        internal_arg_parts[type_name] = t
                        post_lines.append(f"    res['{ian}'] = {struct_handler_name}(o['{kn}'], {type_name})")
                else:
                    post_lines.append(f"    res['{ian}'] = o['{kn}']")
        instantiation_lines = ["  return __cl("] + [f"    {line}" for line in invocation_lines] + ["  )"]
    ...
    total_lines = [f"def {fn_name}(o, _, *, {internal_arg_line}):"] + lines + post_lines + instantiation_lines
...
def make_hetero_tuple_unstructure_fn(
    ...
    handlers = [converter._unstructure_func.dispatch(type_arg) for type_arg in type_args]
...
def make_mapping_unstructure_fn(
    ...
    lines.append(f"    res = __cattr_mapping_cl(({k_u}, {v_u}) for k, v in mapping.items())")
...
def make_mapping_structure_fn(
    ...
    if not is_bare(cl):
        ...
        if not is_bare_dict:
            ...
            k_s = "__cattr_k_s(k, __cattr_k_t)" if key_handler != key_type else "__cattr_k_s(k)"
            v_s = "__cattr_v_s(v, __cattr_v_t)" if val_handler != val_type else "__cattr_v_s(v)"
    else:
        is_bare_dict = True
    ...
    if is_bare_dict:
        ...
        if detailed_validation:
            ...
            internal_arg_parts["val_type"] = val_type if val_type is not NOTHING else Any
            internal_arg_parts["key_type"] = key_type if key_type is not NOTHING else Any
