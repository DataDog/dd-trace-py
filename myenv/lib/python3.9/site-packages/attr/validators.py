...
class _MatchesReValidator:
    ...
    def __call__(self, inst, attr, value):
        ...
        if not self.match_func(value):
            raise ValueError(
                ...
                " ({value!r} doesn't)".format(name=attr.name, pattern=self.pattern.pattern, value=value),
                attr,
                self.pattern,
                value,
            )
    ...
    def __repr__(self):
        return "<matches_re validator for pattern {pattern!r}>".format(pattern=self.pattern)
...
def matches_re(regex, flags=0, func=None):
    ...
    if func not in valid_funcs:
        raise ValueError(
            "'func' must be one of {}.".format(", ".join(sorted(e and e.__name__ or "None" for e in set(valid_funcs))))
        )

    if isinstance(regex, Pattern):
        if flags:
            raise TypeError("'flags' can only be used with a string pattern; " "pass flags to re.compile() instead")
        pattern = regex
    else:
...
class _NumberValidator:
    ...
    def __repr__(self):
        return "<Validator for x {op} {bound}>".format(op=self.compare_op, bound=self.bound)
...
class _MaxLengthValidator:
    ...
    def __call__(self, inst, attr, value):
        ...
        if len(value) > self.max_length:
            raise ValueError(
                "Length of '{name}' must be <= {max}: {len}".format(name=attr.name, max=self.max_length, len=len(value))
            )
...
class _MinLengthValidator:
    ...
    def __call__(self, inst, attr, value):
        ...
        if len(value) < self.min_length:
            raise ValueError(
                "Length of '{name}' must be => {min}: {len}".format(name=attr.name, min=self.min_length, len=len(value))
            )
