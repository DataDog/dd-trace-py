...
_envier_attr_makers = frozenset({"envier.env.Env.%s" % m for m in ("v", "d", "var", "der")})
    ...
    for stmt in ctx.cls.defs.body:
        if isinstance(stmt, AssignmentStmt):
            ...
            if (
                ...
                or t.cast(NameExpr, t.cast(MemberExpr, decl.callee).expr).fullname not in _envier_base_classes
            ):
            ...
            attr.node.type = ctx.api.anal_type(expr_to_unanalyzed_type(decl.args[0], ctx.api.options))
            ...
            # parent class.
            if {_.fullname for _ in stmt.base_type_exprs} & _envier_base_classes and "__item__" in stmt.info.names:
