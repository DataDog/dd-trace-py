...
class BaseExceptionGroup(BaseException, Generic[_BaseExceptionT_co]):
    ...
    def __new__(cls, __message: str, __exceptions: Sequence[_BaseExceptionT_co]) -> Self:
        ...
        for i, exc in enumerate(__exceptions):
            if not isinstance(exc, BaseException):
                raise ValueError(f"Item {i} of second argument (exceptions) is not an " f"exception")
    ...
    def split(
        self: Self,
        __condition: type[_BaseExceptionT] | tuple[type[_BaseExceptionT], ...] | Callable[[_BaseExceptionT_co], bool],
    ) -> (tuple[BaseExceptionGroup[_BaseExceptionT] | None, Self | None] | tuple[Self | None, Self | None]):
        condition = get_condition_filter(__condition)
        if condition(self):
            return self, None
...
class ExceptionGroup(BaseExceptionGroup[_ExceptionT_co], Exception):
    ...
    if TYPE_CHECKING:
        ...
        def split(
            self: Self,
            __condition: type[_ExceptionT] | tuple[type[_ExceptionT], ...] | Callable[[_ExceptionT_co], bool],
        ) -> (tuple[ExceptionGroup[_ExceptionT] | None, Self | None] | tuple[Self | None, Self | None]):
