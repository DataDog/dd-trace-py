...
class KeyMapping(six.with_metaclass(ABCMeta)):
    ...
    def __init__(self, relative_accuracy, offset=0.0):
        ...
        if relative_accuracy <= 0 or relative_accuracy >= 1:
            raise ValueError("Relative accuracy must be between 0 and 1, got %r" % relative_accuracy)
...
class CubicallyInterpolatedMapping(KeyMapping):
    ...
    def __init__(self, relative_accuracy, offset=0.0):
        # type: (float, float) -> None
        super(CubicallyInterpolatedMapping, self).__init__(relative_accuracy, offset=offset)
    ...
    def _cubic_log2_approx(self, value):
        ...
        mantissa, exponent = math.frexp(value)
        significand = 2 * mantissa - 1
        return ((self.A * significand + self.B) * significand + self.C) * significand + (exponent - 1)
    ...
    def _cubic_exp2_approx(self, value):
        ...
        cardano = _cbrt((delta_1 - ((delta_1 * delta_1 - 4 * delta_0 * delta_0 * delta_0) ** 0.5)) / 2.0)
        significand_plus_one = -(self.B + cardano + delta_0 / cardano) / (3.0 * self.A) + 1.0
