...
proc_info_map = dict(
    ppid=0, rss=1, vms=2, create_time=3, nice=4, num_threads=5, status=6, ttynr=7, uid=8, euid=9, gid=10, egid=11
)
...
def swap_memory():
    ...
    p = subprocess.Popen(
        ["/usr/bin/env", "PATH=/usr/sbin:/sbin:%s" % os.environ["PATH"], "swap", "-l"], stdout=subprocess.PIPE
    )
