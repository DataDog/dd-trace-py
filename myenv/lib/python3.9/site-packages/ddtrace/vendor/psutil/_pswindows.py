...
def net_connections(kind, _pid=-1):
    ...
    if kind not in conn_tmap:
        raise ValueError(
            "invalid %r kind argument; choose between %s" % (kind, ", ".join([repr(x) for x in conn_tmap]))
        )
    ...
    def _wrap_exceptions(self):
        ...
        try:
            ...
            if is_permission_err(err):
                raise AccessDenied(
                    pid=None, name=self._name, msg="service %r is not querable (not enough privileges)" % self._name
                )
...
def retry_error_partial_copy(fun):
    ...
    def wrapper(self, *args, **kwargs):
        ...
        for x in range(times):  # retries for roughly 1 second
            ...
            msg = "%s retried %s times, converted to AccessDenied as it's " "still returning %r" % (fun, times, err)
