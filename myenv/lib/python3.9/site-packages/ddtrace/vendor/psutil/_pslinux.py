...
if os.path.exists("/sys/devices/system/cpu/cpufreq/policy0") or os.path.exists("/sys/devices/system/cpu/cpu0/cpufreq"):
    ...
    def cpu_freq():
        ...
        def get_path(num):
            for p in ("/sys/devices/system/cpu/cpufreq/policy%s" % num, "/sys/devices/system/cpu/cpu%s/cpufreq" % num):
                if os.path.exists(p):
                    return p
...
def disk_io_counters(perdisk=False):
    ...
    def read_procfs():
        ...
        for line in lines:
            ...
            if flen == 15:
                ...
                name = fields[3]
                reads = int(fields[2])
                (reads_merged, rbytes, rtime, writes, writes_merged, wbytes, wtime, _, busy_time, _) = map(
                    int, fields[4:14]
                )
            elif flen == 14 or flen == 18:
                # Linux 2.6+, line referring to a disk
                name = fields[2]
                (reads, reads_merged, rbytes, rtime, writes, writes_merged, wbytes, wtime, _, busy_time, _) = map(
                    int, fields[3:14]
                )
            elif flen == 7:
    ...
    if os.path.exists("%s/diskstats" % get_procfs_path()):
        ...
        raise NotImplementedError(
            "%s/diskstats nor /sys/block filesystem are available on this " "system" % get_procfs_path()
        )
...
class Process(object):
                    ...
                    if not fields[0].endswith(b":"):
                        ...
                        try:
                            ...
                            if fields[0].startswith(b"VmFlags:"):
                                ...
                                raise ValueError("don't know how to inte" "rpret line %r" % line)
                yield (current_block.pop(), data)
    ...
    if HAS_CPU_AFFINITY:
        ...
        def _get_eligible_cpus(self, _re=re.compile(br"Cpus_allowed_list:\t(\d+)-(\d+)")):
                ...
                if isinstance(err, ValueError) or err.errno == errno.EINVAL:
                    ...
                    for cpu in cpus:
                        ...
                        if cpu not in eligible_cpus:
                            raise ValueError(
                                "CPU number %r is not eligible; choose " "between %s" % (cpu, eligible_cpus)
                            )
                raise
