...
def check_global_invariants(subject, invariants):
    error_codes = tuple(
        error_code for is_ok, error_code in (invariant(subject) for invariant in invariants) if not is_ok
    )
    if error_codes:
...
def _sequence_field(
    checked_class, item_type, optional, initial, invariant=PFIELD_NO_INVARIANT, item_invariant=PFIELD_NO_INVARIANT
):
    """
...
def pset_field(
    item_type, optional=False, initial=(), invariant=PFIELD_NO_INVARIANT, item_invariant=PFIELD_NO_INVARIANT
):
    """
    ...
    return _sequence_field(
        CheckedPSet, item_type, optional, initial, invariant=invariant, item_invariant=item_invariant
    )
...
def pvector_field(
    ...
    """
    return _sequence_field(
        CheckedPVector, item_type, optional, initial, invariant=invariant, item_invariant=item_invariant
    )
