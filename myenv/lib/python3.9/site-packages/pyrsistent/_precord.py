...
class PRecord(PMap, CheckedType, metaclass=_PRecordMeta):
    ...
    def __repr__(self):
        return "{0}({1})".format(
            self.__class__.__name__, ", ".join("{0}={1}".format(k, repr(v)) for k, v in self.items())
        )
    ...
    def persistent(self):
        ...
        if self._invariant_error_codes or self._missing_fields:
            raise InvariantException(
                tuple(self._invariant_error_codes), tuple(self._missing_fields), "Field invariant failed"
            )
