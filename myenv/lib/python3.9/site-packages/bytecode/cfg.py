...
class BasicBlock(_bytecode._InstrList[Union[Instr, SetLineno, TryBegin, TryEnd]]):
    def __init__(
        self,
        instructions: Optional[Iterable[Union[Instr, SetLineno, TryBegin, TryEnd]]] = None,
    ) -> None:
    ...
    def __iter__(self) -> Iterator[Union[Instr, SetLineno, TryBegin, TryEnd]]:
        ...
        while index < len(self):
            ...
            if isinstance(instr, Instr) and instr.has_jump():
                if index < len(self) and any(isinstance(self[i], Instr) for i in range(index, len(self))):
    ...
    @overload
    def __getitem__(self, index: SupportsIndex) -> Union[Instr, SetLineno, TryBegin, TryEnd]:
        ...
        ...
        for i, instr in enumerate(self.block):
            ...
            if instr.has_jump():
                ...
                maxsize = yield _StackSizeComputer(
                    ...
                    # by a TryEnd.
                    None if instr.is_final() and self.block.get_trailing_try_end(i) else self._current_try_begin,
                )
...
class ControlFlowGraph(_bytecode.BaseBytecode):
    ...
    def add_block(self, instructions: Optional[Iterable[Union[Instr, SetLineno]]] = None) -> BasicBlock:
    ...
    def compute_stacksize(
        ...
        *,
        ...
        coro = _StackSizeComputer(common, self[0], initial_stack_size, 0, 0, None, None).run()
            ...
            if old_block is not None and last_instr is not None:
                ...
                # If we have an active TryBegin and last_instr is:
                    ...
                    if (
                        ...
                        assert isinstance(last_instr.arg, Label)
                        add_try_end[last_instr.arg].append(TryEnd(try_begins[active_try_begin][-1]))
                    ...
                    if last_instr.is_final() and temp:
                        ...
                        new_tb = TryBegin(active_try_begin.target, active_try_begin.push_lasti)
                        block.append(new_tb)
                ...
                if isinstance(instr, (Instr, TryBegin, TryEnd)):
                    ...
                    if isinstance(instr, TryBegin):
                        ...
                        if last_try_begin is not None:
                            ...
                            byt_tb.stack_depth = min(byt_tb.stack_depth, instr.stack_depth)
                        ...
                        if last_try_end is not None:
                            ...
                            entry = cfg_te.entry
                            if entry.target is instr.target and entry.push_lasti == instr.push_lasti:
                                ...
                                if entry.stack_depth is UNSET:
                                    ...
                                    byt_te.entry.stack_depth = min(entry.stack_depth, instr.stack_depth)
