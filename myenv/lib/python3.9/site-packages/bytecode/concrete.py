...
from bytecode.instr import BaseInstr
...
from bytecode.instr import PLACEHOLDER_LABEL
...
from bytecode.instr import _check_arg_int
from bytecode.instr import const_key
...
class ConcreteBytecode(_bytecode._BaseBytecodeList[Union[ConcreteInstr, SetLineno]]):
    ...
    @staticmethod
    def from_code(code: types.CodeType, *, extended_arg: bool = False) -> "ConcreteBytecode":
        ...
        if sys.version_info >= (3, 11):
            ...
            if sys.version_info >= (3, 10):
                line_starts = dict((offset, lineno) for offset, _, lineno in code.co_lines())
            else:
        ...
        if sys.version_info >= (3, 11):
            bytecode.exception_table = bytecode._parse_exception_table(code.co_exceptiontable)
    ...
    # Used on 3.8 and 3.9
    @staticmethod
    def _assemble_lnotab(first_lineno: int, linenos: List[Tuple[int, int, int, Optional[InstrLocation]]]) -> bytes:
        lnotab = []
        old_offset = 0
    ...
    @staticmethod
    def _pack_linetable(linetable: List[bytes], doff: int, dlineno: Optional[int]) -> None:
    ...
    def _assemble_linestable(
        ...
        if old_location is not None:
            old_dlineno = old_location.lineno - first_lineno if old_location.lineno is not None else None
        else:
            old_dlineno = old_lineno - first_lineno

        for offset, i_size, lineno, location in iter_in:
            if location is not None:
                dlineno = location.lineno - old_lineno if location.lineno is not None else None
            else:
                dlineno = lineno - old_lineno
    ...
    @staticmethod
    def _remove_extended_args(instructions: MutableSequence[Union[SetLineno, ConcreteInstr]]) -> None:
    ...
    def _parse_exception_table(self, exception_table: bytes) -> List[ExceptionTableEntry]:
    ...
    def to_code(
        ...
        *,
        ...
        if stacksize is None or (sys.version_info >= (3, 11) and compute_exception_stack_depths):
    ...
    def to_bytecode(
        ...
        for lineno, c_instr in self._normalize_lineno(c_instructions, self.first_lineno):
            ...
            if isinstance(instr, Instr) and instr.name == "CACHE":
                ...
                if not self.seen_manual_cache:
                    ...
                    self.instructions.extend(
                        [
                            ConcreteInstr("CACHE", 0, location=self.instructions[-1].location)
    ...
    def compute_jumps(self) -> bool:
        ...
        for index, label, instr in self.jumps:
            ...
            if instr.is_forward_rel_jump():
                ...
                instr_offset = label_offsets[index]
                target_offset = (
                    instr_offset + (instr.size // 2 if OFFSET_AS_INSTRUCTION else instr.size) - target_offset
                )
