...
- ASM: API Security schema collection is now officially supported for Django, Flask and FastAPI. It can be enabled in the tracer
...
- Fixed an incompatible version requirements for one of the internal
...
- profiling: Fix an issue that prevented threading locks from being traced
  when using gevent.
...
.. _Release Notes_v2.3.0_New Features:
...
- openai: This introduces official tracing support for Azure OpenAI services, specifically for completions, 
...
- Fix an issue that could have caused some tracing integrations to leave the
...
- graphql: add tracing for ``graphql-core>2``. See `the graphql documentation <https://ddtrace.readthedocs.io/en/stable/integrations.html#graphql>`_
  for more information.
...
- The profiler now supports code provenance reporting. This can be enabled by
  using the ``enable_code_provenance=True`` argument to the profiler or by
  setting the environment variable ``DD_PROFILING_ENABLE_CODE_PROVENANCE`` to
  ``true``.


.. _Release Notes_v1.4.0rc1_Bug Fixes:
...
- Fixes cases where a pytest test parameter object string representation includes the ``id()``
...
.. _Release Notes_v1.4.0rc1_Other Changes:
...
.. _Release Notes_v1.3.0rc1:

v1.3.0rc1
=========

.. _Release Notes_v1.3.0rc1_New Features:
...
.. _Release Notes_v1.3.0rc1_Bug Fixes:
...
.. _Release Notes_v1.3.0rc1_Other Changes:
...
.. _Release Notes_v1.2.0rc1:

v1.2.0rc1
=========

.. _Release Notes_v1.2.0rc1_Upgrade Notes:
...
.. _Release Notes_v1.2.0rc1_New Features:
...
- The memory profile collector can now entirely disabled with the
  ``DD_PROFILING_MEMORY_ENABLED`` environment variable.

- psycopg2: add option to enable tracing ``psycopg2.connect`` method.
  
  See our :ref:`psycopg2` documentation for more information.

- Add asyncio support of redis ≥ 4.2.0


.. _Release Notes_v1.2.0rc1_Bug Fixes:
...
.. _Release Notes_v1.2.0rc1_Other Changes:

Other Changes
-------------

- The default number of events kept by the profiler has been reduced to
  decreased CPU and memory overhead.


.. _Release Notes_v1.1.0rc1:

v1.1.0rc1
=========

.. _Release Notes_v1.1.0rc1_Prelude:

Prelude
-------

The Datadog APM Python team is happy to announce the release of v1.0.0 of ddtrace. This release introduces a formal :ref:`versioning policy<versioning>` that simplifies the public :ref:`interface<versioning_interfaces>` and defines a :ref:`release version policy<versioning_release>` for backwards compatible and incompatible changes to the public interface.
...
Functionality changes
*********************
The default logging configuration functionality of ``ddtrace-run`` has changed to address conflicts with application logging configuration. See :ref:`note on the new default behavior<disable-basic-config-call-by-default>` and :ref:`note on deprecation<deprecate-basic-config-call>` for future removal.
...
**********************************
These tracing functions in integrations were no longer used for automatic instrumentation so have been removed. Any manual instrumentation code in an application will need to be replaced with :py:meth:`ddtrace.patch_all` or :py:meth:`ddtrace.patch` when upgrading.
...
.. _Release Notes_v1.1.0rc1_Upgrade Notes:
...
- .. _remove-ddtrace-monkey:
  ...
  The deprecated module ``ddtrace.monkey`` is removed. Use :py:func:`ddtrace.patch <ddtrace.patch>` or :py:func:`ddtrace.patch_all <ddtrace.patch_all>` instead.
...
- .. _remove-tracer-tags:
  ...
  ``ddtrace.Tracer.tags`` is removed. Use the environment variable :ref:`DD_TAGS<dd-tags>` to set the global tags instead.
...
- .. _remove-span-meta:
  ...
  ``ddtrace.Span.meta`` is removed. Use :py:meth:`ddtrace.Span.get_tag` and :py:meth:`ddtrace.Span.set_tag` instead.

- .. _remove-span-metrics:
  
  ``ddtrace.Span.metrics`` is removed. Use :py:meth:`ddtrace.Span.get_metric` and :py:meth:`ddtrace.Span.set_metric` instead.
...
- .. _remove-span-set-meta:
  ...
  ``ddtrace.Span.set_meta`` is removed. Use :py:meth:`ddtrace.Span.set_tag` instead.

- .. _remove-span-set-metas:
  
  ``ddtrace.Span.set_metas`` is removed. Use :py:meth:`ddtrace.Span.set_tags` instead.
...
- .. _remove-tracer-get-call-context:
  ...
  ``ddtrace.Tracer.get_call_context`` is removed. Use :py:meth:`ddtrace.Tracer.current_trace_context` instead.
...
- .. _remove-tracer-writer:
  ...
  ``ddtrace.Tracer.writer`` is removed. To force flushing of buffered traces to the agent, use :py:meth:`ddtrace.Tracer.flush` instead.
...
- .. _remove-flask-middleware:
  
  
  Removed deprecated middleware ``ddtrace.contrib.flask.middleware.py:TraceMiddleware``. Use :py:meth:`ddtrace.patch_all` or :py:meth:`ddtrace.patch` instead.

- .. _remove-pymongo-client:
  
  
  Removed deprecated function ``ddtrace.contrib.pymongo.patch.py:trace_mongo_client``. Use :py:meth:`ddtrace.patch_all` or :py:meth:`ddtrace.patch` instead.

- .. _remove-pymysql-connection:
  
  
  Removed deprecated function ``ddtrace.contrib.pymysql.tracers.py:get_traced_pymysql_connection``. Use :py:meth:`ddtrace.patch_all` or :py:meth:`ddtrace.patch` instead.

- .. _remove-redis-traced:
  
  
  Removed deprecated function ``ddtrace.contrib.redis.tracers.py:get_traced_redis``. Use :py:meth:`ddtrace.patch_all` or :py:meth:`ddtrace.patch` instead.

- .. _remove-redis-traced-from:
  
  
  Removed deprecated function ``ddtrace.contrib.redis.tracers.py:get_traced_redis_from``. Use :py:meth:`ddtrace.patch_all` or :py:meth:`ddtrace.patch` instead.


.. _Release Notes_v1.1.0rc1_Deprecation Notes:
...
.. _Release Notes_v1.1.0rc1_New Features:
...
- aiohttp_jinja2: move into new integration. Formerly the aiohttp_jinja2 instrumentation was enabled using the aiohttp
  integration. Use ``patch(aiohttp_jinja2=True)`` instead of ``patch(aiohttp=True)``. To support legacy
  behavior ``patch(aiohttp=True)`` will still enable aiohttp_jinja2.
...
- MySQLdb: Added optional tracing for MySQLdb.connect, using the configuration option
  :ref:`here<mysqldb_config_trace_connect>`.
...
.. _Release Notes_v1.1.0rc1_Bug Fixes:
...
.. _Release Notes_v1.1.0rc1_Other Changes:
...
.. _Release Notes_v0.59.0rc1:

v0.59.0rc1
==========

.. _Release Notes_v0.59.0rc1_Deprecation Notes:
...
- :py:attr:`ddtrace.Span.metrics` is deprecated. Use :py:meth:`ddtrace.Span.get_metric` and :py:meth:`ddtrace.Span.set_metric` instead.
...
.. _Release Notes_v0.59.0rc1_New Features:

New Features
------------

- The profiler now reports asyncio tasks as part as the task field in
  profiles. This is enabled by default by replacing the default asyncio loop
  policy. CPU time, wall time and threading lock times are supported.

- Add tracing support for ``sanic>=21.9.0``.


.. _Release Notes_v0.59.0rc1_Bug Fixes:
...
.. _Release Notes_v0.58.0rc1:

v0.58.0rc1
==========

.. _Release Notes_v0.58.0rc1_Deprecation Notes:
...
- :py:attr:`ddtrace.Span.meta` has been deprecated. Use :py:meth:`ddtrace.Span.get_tag` and :py:meth:`ddtrace.Span.set_tag` instead.
...
.. _Release Notes_v0.58.0rc1_New Features:
...
.. _Release Notes_v0.58.0rc1_Bug Fixes:
...
- Fix an issue in the heap profiler where it would iterate on the wrong heap
  allocation tracker.

- Pymongo instrumentation raises an AttributeError when ``tracer.enabled == False``


.. _Release Notes_v0.57.0:

v0.57.0
=======

.. _Release Notes_v0.57.0_Deprecation Notes:
...
.. _Release Notes_v0.57.0_New Features:
...
.. _Release Notes_v0.57.0_Bug Fixes:
...
.. _Release Notes_v0.56.0rc3:

v0.56.0rc3
==========

.. _Release Notes_v0.56.0rc3_Deprecation Notes:

Deprecation Notes
-----------------

- The ``ddtrace.utils`` module and all of its submodules have been copied over
  into ``ddtrace.internal`` in an effort to internalize these modules. Their
  public counterparts will be removed entirely in version 1.0.0.


.. _Release Notes_v0.56.0rc3_New Features:
...
.. _Release Notes_v0.56.0rc3_Bug Fixes:
...
.. _Release Notes_v0.56.0rc2:

v0.56.0rc2
==========

.. _Release Notes_v0.56.0rc2_Bug Fixes:
...
.. _Release Notes_v0.56.0rc1:

v0.56.0rc1
==========

.. _Release Notes_v0.56.0rc1_Upgrade Notes:
...
.. _Release Notes_v0.56.0rc1_Deprecation Notes:

Deprecation Notes
-----------------

- The contents of ``monkey.py`` have been moved into ``_monkey.py`` in an
  effort to internalize the module. Public methods have been imported back
  into ``monkey.py`` in order to retain compatibility, but monkey.py will be
  removed entirely in version 1.0.0.


.. _Release Notes_v0.56.0rc1_Bug Fixes:
...
.. _Release Notes_v0.55.0rc1:

v0.55.0rc1
==========

.. _Release Notes_v0.55.0rc1_Upgrade Notes:
...
.. _Release Notes_v0.55.0rc1_Deprecation Notes:

Deprecation Notes
-----------------

- Deprecate DATADOG_TRACE_AGENT_HOSTNAME, DATADOG_TRACE_AGENT_PORT, DATADOG_PRIORITY_SAMPLING, DATADOG_PATCH_MODULES
  in favor of their DD equivalents.
  ...
  * For ``DATADOG_PRIORITY_SAMPLING``, `follow ingestion controls <https://docs.datadoghq.com/tracing/trace_retention_and_ingestion/#recommended-setting-the-global-ingestion-rate-to-100>`_
...
.. _Release Notes_v0.55.0rc1_New Features:
...
- Add new ``DD_TRACE_SAMPLING_RULES`` environment variable to override default sampling rules. For Example::  
  ``DD_TRACE_SAMPLING_RULES='[{"sample_rate":0.5,"service":"my-service"}]'``
...
.. _Release Notes_v0.55.0rc1_Bug Fixes:
...
.. _Release Notes_v0.54.0rc1:

v0.54.0rc1
==========

.. _Release Notes_v0.54.0rc1_Deprecation Notes:

Deprecation Notes
-----------------

- Deprecate the DATADOG_ENV environment variable in favor of DD_ENV. 
  The use of DD_ENV should follow `Unified Service Tagging <https://docs.datadoghq.com/getting_started/tagging/unified_service_tagging>`_
  recommendations.


.. _Release Notes_v0.54.0rc1_New Features:
...
.. _Release Notes_v0.54.0rc1_Bug Fixes:
...
- Fixed the support for Celery workers that fork sub-processes with Python
...
.. _Release Notes_v0.54.0rc1_Other Changes:
...
.. _Release Notes_v0.53.0rc1:

v0.53.0rc1
==========

.. _Release Notes_v0.53.0rc1_Upgrade Notes:
...
.. _Release Notes_v0.53.0rc1_Deprecation Notes:
...
.. _Release Notes_v0.53.0rc1_New Features:
...
.. _Release Notes_v0.53.0rc1_Bug Fixes:
...
.. _Release Notes_v0.52.0rc1:

v0.52.0rc1
==========

.. _Release Notes_v0.52.0rc1_Deprecation Notes:
...
.. _Release Notes_v0.52.0rc1_New Features:
...
.. _Release Notes_v0.52.0rc1_Bug Fixes:
...
.. _Release Notes_v0.51.0rc2:

v0.51.0rc2
==========

.. _Release Notes_v0.51.0rc2_Upgrade Notes:
...
.. _Release Notes_v0.51.0rc2_New Features:
...
.. _Release Notes_v0.51.0rc2_Bug Fixes:
...
.. _Release Notes_v0.51.0rc1:

v0.51.0rc1
==========

.. _Release Notes_v0.51.0rc1_Upgrade Notes:

Upgrade Notes
-------------

- The legacy Django configuration method (deprecated in 0.34) has
  been removed.


.. _Release Notes_v0.51.0rc1_Deprecation Notes:
...
.. _Release Notes_v0.51.0rc1_New Features:
...
.. _Release Notes_v0.51.0rc1_Bug Fixes:
...
.. _Release Notes_v0.50.0rc5:

v0.50.0rc5
==========

.. _Release Notes_v0.50.0rc5_New Features:
...
.. _Release Notes_v0.50.0rc5_Other Changes:
...
.. _Release Notes_v0.50.0rc4:

v0.50.0rc4
==========

.. _Release Notes_v0.50.0rc4_New Features:
...
.. _Release Notes_v0.50.0rc3:

v0.50.0rc3
==========

.. _Release Notes_v0.50.0rc3_Deprecation Notes:

Deprecation Notes
-----------------

- The deprecated dbapi2 configuration has been removed. The
  integration-specific configuration should be used instead. Look at the
  v0.48.0 release notes for migration instructions.


.. _Release Notes_v0.50.0rc3_New Features:
...
.. _Release Notes_v0.50.0rc3_Bug Fixes:
...
.. _Release Notes_v0.50.0rc2:

v0.50.0rc2
==========

.. _Release Notes_v0.50.0rc2_Deprecation Notes:

Deprecation Notes
-----------------

- The reuse of a tracer that has been shut down has been deprecated. A new
  tracer should be created for generating new traces.


.. _Release Notes_v0.50.0rc2_New Features:
...
- The CI tagging for the pytest plugin now includes OS and Python Runtime metadata including
  system architecture, platform, version, and Python runtime name and version.


.. _Release Notes_v0.50.0rc2_Bug Fixes:

Bug Fixes
---------

- The shutdown task is re-registered when a tracer is reused after it has been
  shut down.
...
.. _Release Notes_v0.50.0rc2_Other Changes:

Other Changes
-------------

- The pytest plugin now includes git metadata tags including author name and email as well as commit message
  from CI provider environments.

- The profiler won't be ignoring its own resource usage anymore and will
  report it in the profiles.


.. _Release Notes_v0.50.0rc1:

v0.50.0rc1
==========

.. _Release Notes_v0.50.0rc1_Prelude:
...
.. _Release Notes_v0.50.0rc1_Upgrade Notes:
...
.. _Release Notes_v0.50.0rc1_New Features:
...
- A warning has been added to alert when gevent monkey patching is done after
  ddtrace has been imported.
...
.. _Release Notes_v0.50.0rc1_Bug Fixes:
...
- A possible memory leak that occurs when tracing across a fork has been
  fixed. See https://github.com/DataDog/dd-trace-py/pull/2497 for more
  information.

- Fix double patching of ``pymongo`` client topology.


.. _Release Notes_v0.49.0rc1:

v0.49.0rc1
==========

.. _Release Notes_v0.49.0rc1_Prelude:
...
.. _Release Notes_v0.49.0rc1_Upgrade Notes:
...
- Support for deprecated `DD_PROFILING_API_URL` environment variable has been
  removed. Use `DD_SITE` instead.

- Support for deprecated `DD_PROFILING_API_KEY` environment variable has been
  removed. Use `DD_API_KEY` instead.
...
.. _Release Notes_v0.49.0rc1_Deprecation Notes:
...
- ``Context.get_current_span`` is deprecated and will be removed in 0.50 please
  use `Tracer.current_span` instead.

- ``Context.get_current_root_span`` is deprecated and will be removed in 0.50
  please use ``Tracer.current_root_span`` instead.
...
- The use of ``ddtrace.encoding`` has been deprecated and will be removed
  in version 1.0.0.

- The `ddtrace.http` module has been deprecated and will be removed in version 1.0.0,
  with the `ddtrace.http.headers` module now merged into `ddtrace.trace_utils`.

- The ``collect_metrics`` argument of the ``tracer.configure`` method has been
  deprecated. Runtime metrics should be enabled only via the
  ``DD_RUNTIME_METRICS_ENABLED`` environment variable.


.. _Release Notes_v0.49.0rc1_New Features:
...
.. _Release Notes_v0.49.0rc1_Bug Fixes:

Bug Fixes
---------

- Fix broken builds for Python 2.7 on windows where ``<stdint.h>`` was not available.
  This change also ensures we build and publish ``cp27-win`` wheels.
...
- Fix DD_LOGS_INJECTION incompatibility when using a ``logging.StrFormatStyle`` (``logging.Formatter(fmt, style="{")``)
  log formatter.
...
- Added missing pymongo integration configuration, which allows overriding
  the service name for all the emitted spans.


.. _Release Notes_v0.49.0rc1_Other Changes:
...
- The `attrs` library has been unvendored and is now required as a normal Python dependency
  with a minimum version requirement of 19.2.0.
...
- Documentation on how to use Gunicorn with the ``gevent`` worker class has
  been added.

- Added environment variable `DD_FALCON_DISTRIBUTED_TRACING` to enable distributed tracing for falcon.

- When extracting context information from HTTP headers, a new context is
  created when the trace ID is either 0 or not available within the headers.
...
.. _Release Notes_v0.48.0rc2:

v0.48.0rc2
==========

.. _Release Notes_v0.48.0rc2_New Features:

New Features
------------

- Adds support for the Datadog Lambda Extension. The tracer will send traces
  to the extension by default if it is present.
...
.. _Release Notes_v0.48.0rc1:

v0.48.0rc1
==========

.. _Release Notes_v0.48.0rc1_Upgrade Notes:

Upgrade Notes
-------------

- The deprecated `dogstatsd_host` and `dogstatsd_port` arguments to
  `tracer.configure()` have been removed.
...
- The deprecated ``pyddprofile`` wrapper has been removed. Use
  ``ddtrace-run`` with ``DD_PROFILING_ENABLED=1`` set instead.

- A `ValueError` will now be raised on tracer initialization if the Agent URL specified to the initializer or with
  the environment variable `DD_TRACE_AGENT_URL` is malformed.
...
.. _Release Notes_v0.48.0rc1_New Features:
...
- pylons: now supports all the standard http tagging including query string,
  custom error codes, and request/response headers.

- The ddtrace pytest plugin can now call ``ddtrace.patch_all`` via the
  ``--ddtrace-patch-all`` option.

- ``Span`` now accepts a ``on_finish`` argument used for specifying
  functions to call when a span finishes.


.. _Release Notes_v0.48.0rc1_Bug Fixes:
...
- The tracer now captures the task ID from the cgroups file for Fargate >= 1.4.0
  and reports it to the agent as the Datadog-Container-ID tag.

- Fix a bug when tracing a mako ``DefTemplate`` or any ``Template`` that does
  not have a ``filename`` property.
...
.. _Release Notes_v0.48.0rc1_Other Changes:

Other Changes
-------------

- The logic behind the header extraction for distributed tracing has been
  improved.

- The default connection timeout for the profiling agent has now been reduced
  from 10 to 2 seconds to match the tracer behavior.

- The tracemalloc memory profiler, which was disabled by default, has been removed.

- Query strings are stripped out from URLs by default when setting URL
  metadata on a span. This change affects all integrations that store HTTP
  metadata, like aiohttp, falcon, requests, urllib3.


.. _Release Notes_v0.47.0rc1:

v0.47.0rc1
==========

.. _Release Notes_v0.47.0rc1_Upgrade Notes:
...
.. _Release Notes_v0.47.0rc1_New Features:
...
- Introduce support for the DD_SERVICE_MAPPING environment variable to allow
  remapping service names on emitted spans.
...
.. _Release Notes_v0.47.0rc1_Bug Fixes:
...
- Fix CPU time and wall time profiling not ignoring the profiler tasks with
  gevent.


.. _Release Notes_v0.47.0rc1_Other Changes:

Other Changes
-------------

- The default maximum CPU time used for the stack profiler (CPU time, wall
  time and exceptions profiling) has been decreased from 2% to 1%.


.. _Release Notes_v0.46.0rc1:

v0.46.0rc1
==========

.. _Release Notes_v0.46.0rc1_Upgrade Notes:

Upgrade Notes
-------------

- The profiler will only load tags from the DD_TAGS environment variable
  once at start.


.. _Release Notes_v0.46.0rc1_Deprecation Notes:

Deprecation Notes
-----------------

- flask: Use :ref:`HTTP Custom Error Codes <http-custom-error>` instead of ``ddtrace.config.flask['extra_error_codes']``.


.. _Release Notes_v0.46.0rc1_New Features:
...
- The `ddtrace.profiling.auto` module will warn users if gevent monkey
  patching is done after the profiler is auto-instrumented.

- The `Profiler` object can now be passed tags with the `tags` keyword
  argument.


.. _Release Notes_v0.46.0rc1_Bug Fixes:
...
.. _Release Notes_v0.45.0rc1:

v0.45.0rc1
==========

.. _Release Notes_v0.45.0rc1_Prelude:
...
.. _Release Notes_v0.45.0rc1_Upgrade Notes:
...
.. _Release Notes_v0.45.0rc1_Deprecation Notes:
...
.. _Release Notes_v0.45.0rc1_New Features:
...
- Add functionality to call gevent.monkey.patch_all() with ddtrace-run by
  setting the environment variable DD_GEVENT_PATCH_ALL=true. This ensures
  that gevent patching is done as early as possible in the application.
...
- The profiler is now able to upload profiles to the Datadog Agent by using a
  Unix Domain Socket.

- It is now possible to pass a `url` parameter to the `Profiler` to specify
  the Datadog agent location.

- The new memory profiler for Python is now enabled by default. This improves
  the profiler memory consumption and performance impact. It can still be
  disabled by setting `DD_PROFILING_MEMALLOC=0` as an environment variable.

- The profiler now uses the tracer configuration is no configuration is provided.

- pytest integration.
  This enables the `pytest <https://pytest.org>`_ runner to trace test executions.


.. _Release Notes_v0.45.0rc1_Bug Fixes:
...
.. _Release Notes_v0.45.0rc1_Other Changes:

Other Changes
-------------

- Start-up logs are now disabled by default. To enable start-up logs use
  `DD_TRACE_STARTUP_LOGS=true` or `DD_TRACE_DEBUG=true`.


.. _Release Notes_v0.44.0:

v0.44.0
=======

.. _Release Notes_v0.44.0_Prelude:
...
.. _Release Notes_v0.44.0_New Features:
...
- pyodbc integration.
  This enables the `pyodbc <https://github.com/mkleehammer/pyodbc>`_ library to trace queries. 

- starlette integration resource aggregation
  This aggregates endpoints to the starlette application resource that was accessed.
  It occurs by default but it is configurable through config.starlette["aggregate_resources"].

- The profiler now captures the traces information with the lock profiling.

- The Profiler instances now restart automatically in child process when the
  main program is forked.
  This only works for Python ≥ 3.7.


.. _Release Notes_v0.44.0_Bug Fixes:
