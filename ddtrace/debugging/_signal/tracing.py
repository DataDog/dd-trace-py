            ...
            for d in probe.decorations:
                ...
                for tag in d.tags:
                    try:
                        ...
                        span._set_attribute("_dd.di.%s.evaluation_error" % tag.name, ", ".join([serialize(v) for v in e.args]))
                    else:
                        span._set_attribute(tag.name, tag_value if _isinstance(tag_value, str) else serialize(tag_value))
