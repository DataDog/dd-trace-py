    ...
    async def __call__(self, scope: Mapping[str, Any], receive: Callable, send: Callable):
        ...
        with core.context_with_data(
            ...
            if not self.integration_config.trace_query_string:
                query_string = None
            body = None
            result = core.dispatch_with_results("asgi.request.parse.body", (receive, headers)).await_receive_and_body  # ast-grep-ignore: core-dispatch-with-results
            if result:
            ...
            async def wrapped_blocked_send(message: Mapping[str, Any]):
                result = core.dispatch_with_results("asgi.block.started", (ctx, url)).status_headers_content  # ast-grep-ignore: core-dispatch-with-results
                if result:
