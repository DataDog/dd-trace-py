...
def patch():
    ...
    if algoliasearch_version < V2 and algoliasearch_version >= V1:
        ...
        _w("algoliasearch.search.client", "SearchClientSync.search_single_index", _patched_search_v3)
        _w("algoliasearch.search.client", "SearchClientSync.search", _patched_search_v3)
        _w("algoliasearch.search.client", "SearchClient.search_single_index", _patched_search_v3_async)
        _w("algoliasearch.search.client", "SearchClient.search", _patched_search_v3_async)
...
def _patched_search_v3(func, instance, wrapt_args, wrapt_kwargs):
    pin = Pin.get_from(instance)
    if not pin or not pin.enabled():
        return func(*wrapt_args, **wrapt_kwargs)

    span = _start_v3_span(pin)
    try:
        if span.context.sampling_priority is not None and span.context.sampling_priority <= 0:
            span.finish()
            return func(*wrapt_args, **wrapt_kwargs)

        query_text, params_dict = _extract_v3_query_and_args(func.__name__, wrapt_args, wrapt_kwargs)
...
async def _patched_search_v3_async(func, instance, wrapt_args, wrapt_kwargs):
    pin = Pin.get_from(instance)
    if not pin or not pin.enabled():
