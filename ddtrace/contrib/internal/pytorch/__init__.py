...
``DD_PYTORCH_SUMMARY_PROFILING``
    ...
    ``train.*``, ``optim.*``, ``memory.*`` numeric facets on the rotated
    ``pytorch.rank`` span every ~5 minutes. No per-step spans are emitted
    in this mode. Set to ``false`` to skip the wraps entirely (escape hatch
    for ultra-async pipelines).
