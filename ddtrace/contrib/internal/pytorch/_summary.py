...
drain pass is called from `_rank_root._tag_collective_summary` at
rotation time (~every 5 minutes). The rotation cost is bounded by the
reservoir count, not by the step count.
...
def drain_all_to_facets() -> dict[str, float]:
    ...
    Called once per `_rank_root` rotation. Empty reservoirs contribute
    ...
    if prev_step_end_ns > 0:
        ...
        if mfu_enabled and _model_is_transformer and acc.tokens_this_step > 0 and step_ms > 0:
            try:
                ...
                # The 12 × layers × hidden × seq term is small for typical seqs.
                flops_per_token = 6.0 * _model_param_count
                ...
                peak = lookup_peak_flops(gpu_name, _model_dtype)
                if peak:
                    push_distribution("train.mfu", achieved / peak)
                    if 0 < acc.data_fetch_ms < step_ms:
                        ...
                        push_distribution("train.mfu_exclude_dataload", achieved_compute / peak)
            except Exception:
                pass
