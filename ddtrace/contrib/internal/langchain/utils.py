...
def _iast_create_taint_chunk_callback(source):
    def _iast_chunk_taint(chunk):
        print(f"_iast_chunk_taint: chunk={chunk}, source={source}")
...
def _iast_get_tainted_source_from_chat_prompt_value(chat_prompt_value):
    ...
    if not isinstance(messages, (tuple, list)):
        print(f"_iast_get_tainted_source_from_messages: type={type(messages)} messages={messages}")
        return None
    print(f"_iast_get_tainted_source_from_messages: messages={messages}")
    ...
    for message in messages:
        ...
        print(f"_iast_get_tainted_source_from_messages: message={message}")
        if not isinstance(content, str):
            continue
        tainted_ranges = get_tainted_ranges(content)
        if tainted_ranges:
            print(f"_iast_get_tainted_source_from_messages: tainted_ranges={tainted_ranges}")
...
def _iast_taint_chunk(source, chunk):
    ...
    if not asm_config._iast_enabled:
        return
    print(f"_iast_taint_chunk: source={source}, chunk ({type(chunk)})={chunk}")
    if not source:
        return
    message = chunk
    if not hasattr(message, "content"):
        ...
        if isinstance(additional_kwargs, dict) and "function_call" in additional_kwargs:
            ...
            if isinstance(function_call, dict) and "arguments" in function_call:
                ...
                if isinstance(arguments, str):
                    print("_iast_taint_chunk tainted arguments")
