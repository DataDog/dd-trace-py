...
RAY_SERVICE_NAME = env.dd_environ.get(RAY_JOB_NAME)
...
config._add(
    ...
    dict(
        ...
        use_entrypoint_as_service_name=asbool(env.getenv("DD_TRACE_RAY_USE_ENTRYPOINT_AS_SERVICE_NAME", default=False)),
        redact_entrypoint_paths=asbool(env.getenv("DD_TRACE_RAY_REDACT_ENTRYPOINT_PATHS", default=True)),
...
def _job_supervisor_run_wrapper(method: Callable[..., Any]) -> Any:
    async def _traced_run_method(self: Any, *args: Any, _dd_ray_trace_ctx=None, **kwargs: Any) -> Any:
        ...
        with long_running_ray_span(
            ...
            service=env.dd_environ.get(RAY_JOB_NAME, DEFAULT_JOB_NAME),
            span_type=SpanTypes.RAY,
            child_of=context,
            activate=True,
