...
def extract_embedding_shape_info(tensor: torch.Tensor) -> tuple[Optional[int], Optional[int]]:
...
def extract_lora_name(lora_request: Optional[LoRARequest]) -> Optional[str]:
    """Extract LoRA adapter name from LoRARequest."""
    if not lora_request:
        return None
...
def extract_v0_data(seq_group: SequenceGroup) -> RequestData:
    ...
    data = RequestData(
        ...
        lora_name=extract_lora_name(seq_group.lora_request),
    )

    # Embedding path
    if seq_group.pooling_params is not None:
        num_emb, emb_dim = extract_embedding_shape_info(seq_group.pooled_data)
...
def extract_offline_data(request_output: RequestOutput, prompts=None) -> RequestData:
    ...
    data = RequestData(
        ...
        lora_name=extract_lora_name(request_output.lora_request),
    )
...
def extract_offline_pooling_data(request_output: PoolingRequestOutput, prompts=None) -> RequestData:
    ...
    if outputs is not None:
        ...
        num_emb, emb_dim = extract_embedding_shape_info(maybe_tensor)
