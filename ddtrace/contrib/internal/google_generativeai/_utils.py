...
class TracedGenerateContentResponse(BaseTracedGenerateContentResponse):
    def __iter__(self):
        try:
            ...
            tag_response(GOOGLE_GENERATIVEAI, self._dd_span, self.__wrapped__, self._dd_integration, self._model_instance)
        finally:
...
def _tag_request_content_part(span, integration, tag_prefix, part, part_idx, content_idx):
...
def _tag_request_content(span, integration, tag_prefix, content, content_idx):
    """Tag the generation span with request contents."""
    if isinstance(content, str):
        span.set_tag_str("%s.request.contents.%d.text" % (tag_prefix, content_idx), integration.trunc(content))
        return
    if isinstance(content, dict):
        role = content.get("role", "")
        if role:
            span.set_tag_str("%s.request.contents.%d.role" % (tag_prefix, content_idx), str(content.get("role", "")))
        parts = content.get("parts", [])
        for part_idx, part in enumerate(parts):
            _tag_request_content_part(span, integration, tag_prefix, part, part_idx, content_idx)
        return
    role = getattr(content, "role", "")
    if role:
        span.set_tag_str("%s.request.contents.%d.role" % (tag_prefix, content_idx), str(role))
    parts = getattr(content, "parts", [])
    if not parts:
        span.set_tag_str(
            "%s.request.contents.%d.text" % (tag_prefix, content_idx),
...
def _tag_response_part(tag_prefix, span, integration, part, part_idx, candidate_idx):
...
def get_system_instruction_parts(instance):
    ...
    if system_instruction:
        return system_instruction.parts
...
def get_generation_config_dict(instance, kwargs):
    ...
    if generation_config != {}:
        try:
            return to_generation_config_dict(generation_config)
        except TypeError:
            return None
...
def tag_request(span, integration, args, kwargs, tag_prefix, system_instructions, generation_config_dict):
    ...
    if generation_config_dict is not None:
        for k, v in generation_config_dict.items():
            span.set_tag_str("%s.request.generation_config.%s" % (tag_prefix, k), str(v))

    if stream:
        span.set_tag("%s.request.stream" % tag_prefix, True)
    ...
    if isinstance(contents, str):
        span.set_tag_str("%s.request.contents.0.text" % tag_prefix, integration.trunc(contents))
        return
    elif isinstance(contents, dict):
        span.set_tag_str("%s.request.contents.0.text" % tag_prefix, integration.trunc(str(contents)))
        return
    elif not isinstance(contents, list):
        return
    for content_idx, content in enumerate(contents):
        _tag_request_content(span, integration, tag_prefix, content, content_idx)


def tag_response(tag_prefix, span, generations, integration, instance):
    ...
    for candidate_idx, candidate in enumerate(generations_dict.get("candidates", [])):
        ...
        for part_idx, part in enumerate(parts):
            _tag_response_part(tag_prefix, span, integration, part, part_idx, candidate_idx)

    token_counts = generations_dict.get("usage_metadata", None)
    if not token_counts:
        return
    span.set_metric("%s.response.usage.prompt_tokens" % tag_prefix, token_counts.get("prompt_token_count", 0))
    span.set_metric(
        "%s.response.usage.completion_tokens" % tag_prefix, token_counts.get("candidates_token_count", 0)
    )
    span.set_metric("%s.response.usage.total_tokens" % tag_prefix, token_counts.get("total_token_count", 0))
