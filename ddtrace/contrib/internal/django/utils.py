...
def _extract_body(request):
    ...
    if request.method in _BODY_METHODS:
        ...
        headers = core.dispatch_with_results("django.extract_body").headers.value  # ast-grep-ignore: core-dispatch-with-results
        try:
...
def _after_request_tags(pin, span: Span, request, response):
    ...
    try:
        ...
        if response:
            ...
            request_headers = core.dispatch_with_results("django.after_request_headers").headers.value  # ast-grep-ignore: core-dispatch-with-results
