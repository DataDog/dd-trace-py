...
def normalize_usage(raw: Any, operation: str = "completion") -> Usage:
    ...
    try:
        ...
        completion = get(raw, "completion_tokens_details")
        ...
        if prompt > 2**53:
            raise _UsageError("invalid_usage")
        result.diagnostics.update(input_tokens=prompt, output_tokens=output, context_tokens=prompt)
        ...
        audio_in = count(details, "audio_tokens", 0) or 0
        audio_out = count(completion, "audio_tokens", 0) or 0
        reasoning = count(completion, "reasoning_tokens", 0) or 0
        if cached + written > prompt or audio_out > output or reasoning > output:
            raise _UsageError("inconsistent_usage")
        if audio_in or audio_out or count(details, "image_tokens", 0):
        ...
        for name in ("web_search_requests", "tool_search_requests"):
            value = count(get(raw, "server_tool_use"), name)
            if value is not None:
                result.quantities[name] = value
...
class DatadogSink:
    ...
    def __call__(self, record: UsageRecord) -> None:
        ...
        try:
            ...
            for key, value in record.usage.diagnostics.items():
                span._set_attribute(f"ai.observed.{key}", value)
        finally:
