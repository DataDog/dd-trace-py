...
from .utils import httpx2_url_to_str


HTTPX2_VERSION = parse_version(httpx2.__version__)
HTTP_REQUEST_TAGS = {COMPONENT: config.httpx2.integration_name, SPAN_KIND: SpanKind.CLIENT}
...
def _get_service_name(request: httpx2.Request) -> Optional[str]:
    if config.httpx2.split_by_domain:
    ...
    return ext_service(None, config.httpx2)
...
def _wrapped_sync_send_single_request(
    ...
    req: httpx2.Request = get_argument_value(args, kwargs, 0, "request")
    with core.context_with_event(
        event=HttpClientSendEvent(
            request_url=httpx2_url_to_str(req.url),
...
async def _wrapped_async_send(
    ...
    req: httpx2.Request = get_argument_value(args, kwargs, 0, "request")  # type: ignore
