...
def patch():
    ...
    _w(jinja2, "environment.Template.generate", _wrap_generate)
...
def _wrap_generate(wrapped, instance, args, kwargs):
    pin = Pin.get_from(instance.environment)
    if not pin or not pin.enabled():
        for s in wrapped(*args, **kwargs):
            yield s

    template_name = instance.name or DEFAULT_TEMPLATE_NAME
    with pin.tracer.trace("jinja2.generate", span_type=http.TEMPLATE) as span:
...
def _wrap_render(wrapped, instance, args, kwargs):
    ...
    template_name = instance.name or DEFAULT_TEMPLATE_NAME
    print("render tracer", id(pin.tracer))
    with pin.tracer.trace("jinja2.render", span_type=http.TEMPLATE) as span:
        # update the span service name before doing any action
        span.service = _get_service_name(instance.environment, span)
        try:
            return wrapped(*args, **kwargs)
