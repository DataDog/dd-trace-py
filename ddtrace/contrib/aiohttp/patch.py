...
class _WrappedConnectorClass(wrapt.ObjectProxy):
    ...
    async def connect(self, req, *args, **kwargs):
        ...
        with pin.tracer.trace(
            "{}.connect".format(self.__class__.__name__), span_type=ext_http.TYPE, service=pin.service
        ) as span:
    ...
    async def _create_connection(self, req, *args, **kwargs):
        pin = Pin.get_from(self)
        with pin.tracer.trace(
            "{}._create_connection".format(self.__class__.__name__), span_type=ext_http.TYPE, service=pin.service
        ) as span:
            _set_request_tags(span, _get_url_obj(req))
...
class _WrappedStreamReader(wrapt.ObjectProxy):
    ...
    async def _read(self, method_name, *args, **kwargs):
        ...
        with pin.tracer.trace(
            "{}.{}".format(self.__class__.__name__, method_name), span_type=ext_http.TYPE, service=pin.service
        ) as span:
...
class _WrappedResponseClass(wrapt.ObjectProxy):
    ...
    async def start(self, *args, **kwargs):
        ...
        with pin.tracer.trace(
            "{}.start".format(self.__class__.__name__), span_type=ext_http.TYPE, service=pin.service
        ) as span:
...
_clientsession_wrap_methods = {"get", "options", "head", "post", "put", "patch", "delete", "request"}
...
def patch():
    ...
    if not getattr(aiohttp, "__datadog_patch", False):
        ...
        pin = Pin(config.aiohttp_client.service, _config=config.aiohttp_client)
