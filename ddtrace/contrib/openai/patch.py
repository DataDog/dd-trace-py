...
def patched_create(openai, pin, func, instance, args, kwargs):
    ...
    with pin.tracer.trace("openai.request", resource=resource) as span:
        ...
        set_flattened_tags(
            span, append_tag_prefixes([REQUEST_TAG_PREFIX], process_request(openai, instance.OBJECT_NAME, args, kwargs))
        )
        try:
            resp = func(*args, **kwargs)
            set_flattened_tags(
                span, append_tag_prefixes([RESPONSE_TAG_PREFIX], process_response(openai, instance.OBJECT_NAME, resp))
            )
            return resp
