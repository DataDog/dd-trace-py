...
def traced_get_response(django, pin, func, instance, args, kwargs):
    ...
    print('XXX func: %s' % func)
    print('XXX func args: %s' % args)
    print('XXX func kwargs: %s' % kwargs)
    ...
    with pin.tracer.trace(
        ...
        # XXX factorize to ddwaf_check_blocked(peer_ip, span, request_headers, headers_case_sensitive)
        ip = None
        processor = pin.tracer._appsec_processor

        # Do an early run with ddwaf and check if this IP must be blocked
        if config._appsec_enabled and processor:
            ...
            if ip:
                if processor._is_needed(_Addresses.HTTP_CLIENT_IP):
                    ...
                    if res:
                        ...
                        for r in res_dict:
                            if r.get("rule", {}).get("id") == 'ip_match_rule' and\
                                ...
                                _context.set_item("http.request.waf_json", '{"triggers":%s}' % (res,))
                                _context.set_item("http.request.waf_duration", total_runtime)
                                _context.set_item("http.request.waf_duration_ext", total_overall_runtime)
                                _context.set_item("http.request.blocked", True)
                                _context.set_item("http.request.remote_ip", ip)
                                func = HttpResponse
                                args = []
                                kwargs = {
                                    # XXX move these two to the constants module
                                    "content": "XXX this should be the loaded 403 template",
                                    "status": 403
                                }
        ...
        try:
            ...
            utils._after_request_tags(pin, span, request, response, ip, request_headers, headers_case_sensitive)
