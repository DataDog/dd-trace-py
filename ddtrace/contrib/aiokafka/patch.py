...
async def traced_send(func, instance, args, kwargs):
    ...
    if not pin or not pin.enabled() or caller_frame.function == "send_and_wait":
        return await func(*args, **kwargs)

    topic, value, headers, partition, key, servers = parse_send(instance, args, kwargs)
    ...
    with core.context_with_data(
        ...
        # Callback when message is actually sent
        def sent_callback(future):
            core.dispatch("aiokafka.send.completed", (future.result(),))
...
def create_get_span_tags(instance):
    tags = {
        COMPONENT: config.aiokafka.integration_name,
...
async def traced_getone(func, instance, args, kwargs):
    ...
    try:
        ...
        with core.context_with_data(
            ...
            tags=create_get_span_tags(instance),
            pin=pin,
        ) as ctx, ctx[ctx["call_key"]] as span:
        ...
        for tp, records in result.items():
            for record in records:
                ...
                if config.aiokafka.distributed_tracing_enabled and record.headers:
                    child_of = HTTPPropagator.extract(convert_headers_for_distributed_tracing(record.headers))
                with core.context_with_data(
                    "aiokafka.getmany",
                    call_trace=False,
                    span_name=schematize_messaging_operation(
                        CONSUME, provider="kafka", direction=SpanDirection.INBOUND
                    ),
                    span_type=SpanTypes.WORKER,
                    service=trace_utils.ext_service(pin, config.aiokafka),
                    call_key="instrumented_getmany",
                    distributed_context=child_of,
                    tags=create_get_span_tags(instance),
                    pin=pin,
                ) as ctx, ctx[ctx["call_key"]] as span:
                    core.dispatch("aiokafka.getone.message", (ctx, record, err))
                    core.dispatch("aiokafka.get.completed", (instance, record, span))
        return result
