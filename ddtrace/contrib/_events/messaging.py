...
def _init_messaging_event(event: _MessagingEvent, operation: str, direction: SpanDirection) -> None:
    """Set the span name and common messaging tags shared by all messaging events."""
    event.span_name = schematize_messaging_operation(
...
class MessagingConsumeEvent(TracingEvent):
    ...
    #: If set, overrides the operation token used when building span_name while
