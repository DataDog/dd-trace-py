...
def _done_callback(span, code, details):
    ...
    def func(call):
        # type: (aio.Call) -> None
        try:
            span.set_str_tag(constants.GRPC_STATUS_CODE_KEY, to_unicode(code))
...
def _handle_error(span, call, code, details):
    ...
    span.set_str_tag(ERROR_TYPE, to_unicode(code))
...
def _handle_rpc_error(span, rpc_error):
    ...
    span.error = 1
    span.set_str_tag(constants.GRPC_STATUS_CODE_KEY, code)
    span.set_str_tag(ERROR_MSG, rpc_error.details())
...
async def _handle_cancelled_error(call, span):
    ...
    span.error = 1
    span.set_str_tag(constants.GRPC_STATUS_CODE_KEY, code)
    span.set_str_tag(ERROR_MSG, await call.details())
    span.set_str_tag(ERROR_TYPE, code)
    span.finish()
