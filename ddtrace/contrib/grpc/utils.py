...
def set_grpc_method_meta(span, method, method_kind):
    ...
    if method_path is not None:
        span.set_str_tag(constants.GRPC_METHOD_PATH_KEY, method_path)
    if method_package is not None:
        span.set_str_tag(constants.GRPC_METHOD_PACKAGE_KEY, method_package)
    if method_service is not None:
        span.set_str_tag(constants.GRPC_METHOD_SERVICE_KEY, method_service)
    if method_name is not None:
        span.set_str_tag(constants.GRPC_METHOD_NAME_KEY, method_name)
    if method_kind is not None:
        span.set_str_tag(constants.GRPC_METHOD_KIND_KEY, method_kind)


def set_grpc_client_meta(span, host, port):
    if host:
        span.set_str_tag(constants.GRPC_HOST_KEY, host)
    if port:
        span.set_str_tag(constants.GRPC_PORT_KEY, str(port))
    span.set_str_tag(constants.GRPC_SPAN_KIND_KEY, constants.GRPC_SPAN_KIND_VALUE_CLIENT)
