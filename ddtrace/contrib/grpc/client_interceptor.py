...
def _future_done_callback(span):
    def func(response):
        try:
            ...
            span.set_str_tag(constants.GRPC_STATUS_CODE_KEY, status_code)
...
def _handle_error(span, response_error, status_code):
    ...
    if response_error.cancelled():
        ...
        span.set_str_tag(ERROR_MSG, exc_val)
        span.set_str_tag(ERROR_TYPE, status_code)
        return
    ...
    if exception is not None and traceback is not None:
        ...
        if isinstance(exception, grpc.RpcError):
            ...
            span.set_str_tag(ERROR_MSG, exc_val)
            span.set_str_tag(ERROR_TYPE, status_code)
            span.set_str_tag(ERROR_STACK, stringify(traceback))
        else:
