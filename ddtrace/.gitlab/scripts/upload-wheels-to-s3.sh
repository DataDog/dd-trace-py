...
ddtrace/.gitlab/scripts/generate-index-html.sh | aws s3 cp - "s3://${BUCKET}/${S3_PATH}/${INDEX_FILE}" --content-type text/html
ddtrace/.gitlab/scripts/generate-download-script.sh "${S3_BASE_URL}" "${INDEX_FILE}" | aws s3 cp - "s3://${BUCKET}/${S3_PATH}/${DOWNLOAD_FILE}" --content-type text/x-shellscript
ddtrace/.gitlab/scripts/generate-install-script.sh "${S3_BASE_URL}" "${INDEX_FILE}" | aws s3 cp - "s3://${BUCKET}/${S3_PATH}/${INSTALL_FILE}" --content-type text/x-shellscript
ddtrace/.gitlab/scripts/generate-metadata-txt.sh | aws s3 cp - "s3://${BUCKET}/${S3_PATH}/${METADATA_FILE}" --content-type text/plain
