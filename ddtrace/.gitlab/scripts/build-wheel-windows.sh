...
rm -rf "${PROJECT_DIR}/ddtrace/dist"
...
docker run --rm \
  ...
  powershell -NoProfile -NonInteractive \
    -File 'C:\workspace\ddtrace\.gitlab\scripts\windows-docker-build.ps1' \
    -VcArch "${VC_ARCH}"

export BUILT_WHEEL_FILE
BUILT_WHEEL_FILE=$(ls "${PROJECT_DIR}/ddtrace/dist/"*.whl | head -n 1)
...
docker run --rm \
  ...
  "${IMAGE}" \
  powershell -NoProfile -NonInteractive \
    -File 'C:\workspace\ddtrace\.gitlab\scripts\windows-docker-test.ps1' \
    -UvPython "${UV_PYTHON}"
