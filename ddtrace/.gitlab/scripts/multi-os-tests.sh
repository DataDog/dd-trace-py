...
uv pip install --python $PYTHON_VERSION -r "$CI_PROJECT_DIR/ddtrace/.gitlab/requirements/multi-os-tests.txt" $WHEEL_PATH
...
python -m pytest "$CI_PROJECT_DIR/ddtrace/tests/internal/service_name/test_extra_services_names.py" -v -s
python -m pytest "$CI_PROJECT_DIR/ddtrace/tests/appsec/architectures/test_appsec_loading_modules.py" -v -s
