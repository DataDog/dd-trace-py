...
def upload_coverage_report_if_enabled(
    coverage_report_uploader: t.Optional[CoverageReportUploader],
    coverage_instance: t.Optional[t.Any],
    session_config
) -> bool:
    """
    ...
    log.debug("Starting coverage report upload with uploader: %s, instance: %s", coverage_report_uploader, type(coverage_instance))
    try:
        ...
        if "Telemetry API called before being initialized" in str(e):
            log.debug("Coverage upload succeeded but telemetry failed (expected for old plugin)")
            return True
        else:
    ...
    def setup_coverage_for_session(self, session) -> None:
        ...
        if self._is_cov_enabled:
            ...
            if self.coverage_instance:
                ...
                log.warning("Coverage manager: pytest-cov enabled but instance capture failed")
        else:
...
def upload_coverage_if_available(
    ...
    # Create coverage report uploader if not provided (for old plugin compatibility)
    if coverage_report_uploader is None and coverage_instance is not None:
        try:
            ...
            # For old plugin, we need to create a connector setup directly
            # This is safe because BackendConnectorSetup.detect_setup() just reads environment variables
            connector_setup = BackendConnectorSetup.detect_setup()
            log.debug("Created connector setup for old plugin coverage upload")
            
            workspace_path = get_workspace_path()
            # For old plugin compatibility, use minimal env tags to avoid telemetry issues
            try:
                env_tags = get_env_tags()
            except RuntimeError as e:
                if "Telemetry API called before being initialized" in str(e):
                    log.debug("Telemetry not initialized, using minimal env tags for old plugin")
                    # Use minimal env tags without git information
            ...
            coverage_report_uploader = CoverageReportUploader(
                ...
                env_tags=env_tags,
                skippable_coverage={},  # Old plugin doesn't have ITR coverage data easily accessible
                workspace_path=workspace_path,
            )
            log.debug("Created coverage report uploader for old plugin: %s", coverage_report_uploader)
        except Exception as e:
