...
class TestOptPlugin:
    ...
    def __init__(self, session_manager: SessionManager) -> None:
        # Keep the default test context lightweight: when users only need Test Optimization events,
        # creating a real ddtrace root span for every test is pure per-test overhead. A real ddtrace
        # span is still enabled for log correlation, patch-all/all integrations, or explicit opt-in.
        self.enable_ddtrace_trace_filter = asbool(env.get("_DD_CIVISIBILITY_USE_DDTRACE_CONTEXT", "false"))
