...
class OfflineMode:
    ...
    def __init__(self) -> None:
        ...
        # Whether the manifest was generated by a pytest-xdist controller of ours rather than provided by an external
        # tool (the Bazel rule). Callers that infer things about the environment from manifest mode must check this:
        # a generated manifest says nothing about where the tests are running.
        self.manifest_self_generated: bool = False
        ...
        if manifest_env:
            ...
            if _validate_manifest(resolved):
                ...
                self.manifest_self_generated = os.path.basename(self.test_optimization_dir).startswith(
                    XDIST_MANIFEST_DIR_PREFIX
                )
