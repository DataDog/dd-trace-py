...
if pconfig.tb_backend == "cy":
    from ddtrace.profiling.collector._traceback import _extract_class_name
    from ddtrace.profiling.collector._traceback import pyframe_to_frames
    from ddtrace.profiling.collector._traceback import traceback_to_frames
else:
    import types
    ...
    if t.TYPE_CHECKING:
        from ddtrace.profiling.collector import event
            ...
            try:
                if argname == "self":
                    return object.__getattribute__(
                        type(value), "__name__"
                    )  # use type() and object.__getattribute__ to avoid side-effects
                if argname == "cls":
    ...
    def traceback_to_frames(traceback, max_nframes):
        # type: (types.TracebackType, int) -> t.Tuple[t.List[event.FrameType], int]
        ...
        """
        tb = traceback
        frames = []  # type: t.List[event.FrameType]
        nframes = 0
    ...
    def pyframe_to_frames(frame, max_nframes):
        # type: (types.FrameType, int) -> t.Tuple[t.List[event.FrameType], int]
