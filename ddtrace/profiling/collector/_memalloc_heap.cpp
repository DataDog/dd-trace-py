...
// heap_tracker_t::instance is set in memalloc_heap_tracker_init_no_cpython() and cleared
// before delete in memalloc_heap_tracker_deinit_no_cpython(). Avoid static destructors
// (e.g. file-scope unique_ptr) so the tracker is not torn down during native static
// teardown while allocator hooks may still be installed when memalloc.stop() was skipped.
...
memalloc_heap_tracker_deinit_no_cpython(void)
{
    ...
    // observe nullptr and skip the tracker; then destroy the instance.
