...
// AIDEV-NOTE: Include traceback.h so C++ Traceback type is available
#include "traceback.h"

// AIDEV-NOTE: Include the cxx bridge header for Rust memalloc functions
// This header is generated by the cxx build process and provides C++ bindings to Rust code
// The path structure is: cxxbridge/{crate_name}/{module}.rs.h
#include "ddtrace-native/memalloc.rs.h"
...
memalloc_start(PyObject* Py_UNUSED(module), PyObject* args)
{
    ...
    // AIDEV-NOTE: Test calls to Rust functions via cxx bridge
    // Test 1: Simple function call
    int32_t test_value = 21;
    int32_t result = ddtrace::profiling::rust_multiply_by_two(test_value);
    printf("[memalloc] Rust function test: %d * 2 = %d\n", test_value, result);
    assert(result == 42);
    
    // Test 2: Create a Rust HeapTracker object owned by C++
    auto rust_tracker = ddtrace::profiling::heap_tracker_new((uint32_t)heap_sample_size);
    // Now we can call methods directly on the rust_tracker object!
    uint32_t tracker_sample_size = rust_tracker->get_sample_size();
    printf("[memalloc] Rust HeapTracker created with sample_size: %u\n", tracker_sample_size);
    assert(tracker_sample_size == (uint32_t)heap_sample_size);
    
    // Test 3: Add some traceback frames to the HeapTracker (done from Rust side)
    // Note: Traceback methods are called internally by Rust - we don't call them from C++ here
    // to avoid linking complexities. The Rust HeapTracker owns and manages the Traceback.
    printf("[memalloc] End of test - rust_tracker will be destroyed automatically\n");
    // rust_tracker goes out of scope here and is automatically destroyed (Drop::drop is called)
