...
/* Points to the active heap tracker's per-domain counter array (3 entries,
...
/* Slow path: called only when a domain's byte counter crosses its threshold
 * (~1/1000). Performs the live-allocation cap check and returns true if we
 * should sample. */
bool
memalloc_heap_sample_slow_path();

/* Inline fast-path: bumps the given domain's byte counter and returns true only
 * when that domain's threshold is crossed AND the slow-path cap check passes.
 * ~999/1000 calls return false without crossing a function-call boundary. */
static inline bool
memalloc_heap_sample_check_inline(size_t size, PyMemAllocatorDomain domain, uint64_t* allocated_memory_val)
{
    memalloc_sampling_counter_t* counters = g_memalloc_sampling_counter;
    if (!counters) {
        return false;
    }
    memalloc_sampling_counter_t* counter = &counters[static_cast<int>(domain)];
    ...
    *allocated_memory_val = counter->allocated_memory;
    ...
    return memalloc_heap_sample_slow_path();
}
