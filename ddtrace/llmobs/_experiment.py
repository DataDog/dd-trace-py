...
class ExperimentSummary(_ExperimentSummaryOptional):
    """Summary of an experiment returned by ``LLMObs.list_experiments()``.

    :param id: Unique experiment run UUID.
    :param name: Unique per-run name generated by the backend.
    :param experiment: Logical experiment name shared across all CI runs.
...
class Experiment:
    ...
    def _extract_single_evaluator_result(
        ...
        if isinstance(eval_result, EvaluatorResult):
            ...
            # Fold the judge span's ids into metadata so the score can deep-link to the judge trace.
            # Mirrors LLMObs.submit_evaluation(judge_span=...) for the experiment metric path, which
            # is a separate intake (/experiments/{id}/events) that submit_evaluation never reaches.
