...
class AsyncBaseEvaluator(ABC):
    """Async version of BaseEvaluator for evaluators that need to perform async operations.
    ...
    **Evaluator Return Values**
    ...
    Example::
        ...
        class AsyncSemanticSimilarity(AsyncBaseEvaluator):
            def __init__(self, threshold=0.8):
            ...
            async def evaluate(self, context: EvaluatorContext):
                # Can perform async operations like API calls
                embedding1 = await self.client.embeddings.create(input=context.output_data)
                embedding2 = await self.client.embeddings.create(input=context.expected_output)
    ...
    Note: The ``evaluate`` method may be called concurrently with other evaluators.
    Avoid modifying instance attributes inside ``evaluate()``; use local variables instead.
    """
...
def _is_function_evaluator(evaluator: Any) -> bool:
    ...
    return (
        ...
        and not isinstance(evaluator, AsyncBaseEvaluator)
        and not isinstance(evaluator, BaseSummaryEvaluator)
    )


def _is_async_class_evaluator(evaluator: Any) -> bool:
    """Check if an evaluator is an async class-based evaluator (inherits from AsyncBaseEvaluator).

    :param evaluator: The evaluator to check
    :return: True if it's an async class-based evaluator, False otherwise
    """
    return isinstance(evaluator, AsyncBaseEvaluator)


# Type aliases for cleaner signatures
Task = Callable[[DatasetRecordInputType, Optional[ConfigType]], JSONType]
AsyncTask = Callable[[DatasetRecordInputType, Optional[ConfigType]], Awaitable[JSONType]]

EvaluatorFn = Callable[[DatasetRecordInputType, JSONType, JSONType], Union[JSONType, EvaluatorResult]]
AsyncEvaluatorFn = Callable[[DatasetRecordInputType, JSONType, JSONType], Awaitable[Union[JSONType, EvaluatorResult]]]

SummaryEvaluatorFn = Callable[
    [List[DatasetRecordInputType], List[JSONType], List[JSONType], Dict[str, List[JSONType]]],
    JSONType,
]

Evaluator = Union[EvaluatorFn, BaseEvaluator]
AsyncEvaluator = Union[AsyncEvaluatorFn, AsyncBaseEvaluator]
SummaryEvaluator = Union[SummaryEvaluatorFn, BaseSummaryEvaluator]
...
class _ExperimentRunInfo:
    def __init__(self, run_iteration: int, base_tags: Mapping[str, str]):
        self._id = uuid.uuid4()
...
class Experiment:
    ...
    def run(
        ...
        # These are guaranteed to be set after _setup_experiment()
        assert self._llmobs_instance is not None
    ...
    def _setup_experiment(self) -> None:
        """Setup project and experiment. Called at start of run()."""
    ...
    def _build_evaluator_context(self, record: DatasetRecord, task_result: TaskResult) -> EvaluatorContext:
        """Build EvaluatorContext for a single record."""
        metadata = record.get("metadata", {})
    ...
    def _build_summary_context(
    ...
    ) -> Tuple[SummaryEvaluatorContext, Dict[str, List[JSONType]]]:
        """Build SummaryEvaluatorContext from all results."""
        inputs: List[DatasetRecordInputType] = []
        outputs: List[JSONType] = []
        expected_outputs: List[JSONType] = []
        ...
        return context, eval_results_by_name

    def _process_evaluator_result(self, eval_result: Any) -> Dict[str, JSONType]:
        """Process evaluator result into standard dict format."""
        extra: Dict[str, JSONType] = {}
        if isinstance(eval_result, EvaluatorResult):
            if eval_result.reasoning:
                extra["reasoning"] = eval_result.reasoning
            if eval_result.assessment:
                extra["assessment"] = eval_result.assessment
            if eval_result.metadata:
                extra["metadata"] = eval_result.metadata
            if eval_result.tags:
                extra["tags"] = eval_result.tags
            return {"value": eval_result.value, "error": None, **extra}
        return {"value": eval_result, "error": None}

    def _format_evaluator_error(self, e: Exception) -> Dict[str, JSONType]:
        """Format exception info for evaluator error."""
    ...
    def _create_record_tags(self, run: _ExperimentRunInfo, record: DatasetRecord) -> Dict[str, str]:
        """Create tags dictionary for a record execution."""
        return {
            **run.tags,
            "dataset_id": str(self._dataset._id),
            "dataset_record_id": str(record.get("record_id", "")),
    ...
    def _build_task_result(
        ...
        """Build a TaskResult dictionary from execution results."""
        return {
            "idx": idx,
            "span_id": span_id,
            "trace_id": trace_id,
    ...
    def _build_experiment_result(self, run_results: List[ExperimentRun]) -> ExperimentResult:
        """Build the final ExperimentResult from run results."""
        return {
    ...
    def _process_record(self, idx: int, record: DatasetRecord, run: _ExperimentRunInfo) -> Optional[TaskResult]:
        if not self._llmobs_instance or not self._llmobs_instance.enabled:
            return None
        with self._llmobs_instance._experiment(
            ...
            input_data = record["input_data"]
            tags = self._create_record_tags(run, record)
            output_data = None
            try:
            ...
            for evaluator in self._evaluators:
                ...
                try:
                    if _is_class_evaluator(evaluator):
                        ...
                        context = self._build_evaluator_context(record, task_result)
                    ...
                    row_results[evaluator_name] = self._process_evaluator_result(eval_result)

                except Exception as e:
                    eval_err = self._format_evaluator_error(e)
                    if raise_errors:
                        raise RuntimeError(f"Evaluator {evaluator_name} failed on row {idx}") from e
                    row_results[evaluator_name] = {"value": None, "error": eval_err}
    ...
    def _run_summary_evaluators(
        ...
        context, eval_results_by_name = self._build_summary_context(task_results, eval_results)
        # Extract for function evaluators that need raw args
...
class AsyncExperiment(Experiment):
    """Async version of Experiment for running async tasks and evaluators concurrently.

    AsyncExperiment enables efficient parallel execution of async tasks and evaluators
    using Python's asyncio. Tasks run concurrently with configurable concurrency limits.
    ...
    Example::
        ...
        class AsyncSimilarityEvaluator(AsyncBaseEvaluator):
            async def evaluate(self, context):
                score = await compute_similarity(context.output_data, context.expected_output)
                return score
        ...
        experiment = LLMObs.async_experiment(
            ...
            task=async_task,
            dataset=dataset,
            evaluators=[AsyncSimilarityEvaluator()],
        )
    ...
    def __init__(
        ...
        # Store async-specific attributes with proper types
        self._async_task: AsyncTask = task
        self._async_evaluators: Sequence[AsyncEvaluator] = evaluators

        # Call parent __init__ for shared setup (dataset, config, tags, etc.)
        # Pass dummy task/evaluators since AsyncExperiment uses _async_task/_async_evaluators instead
        super().__init__(
            name=name,
            task=lambda input_data, config: None,
            dataset=dataset,
            evaluators=[],
    ...
    async def run(  # type: ignore[override]
        ...
        evaluator_concurrency: int = 20,
        raise_errors: bool = False,
        sample_size: Optional[int] = None,
    ) -> ExperimentResult:
        """Run the async experiment by executing tasks and evaluators concurrently.

        :param task_concurrency: Maximum number of concurrent task executions (default: 10)
        :param evaluator_concurrency: Maximum number of concurrent evaluator executions (default: 20)
        :param raise_errors: Whether to raise exceptions on task or evaluator errors (default: False)
        :param sample_size: Optional number of dataset records to sample for testing
                            (default: None, uses full dataset)
        :return: ExperimentResult containing evaluation results and metadata
        ...
        if evaluator_concurrency < 1:
            raise ValueError("evaluator_concurrency must be at least 1")
        ...
        async def execute_single_run(run_iteration: int) -> ExperimentRun:
            run = _ExperimentRunInfo(run_iteration, self._tags)
            ...
            # Summary evaluators remain sync (fast aggregations)
            summary_evals = self._run_summary_evaluators(task_results, evaluations, raise_errors, jobs=1)
            run_result = self._merge_results(run, task_results, evaluations, summary_evals)
            experiment_evals = self._generate_metrics_from_exp_results(run_result)
            llmobs_instance._dne_client.experiment_eval_post(
                experiment_id, experiment_evals, convert_tags_dict_to_list(run.tags)
            )
            return run_result

        # Run all iterations concurrently
        run_results = list(await asyncio.gather(*[execute_single_run(i) for i in range(self._runs)]))
    ...
    async def _run_tasks_with_evaluators(
        ...
        evaluator_semaphore = asyncio.Semaphore(evaluator_concurrency)

        async def process_record_with_evaluators(
            idx: int, record: DatasetRecord
        ) -> Tuple[TaskResult, EvaluationResult]:
            async with task_semaphore:
                task_result = await self._async_process_record(idx, record, run)

            if raise_errors:
                err_dict = task_result.get("error") or {}
                if isinstance(err_dict, dict) and err_dict.get("message"):
                    err_msg = err_dict.get("message")
                    err_type = err_dict.get("type")
                    err_stack = err_dict.get("stack")
                    raise RuntimeError("Error on record {}: {}\n{}\n{}".format(idx, err_msg, err_type, err_stack))

            evaluation = await self._async_run_evaluators_for_record(
                idx, record, task_result, evaluator_semaphore, raise_errors
            )

            return task_result, evaluation

        # Run all records concurrently
        tasks = [process_record_with_evaluators(idx, record) for idx, record in enumerate(subset_dataset)]
        results = await asyncio.gather(*tasks)

        # Separate task results and evaluations
        task_results = [r[0] for r in results]
        evaluations = [r[1] for r in results]

        self._llmobs_instance.flush()
        return task_results, evaluations

    async def _async_process_record(self, idx: int, record: DatasetRecord, run: _ExperimentRunInfo) -> TaskResult:
        ...
        if not self._llmobs_instance or not self._llmobs_instance.enabled:
            return self._build_task_result(
                ...
                timestamp=0,
                output_data=None,
                error_msg="LLMObs not enabled",
                error_stack=None,
                error_type=None,
            )

        with self._llmobs_instance._experiment(
            name=self._async_task.__name__,
            experiment_id=self._id,
            run_id=str(run._id),
            run_iteration=run._run_iteration,
            dataset_name=self._dataset.name,
            ...
            output_data = None
            try:
                output_data = await self._async_task(input_data, self._config)
            except Exception:
                span.set_exc_info(*sys.exc_info())
            self._llmobs_instance.annotate(span, input_data=input_data, output_data=output_data, tags=tags)
    ...
    async def _async_run_evaluators_for_record(
        self,
        idx: int,
        ...
        """Run all async evaluators for a single record concurrently.

        Only async evaluators are supported: AsyncBaseEvaluator instances or async functions.
        """
        input_data = record["input_data"]
        output_data = task_result["output"]
        expected_output = record["expected_output"]

        async def run_single_evaluator(evaluator: Any) -> Tuple[str, Dict[str, JSONType]]:
            async with semaphore:
                evaluator_name = ""
                try:
                    if _is_async_class_evaluator(evaluator):
                        # AsyncBaseEvaluator instance
                    ...
                    return evaluator_name, self._process_evaluator_result(eval_result)
        ...
        eval_tasks = [run_single_evaluator(evaluator) for evaluator in self._async_evaluators]
        results = await asyncio.gather(*eval_tasks)

        row_results: Dict[str, Dict[str, JSONType]] = {}
        for evaluator_name, eval_data in results:
            row_results[evaluator_name] = eval_data
