...
def _dedupe_by_id(fns: list) -> list:
...
def _collect_instructions(agent: Any) -> tuple[list[str], list]:
    ...
    if isinstance(instructions, (list, tuple)):
        for entry in instructions:
            if isinstance(entry, str):
                ...
                dynamic_fns.append(entry)
    elif isinstance(instructions, str):
        static_texts.append(instructions)
    elif callable(instructions):
        dynamic_fns.append(instructions)
    for runner in getattr(agent, "_instructions_functions", None) or []:
        fn = getattr(runner, "function", runner)
        if callable(fn):
            dynamic_fns.append(fn)
    return static_texts, _dedupe_by_id(dynamic_fns)


def _collect_system_prompts(agent: Any) -> tuple[list[str], list]:
...
class PydanticAIIntegration(BaseLLMIntegration):
    ...
    def _build_agent_manifest(self, agent: Any) -> dict[str, Any]:
        """Build the RFC-shaped agent manifest dict from a pydantic-ai ``Agent``.

        Split out from ``_tag_agent_manifest`` so the manifest shape is testable without a live run.
        """
        ...
        self._set_manifest_instructions(manifest, agent)
        self._set_manifest_system_prompts(manifest, agent)
        ...
        # Re-parse via json.loads(serialized) rather than aliasing _metadata: the Agent is reused
        # across runs, so sharing the (possibly nested-mutable) dict would let a later mutation rewrite
        # an already-queued span event.
    ...
    @staticmethod
    def _describe_prompt_functions(fns: list) -> list[dict[str, Any]]:
        """Describe each dynamic prompt function as ``{name, signature?, doc?, source?, source_truncated?}``.

        ``inspect.getsource`` returns the full definition INCLUDING the decorator line (e.g.
        ``@agent.instructions``) -- intended, since it shows how the function is wired to the agent.

        AIDEV-NOTE: the captured ``source`` is byte-capped (``_PROMPT_SOURCE_MAX_BYTES``, same rationale as
        the other manifest caps above) but NOT content-redacted: a function body is arbitrary code with no
        allowlistable structure, so there is nothing to scrub the way ``_redact_mcp_uri`` scrubs a URL. This
        is a conscious tradeoff for capturing the definition; a redaction hook would attach here.
        """
    ...
    @classmethod
    def _render_dynamic_prompt_text(cls, described: list[dict[str, Any]]) -> list[str]:
        """Render each described dynamic function to a text blob: its source, else ``name + signature``.

        Lets the primary ``instructions`` / ``system_prompts`` field carry the full definition inline
        (per the owner decision) without losing the structured ``*_functions`` list.
        """
        rendered: list[str] = []
        for entry in described:
            ...
            if source:
                ...
                rendered.append("{}{}".format(entry["name"], entry.get("signature") or ""))
        return rendered

    @classmethod
    def _set_manifest_instructions(cls, manifest: dict[str, Any], agent: Any) -> None:
        """Populate ``instructions`` (+ ``instructions_is_dynamic`` / ``instructions_functions``).

        Emits BOTH shapes (owner decision): the primary ``instructions`` string carries the static text with
        each dynamic function's definition appended, AND ``instructions_functions`` carries the structured
        descriptors. ``instructions`` for a static-only agent is byte-identical to the pre-enrichment output
        (a lone static string, or ``" ".join`` of several).
        """
        if not hasattr(agent, "_instructions"):
            return
        static_texts, dynamic_fns = _collect_instructions(agent)
        described = cls._describe_prompt_functions(dynamic_fns)
        segments = list(static_texts)
        segments.extend(cls._render_dynamic_prompt_text(described))
        manifest["instructions"] = " ".join(segments) if segments else None
        if dynamic_fns:
            manifest["instructions_is_dynamic"] = True
        if described:
            manifest["instructions_functions"] = described

    @classmethod
    def _set_manifest_system_prompts(cls, manifest: dict[str, Any], agent: Any) -> None:
        """Populate ``system_prompts`` (+ ``system_prompts_is_dynamic`` / ``system_prompt_functions``),
        mirroring ``_set_manifest_instructions``; every key is omitted when empty.
        """
        static_texts, dynamic_fns = _collect_system_prompts(agent)
        described = cls._describe_prompt_functions(dynamic_fns)
        system_prompts = list(static_texts)
        system_prompts.extend(cls._render_dynamic_prompt_text(described))
        if system_prompts:
            manifest["system_prompts"] = system_prompts
        if dynamic_fns:
            manifest["system_prompts_is_dynamic"] = True
        if described:
            manifest["system_prompt_functions"] = described

    def _get_agent_tools(self, agent: Any) -> list[dict[str, Any]]:
        """Shape the agent's function tools (from ``_iter_agent_tools``) into ``{name, description?, parameters}``."""
        formatted_tools = []
        for tool_name, tool_instance, _ in _iter_agent_tools(agent):
    ...
    def _get_agent_capabilities(self, agent: Any) -> list[dict[str, Any]]:
        ...
        Plain function tools are excluded -- they already live in ``manifest["tools"]``.
        """
        capabilities: list[dict[str, Any]] = []
        for tool in getattr(agent, "_builtin_tools", None) or []:
            kind = getattr(tool, "kind", None) or type(tool).__name__
            if kind:
                capabilities.append({"kind": "builtin", "name": kind, "provider": "pydantic_ai"})
        capabilities.extend(self._get_mcp_servers(agent))
        capabilities.extend(self._get_sub_agents(agent))
    ...
    def _get_mcp_servers(self, agent: Any) -> list[dict[str, Any]]:
        """List MCP servers as ``{kind: 'mcp', name, uri?}``.
        ...
        for toolset in getattr(agent, "_user_toolsets", None) or []:
            if not isinstance(toolset, mcp_cls):
                continue
            entry: dict[str, Any] = {"kind": "mcp", "name": self._toolset_name(toolset)}
    ...
    def _get_sub_agents(self, agent: Any) -> list[dict[str, Any]]:
        """List function tools delegating to another ``Agent`` as ``{kind: 'sub-agent', name, agent_name?}``.

        Detection caveats are documented on ``_referenced_agent_name``.
        """
        sub_agents: list[dict[str, Any]] = []
        for tool_name, _, fn in _iter_agent_tools(agent):
            if not callable(fn):
                continue
            agent_name = self._referenced_agent_name(fn)
            if agent_name is not None:
                entry: dict[str, Any] = {"kind": "sub-agent", "name": tool_name}
                if agent_name:
    ...
    def _get_custom_toolsets(self, agent: Any) -> list[dict[str, Any]]:
        """List non-MCP, non-function user toolsets as ``{kind: 'custom', name}`` so none is silently dropped."""
        mcp_cls = self._mcp_server_cls()
        ...
        for toolset in getattr(agent, "_user_toolsets", None) or []:
            ...
            if fn_cls is not None and isinstance(toolset, fn_cls):
                continue
            custom.append({"kind": "custom", "name": self._toolset_name(toolset)})
        return custom
    ...
    def _get_agent_handoffs(self, agent: Any) -> list[dict[str, Any]]:
        ...
        Entry ``{tool_name, handoff_description?, agent_name?}``, per
        ``openai_agents._extract_handoffs_from_agent``.
        """
        if not hasattr(agent, "output_type"):
            return []
