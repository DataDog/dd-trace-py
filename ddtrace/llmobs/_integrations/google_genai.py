...
class GoogleGenAIIntegration(BaseLLMIntegration):
    ...
    def _llmobs_set_tags(
        ...
        provider_name, model_name = extract_provider_and_model_name_google_genai(kwargs)
        span._set_ctx_items(
            {
                SPAN_KIND: operation,
    ...
    def _extract_output_messages(self, response) -> List[Dict[str, Any]]:
        if not response:
            return [{"content": "", "role": GENAI_DEFAULT_MODEL_ROLE}]
        messages = []
        ...
        for candidate in candidates:
            ...
            role = _get_attr(content, "role", GENAI_DEFAULT_MODEL_ROLE)
            for part in parts:
