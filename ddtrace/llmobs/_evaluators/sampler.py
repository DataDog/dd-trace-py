...
class EvaluatorRunnerSampler:
    ...
    def parse_rules(self) -> List[EvaluatorRunnerSamplingRule]:
        ...
        def parsing_failed_because(msg: str, maybe_throw_this: Exception):
            if config._raise:
                raise maybe_throw_this
        ...
        try:
            ...
            parsing_failed_because(reason, ValueError(reason))
            return []

        if not isinstance(json_rules, list):
            msg = "Evaluator sampling rules must be a list of dictionaries"
            parsing_failed_because(reason, ValueError(reason))
            return []
        ...
        for rule in json_rules:
            if "sample_rate" not in rule:
                ...
                parsing_failed_because(reason, KeyError(reason))
                continue
