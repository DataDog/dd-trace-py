...
class LLMObs(Service):
    ...
    def annotation_context(
        ...
        :param cost_tags: List of tag keys from `tags` that should also be propagated to LLMObs cost and token
                          ...
                          annotation is applied Since annotation contexts are applied at span start, `cost_tags` should
                          reference tags provided in the same context or already present on the span when it starts.
                          Use only low-cardinality tag keys here; propagation to downstream cost/token metrics depends
                          on backend support for the feature.
    ...
    def annotate(
        ...
        :param cost_tags: List of tag keys from `tags` that should also be propagated to LLMObs cost and token
                          metrics. Each entry must be a tag key already present in the span's tags at the time the
                          annotation is applied. Use only low-cardinality tag keys here; propagation to downstream
                          cost/token metrics depends on backend support for the feature.
