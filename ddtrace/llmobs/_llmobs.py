...
from ddtrace.llmobs._experiment import AsyncEvaluatorFunctionType
...
from ddtrace.llmobs._experiment import EvaluatorFunctionType
from ddtrace.llmobs._experiment import EvaluatorResult
from ddtrace.llmobs._experiment import Experiment
from ddtrace.llmobs._experiment import JSONType
from ddtrace.llmobs._experiment import Project
from ddtrace.llmobs._experiment import SummaryEvaluatorFunctionType
...
class LLMObs(Service):
    ...
    def async_experiment(
        ...
        :param dataset: The dataset to run the experiment on, created with LLMObs.pull/create_dataset().
        :param evaluators: A list of async evaluator functions or BaseAsyncEvaluator instances to evaluate the task
                           output.
                           Function-based evaluators must be async functions accepting parameters ``input_data``,
                           ``output_data``, and ``expected_output``.
                           Class-based evaluators must inherit from BaseAsyncEvaluator and implement the async
                           evaluate method.
        :param project_name: The name of the project to save the experiment to.
        :param description: A description of the experiment.
        :param tags: A dictionary of string key-value tag pairs to associate with the experiment.
        ...
        if not isinstance(dataset, Dataset):
            raise TypeError("Dataset must be an LLMObs Dataset object.")
        if not evaluators or not all(
            asyncio.iscoroutinefunction(evaluator) or isinstance(evaluator, BaseAsyncEvaluator)
            for evaluator in evaluators
        ):
            raise TypeError("Evaluators must be a list of async callable functions or BaseAsyncEvaluator instances.")
        for evaluator in evaluators:
            if isinstance(evaluator, BaseAsyncEvaluator):
                continue
            if not asyncio.iscoroutinefunction(evaluator):
                raise TypeError(
                    f"Evaluator {evaluator} must be an async callable or an instance of BaseAsyncEvaluator."
                )
            sig = inspect.signature(evaluator)
            params = sig.parameters
            evaluator_required_params = ("input_data", "output_data", "expected_output")
