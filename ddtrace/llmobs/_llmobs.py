...
class LLMObs(Service):
    ...
    def annotation_context(
        ...
        :param cost_tags: List of tag keys from `tags` that should also be propagated to cost and token
                          metrics generated by LLM Observability based on LLM/embedding spans inside this
                          context. Each entry must be a tag key supplied via `tags` in this context (or a
                          parent context) — cost_tags are resolved at span start, so tags added later via
                          `annotate()` on an individual span are not retained.
    ...
    def annotate(
        ...
        :param tags: Dictionary of JSON serializable key-value tag pairs to set or update on the LLMObs span
                     regarding the span's context.
        :param cost_tags: List of tag keys from `tags` that should also be propagated to cost and token
                          metrics generated by LLM Observability based on this LLM/embedding span. Each entry
                          must be a tag key supplied via `tags` in this or a prior `annotate`/`annotation_context`
                          call on the same span.
