...
from .max_sample_metric import DistributionMetric
from .max_sample_metric import HistogramMetric
from .max_sample_metric import TimingMetric
from .max_sample_metric_context import MaxSampleMetricContexts
from .metric_types import MetricType
from .metrics import CountMetric
from .metrics import GaugeMetric
from .metrics import SetMetric
...
class Aggregator(object):
    ...
    def count(self, name, value, tags, rate, timestamp=0, cardinality=None):
        return self.add_metric(MetricType.COUNT, CountMetric, name, value, tags, rate, timestamp, cardinality)

    def gauge(self, name, value, tags, rate, timestamp=0, cardinality=None):
        return self.add_metric(MetricType.GAUGE, GaugeMetric, name, value, tags, rate, timestamp, cardinality)

    def set(self, name, value, tags, rate, timestamp=0, cardinality=None):
        return self.add_metric(MetricType.SET, SetMetric, name, value, tags, rate, timestamp, cardinality)

    def add_metric(self, metric_type, metric_class, name, value, tags, rate, timestamp=0, cardinality=None):
        ...
        with self._locks[metric_type]:
            if context in self.metrics_map[metric_type]:
                ...
                self.metrics_map[metric_type][context] = metric_class(name, value, tags, rate, timestamp, cardinality)

    def histogram(self, name, value, tags, rate, cardinality=None):
        return self.add_max_sample_metric(MetricType.HISTOGRAM, name, value, tags, rate, cardinality)

    def distribution(self, name, value, tags, rate, cardinality=None):
        return self.add_max_sample_metric(MetricType.DISTRIBUTION, name, value, tags, rate, cardinality)

    def timing(self, name, value, tags, rate, cardinality=None):
        return self.add_max_sample_metric(MetricType.TIMING, name, value, tags, rate, cardinality)

    def add_max_sample_metric(self, metric_type, name, value, tags, rate, cardinality=None):
        if rate is None:
            rate = 1
