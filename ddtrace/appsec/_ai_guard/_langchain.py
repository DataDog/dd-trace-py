            ...
            if isinstance(message, HumanMessage):
                ...
                if len(message.tool_calls) > 0:
                    tool_calls = [
                        ToolCall(
                            id=call.get("id", ""),
                            function=Function(name=call.get("name", ""), arguments=json.dumps(call.get("args", {}))),
                        )
                    ...
                    for intermediate_step, output in intermediate_steps:
                        if isinstance(intermediate_step, AgentActionMessageLog):
                            ...
                            tool_call = ToolCall(
                                id=tool_call_id,
                                function=Function(
                                    name=intermediate_step.tool, arguments=json.dumps(intermediate_step.tool_input)
                                ),
                            )
                ...
                messages.append(
                    Message(
                        ...
                        tool_calls=[
                            ToolCall(
                                id="",
                                function=Function(name=action.tool, arguments=json.dumps(action.tool_input)),
                            )
                        ],
                    )
                )
...
def _evaluate_langchain_prompt(client: AIGuardClient, prompt):
    try:
        client.evaluate([Message(role="user", content=prompt)], Options(block=True))
    except AIGuardAbortError as e:
        return e
    except Exception:
