...
class AppSecSpanProcessor(SpanProcessor):
    ...
    def on_span_finish(self, span):
        ...
        span.set_str_tag(RUNTIME_FAMILY, "python")
        ...
        try:
            ...
            if info["errors"]:
                span.set_str_tag(APPSEC_EVENT_RULE_ERRORS, json.dumps(info["errors"]))
            span.set_str_tag(APPSEC_EVENT_RULE_VERSION, info["version"])
            span.set_str_tag(APPSEC_WAF_VERSION, "%s.%s.%s" % version())
        ...
        if res is not None:
            ...
            span.set_str_tag("appsec.event", "true")
            span.set_str_tag(APPSEC_JSON, '{"triggers":%s}' % (res,))
            # Right now, we overwrite any value that could be already there. We need to reconsider when ASM/AppSec's
            # specs are updated.
            span.set_tag(MANUAL_KEEP_KEY)
            if span.get_tag(ORIGIN_KEY) is None:
                span.set_str_tag(ORIGIN_KEY, "appsec")
