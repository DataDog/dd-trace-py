...
if(NOT Python_FOUND AND NOT Python3_FOUND)
    ...
    find_package(Python 3.9 REQUIRED COMPONENTS ${_pybind11_interp_component} ${_pybind11_dev_component}
                                                ${_pybind11_quiet} ${_pybind11_global_keyword})

    # If we are in submodule mode, export the Python targets to global targets. If this behavior is not desired,
    # FindPython _before_ pybind11.
    if(NOT is_config
...
if(NOT _PYBIND11_CROSSCOMPILING AND DEFINED ${_Python}_EXECUTABLE)
    if(DEFINED PYBIND11_PYTHON_EXECUTABLE_LAST AND NOT ${_Python}_EXECUTABLE STREQUAL PYBIND11_PYTHON_EXECUTABLE_LAST)
    ...
    if(NOT DEFINED PYTHON_IS_DEBUG)
        ...
        execute_process(COMMAND "${${_Python}_EXECUTABLE}" "-c" "import sys; sys.exit(hasattr(sys, 'gettotalrefcount'))"
    ...
    # Get the suffix - SO is deprecated, should use EXT_SUFFIX, but this is required for PyPy3 (as of 7.3.1)
...
if(DEFINED ${_Python}_INCLUDE_DIRS)
    ...
    #
    # This needs to be a target to be included after the local pybind11 directory, just in case there there is an
    # installed pybind11 sitting next to Python's includes. It also ensures Python is a SYSTEM library.
...
if(TARGET ${_Python}::Module)
    # On Android, older versions of CMake don't know that modules need to link against libpython, so Python::Module will
    # be an INTERFACE target with no associated library files.
...
# WITHOUT_SOABI and WITH_SOABI will disable the custom extension handling used by pybind11. WITH_SOABI is passed on to
# python_add_library.
function(pybind11_add_module target_name)
    cmake_parse_arguments(PARSE_ARGV 1 ARG "STATIC;SHARED;MODULE;THIN_LTO;OPT_SIZE;NO_EXTRAS;WITHOUT_SOABI" "" "")
