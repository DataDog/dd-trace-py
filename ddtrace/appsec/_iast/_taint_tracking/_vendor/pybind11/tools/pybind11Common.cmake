...
# If we are in subdirectory mode, all IMPORTED targets must be GLOBAL. If we are in CONFIG mode, they should be "normal"
# targets instead. In CMake 3.11+ you can promote a target to global after you create it, which might be simpler than
# this check.
get_property(
    is_config
...
# If not run in Python mode, we still would like this to at least include pybind11's include directory:
set(pybind11_INCLUDE_DIRS
    "${pybind11_INCLUDE_DIR}"
    CACHE INTERNAL "Include directory for pybind11 (Python not requested)")

# CMP0190 prohibits calling FindPython with both Interpreter and Development components when cross-compiling, unless the
# CMAKE_CROSSCOMPILING_EMULATOR variable is defined. Default PYBIND11_USE_CROSSCOMPILING to ON in that case, but never
# override a value the project set explicitly (e.g. Emscripten/Pyodide defines an emulator yet still wants it ON).
# PYBIND11_USE_CROSSCOMPILING is undefined for find_package() consumers, but our own CMakeLists.txt always defines it
# via option(); _PYBIND11_USE_CROSSCOMPILING_DEFAULTED tells us whether that came from the project or from the option()
# default.
...
# _pybind11_no_exceptions is a private mechanism to disable this addition. Please open an issue if you need to use it;
# it will be removed if no one needs it.
...
if(MSVC) # That's also clang-cl
    ...
    if("${CMAKE_CXX_COMPILER_ID}" STREQUAL "MSVC") # no Clang no Intel
        # Only set these options for C++ files.  This is important so that, for instance, projects that include other
        # types of source files like CUDA .cu files don't get these options propagated to nvcc since that would cause
        # the build to fail.
        set_property(
            TARGET pybind11::windows_extras
            APPEND
...
set_property(
    ...
    PROPERTY INTERFACE_COMPILE_OPTIONS $<$<CONFIG:Release>:${PYBIND11_OPT_SIZE}>
             $<$<CONFIG:MinSizeRel>:${PYBIND11_OPT_SIZE}> $<$<CONFIG:RelWithDebInfo>:${PYBIND11_OPT_SIZE}>)
...
if(PYBIND11_NOPYTHON)
    ...
    # We won't use new FindPython if PYBIND11_FINDPYTHON is defined and falselike Otherwise, we use if FindPythonLibs is
    # missing or if FindPython was already used
elseif(
    ...
    if(Python_FOUND OR Python3_FOUND)
        ...
        if(PYBIND11_FINDPYTHON STREQUAL "COMPAT")
            message("Using compatibility mode for Python, set PYBIND11_FINDPYTHON to NEW/OLD to silence this message")
...
if(NOT _pybind11_nopython AND NOT _PYBIND11_CROSSCOMPILING)
    # Check to see if modules are importable. Use REQUIRED to force an error if one of the modules is not found.
    # <package_name>_FOUND will be set if the package was found (underscores replace dashes if present). QUIET will hide
    # the found message, and VERSION will require a minimum version. A successful find will cache the result.
...
# Checks whether the given CXX/linker flags can compile and link a cxx file. cxxflags and linkerflags are lists of flags
# to use.  The result variable is a unique variable name for each set of flags: the compilation result will be cached
# base on the result variable.  If the flags work, sets them in cxxflags_out/linkerflags_out internal cache variables
...
function(_pybind11_generate_lto target prefer_thin_lto)
    ...
    if(CMAKE_CXX_COMPILER_ID MATCHES "GNU|Clang")
        ...
        elseif(CMAKE_CXX_COMPILER_ID MATCHES "Clang")
            _pybind11_return_if_cxx_and_linker_flags_work(
                HAS_FLTO_THIN "-flto${thin}${cxx_append}" "-flto${thin}${linker_append}" PYBIND11_LTO_CXX_FLAGS
                PYBIND11_LTO_LINKER_FLAGS)
        endif()
        if(NOT HAS_FLTO_THIN)
            _pybind11_return_if_cxx_and_linker_flags_work(
                HAS_FLTO_AUTO "-flto=auto${cxx_append}" "-flto=auto${linker_append}" PYBIND11_LTO_CXX_FLAGS
                PYBIND11_LTO_LINKER_FLAGS)
        endif()
        if(NOT HAS_FLTO_AUTO)
            _pybind11_return_if_cxx_and_linker_flags_work(HAS_FLTO "-flto${cxx_append}" "-flto${linker_append}"
                                                          PYBIND11_LTO_CXX_FLAGS PYBIND11_LTO_LINKER_FLAGS)
        endif()
    elseif(CMAKE_CXX_COMPILER_ID MATCHES "IntelLLVM")
        # IntelLLVM equivalent to LTO is called IPO; also IntelLLVM is WIN32/UNIX WARNING/HELP WANTED: This block of
        # code is currently not covered by pybind11 GitHub Actions!
        if(WIN32)
            _pybind11_return_if_cxx_and_linker_flags_work(HAS_INTEL_IPO "-Qipo" "-Qipo" PYBIND11_LTO_CXX_FLAGS
                                                          PYBIND11_LTO_LINKER_FLAGS)
        else()
            _pybind11_return_if_cxx_and_linker_flags_work(HAS_INTEL_IPO "-ipo" "-ipo" PYBIND11_LTO_CXX_FLAGS
                                                          PYBIND11_LTO_LINKER_FLAGS)
        endif()
    ...
    elseif(MSVC)
        ...
        # non-standard:
        _pybind11_return_if_cxx_and_linker_flags_work(HAS_MSVC_GL_LTCG "/GL" "-LTCG" PYBIND11_LTO_CXX_FLAGS
                                                      PYBIND11_LTO_LINKER_FLAGS)
    endif()
