...
function(pybind11_guess_python_module_extension python)
    ...
    if(NOT DEFINED PYTHON_MODULE_EXT_SUFFIX AND DEFINED ENV{SETUPTOOLS_EXT_SUFFIX})
        message(STATUS "Getting Python extension suffix from ENV{SETUPTOOLS_EXT_SUFFIX}: $ENV{SETUPTOOLS_EXT_SUFFIX}")
        set(PYTHON_MODULE_EXT_SUFFIX
            "$ENV{SETUPTOOLS_EXT_SUFFIX}"
            CACHE STRING "Extension suffix for Python extension modules (Initialized from SETUPTOOLS_EXT_SUFFIX)")
    endif()
    ...
    if(NOT DEFINED PYTHON_MODULE_EXT_SUFFIX AND DEFINED ${python}_SOABI)
        message(STATUS "Determining Python extension suffix based on ${python}_SOABI: ${${python}_SOABI}")
        # If the SOABI already has an extension, use it as the full suffix (used for debug versions of Python on
        # Windows)
        if(${python}_SOABI MATCHES "\\.")
            set(PYTHON_MODULE_EXT_SUFFIX "${${python}_SOABI}")
            # If the SOABI is empty, this is usually a bug, but we generate a correct extension anyway, which is the
            # best we can do
        elseif("${${python}_SOABI}" STREQUAL "")
            message(
                WARNING "${python}_SOABI is defined but empty. You may want to set PYTHON_MODULE_EXT_SUFFIX explicitly."
            )
    ...
    # Sanity checks:
    if(${python}_SOABI AND NOT (PYTHON_MODULE_EXT_SUFFIX STREQUAL ${python}_SOABI OR PYTHON_MODULE_EXT_SUFFIX MATCHES
                                                                                     "\\.${${python}_SOABI}\\."))
        message(
            WARNING
    ...
    if(NOT DEFINED _PYTHON_IS_DEBUG)
        if(_PYTHON_MODULE_EXTENSION MATCHES "^\\.(cpython-|cp|pypy)[0-9]+dm?-" OR _PYTHON_MODULE_DEBUG_POSTFIX MATCHES
                                                                                  "^_d")
