...
from .._taint_tracking import TagMappingMode, taint_pyobject, _taint_pyobject_base, ranges_to_iterable_with_strings, \
    ranges_to_string
...
def re_findall_aspect(
    ...
    if len(args) >= 1:
        ...
        if ranges:
            result = ranges_to_iterable_with_strings(result, ranges)
...
def re_groups_aspect(orig_function: Optional[Callable], flag_added_args: int, *args: Any, **kwargs: Any) -> Any:
    ...
    return ranges_to_iterable_with_strings(result, get_ranges(self))
...
def re_group_aspect(orig_function: Optional[Callable], flag_added_args: int, *args: Any, **kwargs: Any) -> Any:
    ...
    if isinstance(result, tuple):
        result = ranges_to_iterable_with_strings(result, get_ranges(self))
    else:
        result = ranges_to_string(result, get_ranges(self))
...
def re_expand_aspect(orig_function: Optional[Callable], flag_added_args: int, *args: Any, **kwargs: Any) -> Any:
    ...
    if is_pyobject_tainted(self):
        ...
        result = ranges_to_string(result, get_ranges(args[0]))
