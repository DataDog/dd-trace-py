...
class AppSecIastSpanProcessor(SpanProcessor):
    ...
    def on_span_start(self, span):
        ...
        if span.span_type != SpanTypes.WEB:
            log.warning("IAST>> [%s] Span type is not web, skipping processing", span.span_id)
            return

        if not _is_iast_enabled():
            log.warning("IAST>> [%s] IAST not enabled, skipping processing", span.span_id)
            return
        ...
        if oce.acquire_request(span):
            ...
            create_context()
            log.warning("IAST>> [%s] Span is going to be analyzed", span.span_id)
        else:
            log.warning("IAST>> [%s] Span is NOT going to be analyzed", span.span_id)
