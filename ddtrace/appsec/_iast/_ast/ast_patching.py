...
def astpatch_module(module: ModuleType) -> Tuple[str, Optional[ast.Module]]:
    ...
    if module_origin is None:
        iast_compiling_debug_log("could not find the module: %s" % module_name)
        return "", None
    ...
    try:
        if module_origin.stat().st_size == 0:
            ...
            iast_compiling_debug_log("empty file: %s" % module_path)
            return "", None
    except OSError:
        iast_compiling_debug_log("could not find the file: %s" % module_path, exc_info=True)
        return "", None
    ...
    if module_ext.lower() not in {".pyo", ".pyc", ".pyw", ".py"}:
        ...
        iast_compiling_debug_log("Extension not supported: %s for: %s" % (module_ext, module_path))
        return "", None
    ...
    with open(module_path, "rb") as source_file:
        try:
            ...
            iast_compiling_debug_log("Encode decode error for file: %s" % module_path, exc_info=True)
            return "", None
        except Exception:
            iast_compiling_debug_log("Unexpected read error: %s" % module_path, exc_info=True)
            return "", None
    ...
    if new_ast is None:
        iast_compiling_debug_log("file not ast patched: %s" % module_path)
        return "", None
