...
class AppSecHttpxRequestContextSubscriber(ContextSubscriber[HttpClientRequestEvent]):
    ...
    def on_ended(
        ...
        if ctx.get_item(APPSEC_SSRF_ANALYZE_BODY_KEY):
            load_body = event.response_body_json
            if load_body is not None:
                with contextlib.suppress(Exception):
                    addresses["DOWN_RES_BODY"] = load_body()
...
class AppSecHttpxSingleRequestContextSubscriber(ContextSubscriber[HttpClientSingleRequestEvent]):
    event_names = (HttpClientEvents.HTTPX_SINGLE_REQUEST.value,)

    @classmethod
    def on_started(cls, ctx: core.ExecutionContext[HttpClientSingleRequestEvent]):
        if not _get_rasp_capability("ssrf"):
            return
    ...
    def on_ended(
        cls,
        ctx: core.ExecutionContext[HttpClientSingleRequestEvent],
        exc_info: tuple[Optional[type], Optional[BaseException], Optional[TracebackType]],
    ):
        exc_type, _, _ = exc_info
        if exc_type is not None:
