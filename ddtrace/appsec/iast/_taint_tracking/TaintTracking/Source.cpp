...
// will re-hash it!
uint Source::get_hash() const {
  return std::hash<size_t>()(std::hash<string>()(name) ^ (long)origin ^
                             std::hash<string>()(value));
};
...
void pyexport_source(py::module &m) {
  ...
  py::class_<Source>(m, "Source")
      .def(py::init<string, string, const OriginType>(), "name"_a = "",
           "value"_a = "", "origin"_a = OriginType())
      ...
      .def("__hash__",
           [](const Source &self) {
             return hash<string>{}(self.name + self.value) *
                    (33 + int(self.origin));
           })
