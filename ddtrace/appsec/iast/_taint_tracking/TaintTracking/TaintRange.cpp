...
is_notinterned_notfasttainted_unicode(const PyObject *objptr) {
  ...
  const PyASCIIObject_State_Hidden *e =
      (PyASCIIObject_State_Hidden *)&(((PyASCIIObject *)objptr)->state);
  if (!e) {
...
set_fast_tainted_if_notinterned_unicode(const PyObject *objptr) {
  if (not objptr or !PyUnicode_Check(objptr) or
      PyUnicode_CHECK_INTERNED(objptr)) {
    return;
  }
...
TaintRange::TaintRange(int start, int length, const Source &source)
  ...
  this->source = initializer->allocate_taint_source(source.name, source.value,
                                                    source.origin);
}
...
TaintRangePtr api_shift_taint_range(const TaintRangePtr &source_taint_range,
                                    int offset) {
  auto tptr = initializer->allocate_taint_range(
      source_taint_range->start + offset, // start
      source_taint_range->length,         // length
      source_taint_range->source);        // origin
  return tptr;
}

TaintRangeRefs api_shift_taint_ranges(const TaintRangeRefs &source_taint_ranges,
                                      long offset) {
...
are_all_text_all_ranges(const PyObject *candidate_text,
  ...
  all_ranges.insert(all_ranges.end(), candidate_text_ranges.begin(),
                    candidate_text_ranges.end());
...
void pyexport_taintrange(py::module &m) {
  m.def("is_notinterned_notfasttainted_unicode",
        ...
        "candidate_text"_a);
  m.def("is_notinterned_notfasttainted_unicode",
        &api_is_unicode_and_not_fast_tainted, "candidate_text"_a);
  ...
  m.def("are_all_text_all_ranges", &api_are_all_text_all_ranges,
        "candidate_text"_a, "parameter_list"_a, py::return_value_policy::move);

  // TODO: check return value policy
  m.def("get_tainted_object", &get_tainted_object, "str"_a, "tx_taint_map"_a);

  m.def("shift_taint_range", &api_shift_taint_range,
        py::return_value_policy::move, "source_taint_range"_a, "offset"_a);
  m.def("shift_taint_ranges", &api_shift_taint_ranges,
        py::return_value_policy::move, "ranges"_a, "offset"_a);

  m.def(
      "set_ranges",
      py::overload_cast<const PyObject *, const TaintRangeRefs &>(&set_ranges),
      "str"_a, "ranges"_a);
  m.def("set_ranges", &api_set_ranges, "str"_a, "ranges"_a);

  m.def("get_ranges", py::overload_cast<const PyObject *>(&get_ranges),
        "string_input"_a, py::return_value_policy::take_ownership);
  m.def("get_ranges", &api_get_ranges, "string_input"_a,
        py::return_value_policy::take_ownership);

  m.def("get_range_by_hash", &get_range_by_hash, "range_hash"_a,
        "taint_ranges"_a);
