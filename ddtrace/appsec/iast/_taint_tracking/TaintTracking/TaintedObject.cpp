...
TaintRangePtr shift_taint_range(const TaintRangePtr &source_taint_range,
                                long offset) {
...
void TaintedObject::add_ranges_shifted(TaintedObjectPtr tainted_object,
                                       long offset) {
  const auto &ranges = tainted_object->get_ranges();
  const auto to_add =
      (long)min(ranges.size(), TAINT_RANGE_LIMIT - ranges_.size());
...
void TaintedObject::release() {
  // If rc_ is negative, there is a bug. We check it with an assert (let release
  // builds to tolerate it).
  assert(rc_ == 0);
