...
using is_pyobj_ptr_or_nullptr_t =
  detail::any_of<std::is_same<T, PyObject*>, std::is_same<T, PyObject* const>, std::is_same<T, std::nullptr_t>>;
    ...
    std::string format_value_and_trace() const
    {
        ...
        if (m_value) {
            ...
            if (!value_str) {
                ...
                auto value_bytes =
                  reinterpret_steal<object>(PyUnicode_AsEncodedString(value_str.ptr(), "utf-8", "backslashreplace"));
                if (!value_bytes) {
                    message_error_string = detail::error_string();
                    result = message_unavailable_exc;
                } else {
...
class PYBIND11_EXPORT_EXCEPTION error_already_set : public std::exception
{
  public:
    ...
    error_already_set()
      : m_fetched_error{ new detail::error_fetch_and_normalize("pybind11::error_already_set"), m_fetched_error_deleter }
    {
    }
...
inline void
raise_from(PyObject* type, const char* message)
{
...
inline bool
isinstance(handle obj, handle type)
{
...
/// @{
inline bool
hasattr(handle obj, handle name)
{
...
inline void
delattr(handle obj, handle name)
{
...
inline object
getattr(handle obj, handle name)
{
...
inline object
getattr(handle obj, const char* name)
{
...
inline object
getattr(handle obj, handle name, handle default_)
{
...
inline object
getattr(handle obj, const char* name, handle default_)
{
...
inline void
setattr(handle obj, handle name, handle value)
{
...
inline void
setattr(handle obj, const char* name, handle value)
{
...
inline PyObject*
dict_getitemstring(PyObject* v, const char* key)
{
...
inline PyObject*
dict_getitem(PyObject* v, PyObject* key)
{
...
auto
object_or_cast(T&& o) -> decltype(std::forward<T>(o))
{
...
class list : public object
{
  public:
    ...
    {
        if (PyList_Insert(m_ptr, ssize_t_cast(index), detail::object_or_cast(std::forward<ValType>(val)).ptr()) != 0) {
            throw error_already_set();
        }
    }
};
...
/// @cond DUPLICATE
inline memoryview
memoryview::from_buffer(void* ptr,
                        ssize_t itemsize,
                        const char* format,
                        detail::any_container<ssize_t> shape,
                        detail::any_container<ssize_t> strides,
...
template<typename D>
iterator
object_api<D>::begin() const
{
    return iter(derived());
}
template<typename D>
iterator
object_api<D>::end() const
{
    return iterator::sentinel();
}
template<typename D>
item_accessor
object_api<D>::operator[](handle key) const
{
    return { derived(), reinterpret_borrow<object>(key) };
}
template<typename D>
item_accessor
object_api<D>::operator[](object&& key) const
{
    return { derived(), std::move(key) };
}
template<typename D>
item_accessor
object_api<D>::operator[](const char* key) const
{
    return { derived(), pybind11::str(key) };
}
template<typename D>
obj_attr_accessor
object_api<D>::attr(handle key) const
{
    return { derived(), reinterpret_borrow<object>(key) };
}
template<typename D>
obj_attr_accessor
object_api<D>::attr(object&& key) const
{
    return { derived(), std::move(key) };
}
template<typename D>
str_attr_accessor
object_api<D>::attr(const char* key) const
{
    return { derived(), key };
}
template<typename D>
args_proxy
object_api<D>::operator*() const
{
...
template<typename T>
bool
object_api<D>::contains(T&& item) const
{
...
template<typename D>
bool
object_api<D>::rich_compare(object_api const& other, int value) const
{
