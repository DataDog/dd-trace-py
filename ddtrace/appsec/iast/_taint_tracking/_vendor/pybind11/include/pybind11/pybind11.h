  ...
  protected:
    ...
    void initialize(Func&& f, Return (*)(Args...), const Extra&... extra)
    {
        ...
        rec->impl = [](function_call& call) -> handle {
            ...
            const auto* data = (sizeof(capture) <= sizeof(call.func.data) ? &call.func.data : call.func.data[0]);
        ...
        rec->nargs_pos = cast_in::args_pos >= 0 ? static_cast<std::uint16_t>(cast_in::args_pos)
        ...
        for (const auto* pc = text; *pc != '\0'; ++pc) {
            ...
            if (c == '{') {
                ...
                if (auto* tinfo = detail::get_type_info(*t)) {
                    ...
                    signature +=
                      th.attr("__module__").cast<std::string>() + "." + th.attr("__qualname__").cast<std::string>();
...
class module_ : public object
{
  public:
    ...
    static module_ create_extension_module(const char* name, const char* doc, module_def* def)
    {
        ...
        def = new (def) PyModuleDef{ /* m_base */ PyModuleDef_HEAD_INIT,
                                     /* m_name */ name,
...
void
set_operator_new(type_record* r)
{
...
void
call_operator_delete(T* p, size_t, size_t)
{
    T::operator delete(p);
}
template<typename T, enable_if_t<!has_operator_delete<T>::value && has_operator_delete_size<T>::value, int> = 0>
void
call_operator_delete(T* p, size_t s, size_t)
{
    T::operator delete(p, s);
}

inline void
call_operator_delete(void* p, size_t s, size_t a)
{
    (void)s;
    (void)a;
...
auto
method_adaptor(F&& f) -> decltype(std::forward<F>(f))
{
...
auto
method_adaptor(Return (Class::*pmf)(Args...)) -> Return (Derived::*)(Args...)
{
...
template<typename Derived, typename Return, typename Class, typename... Args>
auto
method_adaptor(Return (Class::*pmf)(Args...) const) -> Return (Derived::*)(Args...) const
{
    static_assert(detail::is_accessible_base_of<Class, Derived>::value,
                  "Cannot bind an inaccessible base class method; use a lambda definition instead");
    return pmf;
...
class class_ : public detail::generic_type
{
  ...
  public:
    ...
    class_(handle scope, const char* name, const Extra&... extra)
    {
        ...
        if (has_alias) {
            auto& instances =
              record.module_local ? get_local_internals().registered_types_cpp : get_internals().registered_types_cpp;
    ...
    {
        rec.add_base(typeid(Base), [](void* src) -> void* { return static_cast<Base*>(reinterpret_cast<type*>(src)); });
    }
...
detail::initimpl::constructor<Args...>
init()
{
    return {};
}
...
template<typename... Args>
detail::initimpl::alias_constructor<Args...>
init_alias()
{
    return {};
}
        ...
        if (is_convertible) {
            ...
            if (is_arithmetic) {
                ...
                m_base.attr("__invert__") =
                  cpp_function([](const object& arg) { return ~(int_(arg)); }, name("__invert__"), is_method(m_base));
            }
        } else {
...
#undef PYBIND11_ENUM_OP_STRICT
    ...
    PYBIND11_NOINLINE void value(char const* name_, object value, const char* doc = nullptr)
    {
        ...
        if (entries.contains(name)) {
            ...
            throw value_error(std::move(type_name) + ": element \"" + std::string(name_) + "\" already exists!");
        }
...
class enum_ : public class_<Type>
{
  public:
    ...
    using Scalar =
      detail::conditional_t<detail::any_of<detail::is_std_char_type<Underlying>, std::is_same<Underlying, bool>>::value,
...
keep_alive_impl(handle nurse, handle patient)
{
    if (!nurse || !patient) {
...
template<return_value_policy Policy = return_value_policy::reference_internal, typename Type, typename... Extra>
iterator
make_value_iterator(Type& value, Extra&&... extra)
{
...
PYBIND11_NAMESPACE_BEGIN(detail)
PYBIND11_NOINLINE void
print(const tuple& args, const dict& kwargs)
{
...
inline const char*
error_already_set::what() const noexcept
{
    gil_scoped_acquire gil;
    error_scope scope;
...
template<class T>
function
get_override(const T* this_ptr, const char* name)
{
...
template<class T>
inline function
get_overload(const T* this_ptr, const char* name)
{
