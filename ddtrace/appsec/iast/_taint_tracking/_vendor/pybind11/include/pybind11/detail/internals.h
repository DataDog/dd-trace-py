...
inline PyObject*
make_object_base_type(PyTypeObject* metaclass);
...
// then set again.
inline void
tls_replace_value(PYBIND11_TLS_KEY_REF key, void* value)
{
...
bool
handle_nested_exception(const T& exc, const std::exception_ptr& p)
{
...
inline bool
raise_err(PyObject* exc_type, const char* msg)
{
    if (PyErr_Occurred()) {
...
inline bool
is_function_record_capsule(const capsule& cap)
{
...
get_shared_data(const std::string& name)
{
...
PYBIND11_NOINLINE void*
set_shared_data(const std::string& name, void* data)
{
...
template<typename T>
T&
get_or_create_shared_data(const std::string& name)
{
    auto& internals = detail::get_internals();
    auto it = internals.shared_data.find(name);
