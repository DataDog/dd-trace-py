...
load_numpy_internals(numpy_internals*& ptr)
{
...
struct npy_api
{
    enum constants
    {
        ...
        NPY_UINT32_ = platform_lookup<std::uint32_t, unsigned long, unsigned int, unsigned short>(NPY_ULONG_,
                                                                                                  NPY_UINT_,
                                                                                                  NPY_USHORT_),
        NPY_INT64_ = platform_lookup<std::int64_t, long, long long, int>(NPY_LONG_, NPY_LONGLONG_, NPY_INT_),
        NPY_UINT64_ = platform_lookup<std::uint64_t, unsigned long, unsigned long long, unsigned int>(NPY_ULONG_,
                                                                                                      NPY_ULONGLONG_,
                                                                                                      NPY_UINT_),
    };
...
struct array_info<std::array<T, N>>
{
    ...
    static constexpr auto extents =
      const_name<array_info<T>::is_array>(concat(const_name<N>(), array_info<T>::extents), const_name<N>());
};
...
ssize_t
byte_offset_unsafe(const Strides&)
{
    return 0;
}
template<ssize_t Dim = 0, typename Strides, typename... Ix>
ssize_t
byte_offset_unsafe(const Strides& strides, ssize_t i, Ix... index)
{
  ...
  private:
    ...
    dtype strip_padding(ssize_t itemsize)
    {
        ...
        for (auto field : field_dict.attr("items")()) {
            ...
            field_descriptors.emplace_back(std::move(name), format.strip_padding(format.itemsize()), std::move(offset));
        }
...
{
    static constexpr auto name = const_name < std::is_same<T, float>::value || std::is_same<T, const float>::value ||
...
struct npy_format_descriptor_name<T, enable_if_t<is_complex<T>::value>>
{
    static constexpr auto name = const_name < std::is_same<typename T::value_type, float>::value ||
                                 ...
                                 std::is_same<typename T::value_type, const double>::value >
                                   (const_name("numpy.complex") + const_name<sizeof(typename T::value_type) * 16>(),
  ...
  : npy_format_descriptor_name<T>
{
  private:
    ...
    constexpr static const int values[15] = { npy_api::NPY_BOOL_,   npy_api::NPY_BYTE_,    npy_api::NPY_UBYTE_,
...
#define PYBIND11_MAP2_LIST_NEXT1(test, next) PYBIND11_EVAL0(PYBIND11_MAP_NEXT0(test, PYBIND11_MAP_COMMA next, 0))
#else
...
template<
  ...
  typename Class,
  typename... Args,
  typename Helper = detail::vectorize_helper<decltype(std::mem_fn(std::declval<Return (Class::*)(Args...) const>())),
                                             Return,
                                             const Class*,
                                             Args...>>
Helper
vectorize(Return (Class::*f)(Args...) const)
{
    return Helper(std::mem_fn(f));
}
