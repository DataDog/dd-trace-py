...
#define PYBIND11_TYPE_CASTER(type, py_name)                                                                            \
  ...
  public:                                                                                                              \
    ...
    template<typename T_,                                                                                              \
             ::pybind11::detail::enable_if_t<std::is_same<type, ::pybind11::detail::remove_cv_t<T_>>::value, int> = 0> \
...
template<typename T>
detail::enable_if_t<!detail::move_never<T>::value, T>
move(object&& obj)
{
    if (obj.ref_count() > 1) {
#if !defined(PYBIND11_DETAILED_ERROR_MESSAGES)
...
template<typename T>
detail::enable_if_t<detail::is_pyobject<T>::value, T>
cast(object&& object)
{
...
template<typename T>
enable_if_t<std::is_void<T>::value, void>
cast_safe(object&&)
{
}
template<typename T>
...
#else
inline cast_error
cast_error_unable_to_convert_call_arg(const std::string& name, const std::string& type)
{
  ...
  private:
    ...
    arg_v(arg&& base, T&& x, const char* descr = nullptr)
      ...
      , value(reinterpret_steal<object>(
          detail::make_caster<T>::cast(std::forward<T>(x), return_value_policy::automatic, {})))
      , descr(descr)
#if defined(PYBIND11_DETAILED_ERROR_MESSAGES)
      , type(type_id<T>())
#endif
...
object
object_api<Derived>::operator()(Args&&... args) const
{
#ifndef NDEBUG
    if (!PyGILState_Check()) {
...
template<return_value_policy policy, typename... Args>
object
object_api<Derived>::call(Args&&... args) const
{
