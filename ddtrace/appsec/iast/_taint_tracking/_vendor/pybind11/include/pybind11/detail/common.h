...
// See description of PR #4246:
  ...
  !(defined(PYPY_VERSION) && defined(_MSC_VER)) /* PyPy Windows: pytest hangs indefinitely at the end of the           \
                                                   process (see PR #4268) */                                           \
...
inline ssize_t
ssize_t_cast(const IntType& val)
{
...
template<typename T, typename... Ts>
constexpr int
first(int i, T v, Ts... vs)
{
    return v ? i : first(i + 1, vs...);
}

constexpr int
last(int /*i*/, int result)
{
    return result;
}
template<typename T, typename... Ts>
constexpr int
last(int i, int result, T v, Ts... vs)
{
...
[[noreturn]] PYBIND11_NOINLINE void
pybind11_fail(const char* reason)
{
...
std::string
get_fully_qualified_tp_name(PyTypeObject*);
...
#define PYBIND11_WORKAROUND_INCORRECT_GCC_UNUSED_BUT_SET_PARAMETER(...)
  ...
  (defined(__apple_build_version__)                   /* AppleClang 13.0.0.13000029 was the only data point            \
                                                         available. */                                                 \
   || (__clang_major__ >= 7 && __clang_major__ <= 12) /* Clang 3, 5, 13, 14, 15 do not generate the warning. */        \
  )
