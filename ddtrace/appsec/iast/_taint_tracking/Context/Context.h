...
private:
  ...
  static size_t pyint_and_analyzer_hash(const py::int_ &vuln_hash,
                                        string_view analyzer_name) {
    return std::hash<size_t>()(
        std::hash<long long>()(PyIntHasher()(vuln_hash)) +
...
public:
  ...
  void add_sent_blocking_vulnerability_hash(const py::int_ &vuln_hash,
                                            string_view analyzer_name) {
    blocking_vulnerabilities_sent_hashes.insert(
        pyint_and_analyzer_hash(vuln_hash, analyzer_name));
  }

  bool
  blocking_vulnerability_hash_already_sent(const py::int_ &vuln_hash,
                                           string_view analyzer_name) const {
    auto res_hash = pyint_and_analyzer_hash(vuln_hash, analyzer_name);
    return blocking_vulnerabilities_sent_hashes.find(res_hash) !=
           blocking_vulnerabilities_sent_hashes.end();
  }

  void reset_blocking_vulnerability_hashes() {
    blocking_vulnerabilities_sent_hashes.clear();
  }
};

void pyexport_context(py::module &m);
