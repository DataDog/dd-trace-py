...
TaintRangeMapType* Initializer::create_tainting_map() {
...
TaintRangeMapType* Initializer::get_tainting_map() {
...
TaintedObjectPtr Initializer::allocate_tainted_object() {
...
void Initializer::release_tainted_object(TaintedObjectPtr tobj) {
    ...
    // Stack full, just delete the object (but to a reset before so ranges are reused or freed)
    delete tobj;
}

TaintRangePtr Initializer::allocate_taint_range(int start, int lenght, SourcePtr origin) {
...
void Initializer::release_taint_range(TaintRangePtr rangeptr) {
    ...
    // Stack full or initializer already cleared (interpreter finishing), just release the object
    rangeptr.reset();
}
...
SourcePtr Initializer::allocate_taint_origin(string name, OriginType origin, string value) {
    ...
    // else: not in the map, retrieve from the stack and insert in the map before returning it
...
shared_ptr<Context> Initializer::get_context(size_t tx_id_) {
    if (tx_id_ == 0) {
        if (ThreadContextCache.tx_id == 0) {
...
void pyexport_initializer(py::module& m) {
    ...
    //        // In this case we want the usually dangerous "create if it doesn't exist and return it"
...
//          [](const string& key, const string& value) { initializer->global_context->framework_data[key] = value; });
//
//    m.def("gcontext_get_framework_data", [](const string& key) -> string {
//        try {
//            return initializer->global_context->framework_data.at(key);
//        } catch (const out_of_range&) {
//            throw_with_nested(py::key_error("Key " + key + " not in global framework data"));
//        }
//    });
//
//    m.def(
...
//
//    m.def(
//        "get_context", [](const size_t tx_id) { return initializer->get_context(tx_id); },
...
//        return initializer->stdlib_modules.find(module_name) != initializer->stdlib_modules.end();
//    });
//
