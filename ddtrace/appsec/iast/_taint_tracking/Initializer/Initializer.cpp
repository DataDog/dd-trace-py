...
void Initializer::load_modules() {
  pyfunc_get_python_lib =
      py::module::import("distutils.sysconfig").attr("get_python_lib");
...
SourcePtr Initializer::allocate_taint_source(string name, string value,
                                             OriginType origin) {
...
shared_ptr<Context> Initializer::get_context(size_t tx_id_) {
  ...
  auto it =
      active_map_addreses.find((TaintRangeMapType *)ThreadContextCache.tx_id);
  if (it == active_map_addreses.end()) {
    throw ContextNotInitializedException(
        "Context doesnt have available tainted map allocated");
  }
...
size_t Initializer::context_id() { return ThreadContextCache.tx_id; }
...
void pyexport_initializer(py::module &m) {
  m.def("get_taint_debug",
        [] { return py::bool_(initializer->get_taint_debug()); });

  m.def("set_taint_debug",
        [](bool newvalue) { initializer->set_taint_debug(newvalue); });

  m.def("clear_tainting_maps", [] { initializer->clear_tainting_maps(); });

  m.def("reset_stdlib_paths_and_modules",
