 ...
 **/
static PyObject* get_file_and_line(PyObject* self, PyObject* const* args, Py_ssize_t nargs) {
    PyThreadState *tstate = PyThreadState_GET();
    PyFrameObject* frame;
    if (NULL != tstate && NULL != tstate->frame) {
        frame = tstate->frame;
        while (NULL != frame) {
            char *filename = PyBytes_AsString(PyUnicode_AsEncodedString(frame->f_code->co_filename, "utf-8", "surrogatepass"));
