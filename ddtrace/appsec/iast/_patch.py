    ...
    if _is_iast_enabled():
        try:
            ...
            if not is_pyobject_tainted(value):
                ...
                return taint_pyobject(value, Source(name, value, origin))
        except Exception:
...
def if_iast_taint_yield_tuple_for(origins, wrapped, instance, args, kwargs):
    if _is_iast_enabled():
        ...
        for key, value in wrapped(*args, **kwargs):
            new_key = taint_pyobject(key, Source(key, key, origins[0]))
            new_value = taint_pyobject(value, Source(key, value, origins[1]))
