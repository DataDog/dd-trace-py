...
def _asm_manual_keep(span: Span) -> None:
    ...
    span._set_attribute(SAMPLING_DECISION_TRACE_TAG_KEY, "-%d" % SamplingMechanism.APPSEC)
...
def _aiguard_manual_keep(span: Span) -> None:
    ...
    span._set_attribute(SAMPLING_DECISION_TRACE_TAG_KEY, "-%d" % SamplingMechanism.AI_GUARD)
...
def _handle_metadata(entry_span: Span, prefix: str, metadata: dict) -> None:
    ...
    while stack:
        ...
        if isinstance(data, list):
            ...
            entry_span._set_attribute(f"{prefix}", str(data))
...
def _track_user_login_common(
    ...
    if span:
        ...
        if login_events_mode in (LOGIN_EVENTS_MODE.SDK, LOGIN_EVENTS_MODE.AUTO):
            span._set_attribute("%s.sdk" % tag_prefix, "true")
        ...
        if login:
            ...
            span._set_attribute("%s.login" % tag_prefix, login)

        if email:
            span._set_attribute("%s.email" % tag_prefix, email)

        if name:
            span._set_attribute("%s.username" % tag_prefix, name)
...
def track_user_login_failure_event(
    ...
    if exists is not None:
        ...
        span._set_attribute("%s.failure.%s" % (APPSEC.USER_LOGIN_EVENT_PREFIX_PUBLIC, user.EXISTS), exists_str)
    if user_id:
        if real_mode == LOGIN_EVENTS_MODE.ANON and isinstance(user_id, str):
            user_id = _hash_user_id(user_id)
        if login_events_mode != LOGIN_EVENTS_MODE.SDK:
            span._set_attribute(APPSEC.USER_LOGIN_USERID, str(user_id))
        span._set_attribute("%s.failure.%s" % (APPSEC.USER_LOGIN_EVENT_PREFIX_PUBLIC, user.ID), str(user_id))
    span._set_attribute(APPSEC.AUTO_LOGIN_EVENTS_COLLECTION_MODE, real_mode)
    # if called from the SDK, set the login, email and name
    if login_events_mode in (LOGIN_EVENTS_MODE.SDK, LOGIN_EVENTS_MODE.AUTO):
        if login:
            span._set_attribute("%s.failure.login" % APPSEC.USER_LOGIN_EVENT_PREFIX_PUBLIC, login)
        if email:
            span._set_attribute("%s.failure.email" % APPSEC.USER_LOGIN_EVENT_PREFIX_PUBLIC, email)
        if name:
            span._set_attribute("%s.failure.username" % APPSEC.USER_LOGIN_EVENT_PREFIX_PUBLIC, name)
    if in_asm_context():
...
def track_user_signup_event(
    ...
    if span:
        ...
        # This is used to mark if the call was done from the SDK of the automatic login events
        if login_events_mode == LOGIN_EVENTS_MODE.SDK:
            span._set_attribute("%s.sdk" % APPSEC.USER_SIGNUP_EVENT, "true")
        else:
            span._set_attribute("%s.auto.mode" % APPSEC.USER_SIGNUP_EVENT_MODE, str(login_events_mode))
...
def track_custom_event(tracer: Any, event_name: str, metadata: dict[str, Any]) -> None:
    ...
    span._set_attribute("%s.%s.track" % (APPSEC.CUSTOM_EVENT_PREFIX, event_name), "true")
    if metadata:
