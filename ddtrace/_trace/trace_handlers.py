...
def _set_inferred_proxy_tags(span, status_code):
    if span._parent and span._parent.name in SUPPORTED_PROXY_SPAN_NAMES:
        ...
        status_code = status_code if status_code else span._get_str_attribute("http.status_code")
        if status_code:
...
def _set_flask_request_tags(request, span, flask_config):
    try:
        ...
        if not span._get_str_attribute(FLASK_ENDPOINT) and request.endpoint:
            span.resource = " ".join((request.method, request.endpoint))
            span._set_attribute(FLASK_ENDPOINT, request.endpoint)

        if not span._get_str_attribute(FLASK_URL_RULE) and request.url_rule and request.url_rule.rule:
            span.resource = " ".join((request.method, request.url_rule.rule))
            span._set_attribute(FLASK_URL_RULE, request.url_rule.rule)

        if not span._get_str_attribute(FLASK_VIEW_ARGS) and request.view_args and flask_config.get("collect_view_args"):
...
def _on_start_response_pre(request, ctx, flask_config, status_code, headers):
    ...
    #      even if it is a 404
    if not span._get_str_attribute(FLASK_ENDPOINT) and not span._get_str_attribute(FLASK_URL_RULE):
    ...
    _on_web_framework_finish_request(
        ...
        req_headers=None,
        res_headers=headers,
        route=span._get_str_attribute(FLASK_URL_RULE),
        finish=False,
...
def _on_django_finalize_response_pre(ctx, after_request_tags, request, response):
    ...
    trace_utils.set_http_meta(span, ctx.get_item("integration_config"), route=span._get_str_attribute("http.route"))
    _set_inferred_proxy_tags(span, None)
...
def _on_botocore_patched_api_call_exception(ctx, response, exception_type, is_error_code_fn):
    ...
    status_code = span._get_str_attribute(http.STATUS_CODE)
...
def _on_router_match(route):
    ...
    if not req_span._get_str_attribute(MOLTEN_ROUTE):
        req_span._set_attribute(MOLTEN_ROUTE, route.name)
    if not req_span._get_str_attribute(http.ROUTE):
