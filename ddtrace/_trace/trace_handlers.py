...
def _set_flask_request_tags(request, span, flask_config):
    try:
        ...
        if not span.get_tag(FLASK_URL_RULE) and request.url_rule and request.url_rule.rule:
            # Prefix script_root so mounted sub-apps report the client-hit path (e.g. /api/v2/users) rather
            # than the app-local rule (/users). script_root is empty for non-mounted apps.
