...
_SpanContextEventType = TypeVar("_SpanContextEventType", bound=SpanContextEvent)


def _start_span(ctx: core.ExecutionContext[_SpanContextEventType]) -> Span:
    """Start a span from an event stored in the context.

    Args:
        ctx: ExecutionContext containing the event
        call_trace: Whether to use tracer.trace (True) or tracer.start_span (False)
    ...
    # Handle distributed headers activation (for integrations that need it)
    ...
    # Build span_kwargs from event attributes
    span_kwargs = {"span_type": event.span_type, "resource": event.resource, "service": event.service}
...
class SpanTracingSubscriber(BaseContextSubscriber[_SpanContextEventType], Generic[_SpanContextEventType]):
    ...
    @classmethod
    def _on_context_started(cls, ctx: core.ExecutionContext[_SpanContextEventType]) -> None:
        _start_span(ctx)
    ...
    def _on_context_ended(
        cls,
        ctx: core.ExecutionContext[_SpanContextEventType],
