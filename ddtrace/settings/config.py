...
class Config(object):
    ...
    def __init__(self):
        ...
        self._tracing_api = os.getenv("DD_TRACE_API_VERSION")
        self._tracing_writer_buffer_size = int(
            os.getenv("DD_TRACE_WRITER_BUFFER_SIZE_BYTES", default=DEFAULT_BUFFER_SIZE)
        )
        self._tracing_writer_payload_size = int(
            os.getenv("DD_TRACE_WRITER_MAX_PAYLOAD_SIZE_BYTES", default=DEFAULT_MAX_PAYLOAD_SIZE)
        )
        self._tracing_interval_seconds = float(
