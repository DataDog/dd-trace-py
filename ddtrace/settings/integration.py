...
class IntegrationConfig(AttrDict):
    ...
    def __init__(self, global_config, name, *args, **kwargs):
        ...
        object.__setattr__(self, "_integration_name", name)
        object.__setattr__(self, "_hooks", Hooks())
        object.__setattr__(self, "_http", HttpConfig())
    ...
    def _get_analytics_settings(self):
        ...
        env = "DD_TRACE_%s_ANALYTICS_ENABLED" % self._integration_name.upper()
        legacy_env = "DD_%s_ANALYTICS_ENABLED" % self._integration_name.upper()
        ...
        if analytics_enabled:
            deprecate(
                ...
                f"environment variables and the ddtrace.config.{self._integration_name}.analytics_enabled "
                "configuration. This feature and its associated configurations will be removed in a future release.",
                category=DDTraceDeprecationWarning,
            )

        analytics_sample_rate = float(
            os.getenv(
                "DD_TRACE_%s_ANALYTICS_SAMPLE_RATE" % self._integration_name.upper(),
                os.getenv("DD_%s_ANALYTICS_SAMPLE_RATE" % self._integration_name.upper(), default=1.0),
            )
        )
    ...
    def get_http_tag_query_string(self, value):
        if self._global_config._http_tag_query_string:
    ...
    def trace_query_string(self):
        ...
        return self._global_config.http.trace_query_string
    ...
    def is_header_tracing_configured(self):
        ...
        """
        return self.http.is_header_tracing_config.red or self._global_config._http.is_header_tracing_configured
    ...
    def _header_tag_name(self, header_name):
        ...
        if tag_name is None:
            return self._global_config._header_tag_name(header_name)
        return tag_name
    ...
    def _is_analytics_enabled(self, use_global_config):
        ...
        if use_global_config and self._global_config._analytics_enabled:
    ...
    def copy(self):
        new_instance = self.__class__(self._global_config, self._integration_name)
