...
from .encoding import encoder_for_endpoints
...
class AgentWriter(periodic.PeriodicService, TraceWriter):
    ...
    def __init__(
        ...
        self._infer_endpoint_and_encoder(agent_url)
    ...
    def _infer_endpoint_and_encoder(self, url):
        # type: (str) -> None
        info = agent._get_info(url)
        if info is None:
            # The agent is not running
            raise RuntimeError("Agent not reachable on %s" % url)
        ...
        if info == {}:
            ...
            endpoints = [_ for _ in info.get("endpoints", []) if _.endswith("/traces")]

        if not endpoints:
            raise RuntimeError("No traces endpoint found")

        self._endpoint, encoder_class = encoder_for_endpoints(endpoints)
