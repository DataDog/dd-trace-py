...
class OpenAIIntegration(BaseLLMIntegration):
    ...
    def generate_completion_llm_records(self, resp: Any, err: Any, span: Span, kwargs: Dict[str, Any]) -> None:
        ...
        if not self.llmobs_enabled:
            return
        if err is not None:
            attrs_dict = self._llm_error_record(span, kwargs, "completion")
        ...
        for i in range(n):
            ...
            attrs_dict = self._llmobs_record(span, kwargs, resp, "completion")
    ...
    def generate_chat_llm_records(self, resp: Any, err: Any, span: Span, kwargs: Dict[str, Any]) -> None:
        ...
        if not self.llmobs_enabled:
            return
        if err is not None:
            attrs_dict = self._llm_error_record(span, kwargs, "chat")
            self.llm_record(span, attrs_dict)
            return
        # Note: LLMObs ingest endpoint only accepts a 1:1 prompt-response mapping per record,
        ...
        for choice in resp.choices:
            ...
            attrs_dict = self._llmobs_record(span, kwargs, resp, "chat")
    ...
    def _llmobs_record(self, span: Span, kwargs: Dict[str, Any], resp: Any, record_type: str) -> Dict[str, Any]:
