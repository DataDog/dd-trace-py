...
class AgentWriter(HTTPWriter, AgentWriterInterface):
    ...
    def __init__(
        ...
        additional_header_str = env.dd_environ.get("_DD_TRACE_WRITER_ADDITIONAL_HEADERS")
...
class NativeWriter(periodic.PeriodicService, TraceWriter, AgentWriterInterface):
    ...
    def _create_exporter(self) -> native.TraceExporter:
        ...
        if self._stats_opt_out:
            ...
            stats_interval = float(env.getenv("_DD_TRACE_STATS_WRITER_INTERVAL") or 10.0)
...
def _use_log_writer() -> bool:
    ...
    """
    if (
        env.dd_environ.get("DD_AGENT_HOST")
        or env.dd_environ.get("DATADOG_TRACE_AGENT_HOSTNAME")
        or env.dd_environ.get("DD_TRACE_AGENT_URL")
    ):
