...
from .data.payload import AppGenerateMetricsPayload
...
class TelemetryWriter(PeriodicService):
    ...
    def add_metric(self, series):
        ...
        if metric_name in self.series_map:
            self.series_map[metric_name].combine(series)
            return

        self.series_map[metric_name] = series
    ...
    def _add_integrations_to_requests(self):
        ...
        if self.integrations_queue:
            integrations_request = create_telemetry_request(
                payload=AppIntegrationsChangedPayload(self.integrations_queue)
            )
            self.requests.append(integrations_request)
            self.integrations_queue = []

    def _add_metrics_to_requests(self):
        # type: () -> None
        if self.series_map:
            series_queue = self.series_map.values()
            self.series_map = {}  # type: Dict[str, Series]
            metrics_request = create_telemetry_request(payload=AppGenerateMetricsPayload(series_queue))
            self.requests.append(metrics_request)
    ...
    def shutdown(self):
        # type: () -> None
        appclosed_request = create_telemetry_request(payload=AppClosedPayload())
