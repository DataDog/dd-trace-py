...
from ddtrace.internal.settings import env as env_config
...
def get_config(
    ...
    if otel_env is not None and otel_env in env_config.dd_environ:
    ...
    for env in envs:
        if env in env_config.dd_environ:
            val = env_config.dd_environ[env]
            if modifier:
                val = modifier(val)
...
def _invalid_otel_config(otel_env):
    log.warning(
        ...
        otel_env,
        env_config.dd_environ.get(otel_env, ""),
    )
...
def validate_otel_envs():
    user_envs = {key.upper(): value for key, value in env_config.dd_environ.items()}
    for otel_env, _ in user_envs.items():
        if (
            ...
            otel_value = env_config.dd_environ.get(otel_env, "none").lower()
            if otel_value != "none":
                _invalid_otel_config(otel_env)
...
def validate_and_report_otel_metrics_exporter_enabled():
    ...
    if "OTEL_METRICS_EXPORTER" in user_envs:
        otel_value = env_config.dd_environ.get("OTEL_METRICS_EXPORTER", "otlp").lower()
        if otel_value == "none":
...
def _hiding_otel_config(otel_env, dd_env):
    log.debug(
        ...
        dd_env,
        otel_env,
        env_config.dd_environ[otel_env],
    )
    telemetry_writer.add_count_metric(
        TELEMETRY_NAMESPACE.TRACERS,
        "otel.env.hiding",
