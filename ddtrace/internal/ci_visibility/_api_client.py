...
def _get_known_tests_max_pages() -> int:
    ...
    try:
        value = int(env.dd_environ.get("_DD_CIVISIBILITY_KNOWN_TESTS_MAX_PAGES", str(_DEFAULT_KNOWN_TESTS_MAX_PAGES)))
    except ValueError:
        log.warning(
    ...
    def fetch_settings(self, read_from_cache: bool = True) -> TestVisibilityAPISettings:
        ...
        try:
            ...
            flaky_test_retries_enabled = attributes["flaky_test_retries_enabled"] or asbool(
                env.getenv("_DD_TEST_FORCE_ENABLE_ATR")
            )
            ...
            attempt_to_fix_retries_env = env.getenv("DD_TEST_MANAGEMENT_ATTEMPT_TO_FIX_RETRIES")
            ...
            test_management = TestManagementSettings(
                enabled=test_management_enabled or asbool(env.getenv("_DD_TEST_FORCE_ENABLE_TEST_MANAGEMENT")),
