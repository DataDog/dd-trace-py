...
class CIVisibility(Service):
    ...
    def _fetch_known_tests(self) -> Optional[Set[InternalTestId]]:
        try:
            ...
            log.warning("API client not initialized, cannot fetch known tests")
        except Exception:
            log.debug("Error fetching known tests", exc_info=True)
        return None
    ...
    def is_known_test(cls, test_id: Union[TestId, InternalTestId]) -> bool:
        ...
        # The assumption that we were not able to fetch known tests properly if the length is 0 is acceptable
        # because the current EFD usage would cause the session to be faulty even if the query was successful but
        # not known tests exist. In this case, we assume all tests are known.
