...
from ddtrace.internal._span_protocol import SpanProtocol
...
class CIVisibilityEncoderV01(BufferedEncoder):
    ...
    def _get_parent_session(self, traces: Sequence[Sequence["SpanProtocol"]]) -> int:
        for trace in traces:
            for span in trace:
    ...
    def _build_payload(self, traces: Sequence[Sequence["SpanProtocol"]]) -> list[tuple[Optional[bytes], int]]:
        """
    ...
    def _build_payloads_recursive(
        self, traces: Sequence[Sequence["SpanProtocol"]], start_idx: int, end_idx: int, new_parent_session_span_id: int
    ...
    def _convert_span(
        self, span: "SpanProtocol", dd_origin: Optional[str] = None, new_parent_session_span_id: int = 0
    ) -> dict[str, Any]:
...
class CIVisibilityCoverageEncoderV02(CIVisibilityEncoderV01):
    ...
    def _build_data(self, traces: Sequence[Sequence["SpanProtocol"]]) -> Optional[bytes]:
        new_parent_session_span_id = self._get_parent_session(traces)
        normalized_covs = [
