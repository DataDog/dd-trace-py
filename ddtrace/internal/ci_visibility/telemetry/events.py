...
def _record_event(
    ...
    if has_codeowners is not None and event_type != EVENT_TYPES.SESSION:
        log.debug("has_codeowners tag can only be set for sessions, but event type is %s", event_type)
    if unsupported_ci is not None and event_type != EVENT_TYPES.SESSION:
        log.debug("unsupported_ci tag can only be set for sessions, but event type is %s", event_type)
    if is_benchmark is not None and event_type != EVENT_TYPES.TEST:
...
def record_events_enqueued_for_serialization(events_count: int):
    try:
        ...
        log.debug("Invalid events count for events_enqueued_for_serialization telemetry: %s", events_count)
        return
