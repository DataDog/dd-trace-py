...
RETRIES = 1
try:
    if _CONFIG_ENDPOINT_RETRIES_ENV in env.dd_environ:
        RETRIES = int(env.getenv(_CONFIG_ENDPOINT_RETRIES_ENV, str(RETRIES)))
except ValueError:
...
try:
    if _CONFIG_ENDPOINT_TIMEOUT_ENV in env.dd_environ:
        TIMEOUT = int(env.getenv(_CONFIG_ENDPOINT_TIMEOUT_ENV, str(TIMEOUT)))
except ValueError:
...
def fetch_config_from_endpoint() -> dict:
    ...
    """
    config_endpoint = env.getenv(_CONFIG_ENDPOINT_ENV, None)
