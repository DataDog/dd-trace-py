"""Fast coverage instrumentation for Python 3.14."""
...
def add_file_coverage_hook(code: CodeType, hook_func: t.Callable[[str], None], file_path: str) -> CodeType:
    """Add a single hook call at the beginning of a code object for Python 3.14."""
    ...
    # Python 3.14+ bytecode (same as 3.12/3.13 for now)
    hook_bytecode = bytes([
...
def _adjust_linetable(original_linetable: bytes, added_bytes: int) -> bytes:
    """Adjust line number table to account for added bytecode for Python 3.14."""
    
    if not original_linetable:
        return original_linetable
    
    # Python 3.14+ format - add entry for added instructions with no line change
