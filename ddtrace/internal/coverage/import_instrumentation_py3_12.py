...
# Accurate import-hook injection is intentionally unsupported on 3.15+; see the comment at
# instrumentation_py3_12._USE_ACCURATE_IMPORTS for the full rationale and the work required to
# re-enable it (the `bytecode` library's CALL codegen segfaults on 3.15, so injection must be
# reimplemented on sys.monitoring INSTRUCTION events).
