"""Fast coverage instrumentation for Python 3.12."""
...
assert sys.version_info >= (3, 12) and sys.version_info < (3, 13)  # nosec


def add_file_coverage_hook(code: CodeType, hook_func: t.Callable[[str], None], file_path: str) -> CodeType:
    """Add a single hook call at the beginning of a code object for Python 3.12."""
    ...
    # Python 3.12+ bytecode (PRECALL was removed in 3.12)
    hook_bytecode = bytes([
...
def _adjust_linetable(original_linetable: bytes, added_bytes: int) -> bytes:
    """Adjust line number table to account for added bytecode for Python 3.12."""
    
    if not original_linetable:
        return original_linetable
    
    # Python 3.12+ format - add entry for added instructions with no line change
