...
def update_location_data(
    ...
    while True:
        try:
            ...
            if offset in trap_map:
                # No location info for the trap bytecode
                n, r = divmod(trap_map[offset], 8)
                for _ in range(n):
...
def compile_exception_table(etab: t.List[ExceptionTableEntry]) -> bytes:
    table = bytearray()
    for entry in etab:
...
def instrument_all_lines(code: CodeType, hook: HookType, path: str) -> t.Tuple[CodeType, t.Set[int]]:
    ...
    try:
        resume_offset = code.co_code[::2].index(dis.opmap["RESUME"]) << 1
    except ValueError:
        resume_offset = -1
    ...
    try:
        ...
        while True:
            ...
            if original_offset in line_starts and original_offset > resume_offset:
                ...
                # Instrument the line only once. This also prevents
                # instrumenting in inappropriate places, such as before
                # END_ASYNC_FOR.
    ...
    # jumps
    for original_offset, instr in enumerate(instructions):
        new_offset = original_offset << 1
    ...
    while process_branches:
        ...
        for branch in branches:
            ...
            if c:
                ...
                for opcode, instr in enumerate(instructions[index + 1 :], index + 1):
                    instr.offset = opcode << 1
