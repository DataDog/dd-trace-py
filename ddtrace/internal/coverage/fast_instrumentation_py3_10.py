"""Fast coverage instrumentation for Python 3.10."""
...
def add_file_coverage_hook(code: CodeType, hook_func: t.Callable[[str], None], file_path: str) -> CodeType:
    """Add a single hook call at the beginning of a code object for Python 3.10."""
...
def _adjust_linetable(original_linetable: bytes, added_bytes: int) -> bytes:
    """Adjust line number table to account for added bytecode for Python 3.10."""
    
    if not original_linetable:
        return original_linetable
    
    # Python 3.10 format - add entry for added instructions with no line change
    added_instructions = added_bytes
    
    # Add entry for added instructions with no line change
    return bytes([added_instructions, 0x80]) + original_linetable
