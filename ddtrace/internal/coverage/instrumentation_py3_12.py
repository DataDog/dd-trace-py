...
def ensure_coverage_monitoring_hooks_initialized():
    ...
    if _coverage_monitoring_hooks_initialized:
        return _coverage_monitoring_hooks_successful

    _coverage_monitoring_hooks_initialized = True

    try:
        sys.monitoring.use_tool_id(sys.monitoring.COVERAGE_ID, "datadog")  # noqa
    except ValueError:
        log.warning("Another coverage tool is already installed, Datadog tracer will not collect coverage.")
        return
    ...
    _coverage_monitoring_hooks_successful = True
    return True
