...
class ModuleCodeCollector(ModuleWatchdog):
    ...
    def instrument_code(self, code: CodeType, package) -> CodeType:
        ...
        if not lines:
            # Instrumentation returned empty — COVERAGE_ID is temporarily released
            # (e.g. during pytest hook yield) or held by another tool.  Don't mark
            # as seen or record lines, so the code can be re-instrumented once the
            # slot is reclaimed.
            return code
