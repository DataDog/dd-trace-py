...
class _IdentityWeakKeyDictionary:
    """Weak mapping keyed by object identity instead of equality."""

    # AIDEV-NOTE: CodeType equality is structural. Keep identity semantics here and in
...
def register(code: CodeType, handler: MonitoringEventHandler) -> None:
    """Register handler for code, using the handler instance as its key."""
...
def refresh(code: CodeType) -> None:
    """Re-arm this tool's local events for code without changing other tools."""
    with _registry_lock:
        handlers: Optional[_CodeHandlers] = _registry.get(code)
