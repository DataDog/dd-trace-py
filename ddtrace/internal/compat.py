...
try:
    ...
    # mypy-py2 error:
    #   Incompatible types in assignment (expression has type "Type[StringIO[Any]]",
    #     variable has type overloaded function)  [assignment]
    StringIO = six.StringIO  # type: ignore[misc,assignment]
...
# mypy-py2 error: bug with this try/except requires ignoring typing error;
# see https://github.com/python/mypy/issues/1153
#   Module "time" has no attribute "time_ns"  [attr-defined]
#   Module "time" has no attribute "monotonic"  [attr-defined]
#   Module "time" has no attribute "monotonic_ns"  [attr-defined]
#   Module "time" has no attribute "process_time_ns"  [attr-defined]
try:
    from time import time_ns  # type: ignore[attr-defined]
except ImportError:
...
try:
    ...
    from ddtrace.vendor.monotonic import monotonic  # type: ignore[attr-defined]


try:
    from time import monotonic_ns  # type: ignore[attr-defined]
except ImportError:
...
try:
    from time import process_time_ns  # type: ignore[attr-defined]
except ImportError:
...
# see https://github.com/python/mypy/issues/1153
try:
    from types import MethodWrapperType  # type: ignore[attr-defined]
