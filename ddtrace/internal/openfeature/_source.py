...
DEFAULT_CDN_DELIVERY = "python"
SUPPORTED_CDN_DELIVERIES = frozenset(("hybrid", "native", "python"))
...
class FeatureFlagCdnConfig:
    ...
    delivery: str = DEFAULT_CDN_DELIVERY
...
def resolve_feature_flag_source_config(env: t.Optional[t.Mapping[str, str]] = None) -> FeatureFlagSourceConfig:
    ...
    cdn = FeatureFlagCdnConfig(
        ...
        delivery=_resolve_cdn_delivery(env.get("DD_FLAGGING_CDN_DELIVERY")),
    )
...
def _resolve_cdn_delivery(raw_delivery: t.Optional[str]) -> str:
    delivery = (_optional_value(raw_delivery) or DEFAULT_CDN_DELIVERY).lower().replace("-", "_")
    if delivery not in SUPPORTED_CDN_DELIVERIES:
        raise ValueError("DD_FLAGGING_CDN_DELIVERY must be one of: %s" % ", ".join(sorted(SUPPORTED_CDN_DELIVERIES)))
    return delivery
