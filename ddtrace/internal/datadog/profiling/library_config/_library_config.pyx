...
cdef extern from "datadog/common.h" nogil:
    ctypedef enum ddog_LibraryConfigValue_Tag:
        DDOG_LIBRARY_CONFIG_VALUE_NUM_VAL
        DDOG_LIBRARY_CONFIG_VALUE_BOOL_VAL
        DDOG_LIBRARY_CONFIG_VALUE_STR_VAL

cdef extern from "library_config.hpp" namespace "Datadog" nogil:
    cdef struct ConfigEntry:
        string key
        ddog_LibraryConfigValue_Tag config_type
        bint bool_val
        string str_val
        bint num_val
...
def get_config(debug_logs: bool) -> Dict[StringType, ValueType]:
    ...
    for i in range(cpp_config.len):
        ...
        value = None
        if config_entry.config_type == DDOG_LIBRARY_CONFIG_VALUE_NUM_VAL:
            value = config_entry.num_val
        elif config_entry.config_type == DDOG_LIBRARY_CONFIG_VALUE_BOOL_VAL:
            value = config_entry.bool_val != 0
        elif config_entry.config_type == DDOG_LIBRARY_CONFIG_VALUE_STR_VAL:
            value = config_entry.str_val.decode('utf-8')
        result[config_entry.key.decode('utf-8')] = value
    return result
