...
void DdogProfExporterDeleter::operator()(ddog_prof_Exporter *ptr) const {
  if (ptr)
...
bool Uploader::upload(const Profile *profile) {
  ...
  ddog_prof_Exporter_Request_BuildResult build_res =
      ddog_prof_Exporter_Request_build(ddog_exporter.get(), start, end,
  ...
  ddog_prof_Exporter_SendResult res =
      ddog_prof_Exporter_send(ddog_exporter.get(), &req, nullptr);
...
bool Profile::push_label(const ExportLabelKey key, int64_t val) {
  constexpr std::array<std::string_view,
                       static_cast<size_t>(ExportLabelKey::_Length)>
      keys = {EXPORTER_LABELS(X_STR)};
  if (cur_label >= labels.size())
...
bool Profile::push_cputime(int64_t cputime, int64_t count) {
