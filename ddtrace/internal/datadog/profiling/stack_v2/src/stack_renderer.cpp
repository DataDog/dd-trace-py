...
StackRenderer::render_frame(Frame& frame)
{
    ...
    if (maybe_name_id == string_id_cache.end()) {
        ...
        name_id = ddup_intern_string(name_str);
    ...
    if (maybe_filename_id == string_id_cache.end()) {
        ...
        filename_id = ddup_intern_string(filename_str);
    ...
    if (maybe_function_id == function_id_cache.end()) {
        function_id = ddup_intern_function(name_id, filename_id);
        function_id_cache.insert({ { name_id, filename_id }, function_id });
    } else {
        function_id = maybe_function_id->second;
    }

    ddup_push_frame_id(sample, function_id, 0, line);
}
