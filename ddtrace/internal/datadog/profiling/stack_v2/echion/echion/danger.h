  ...
  public:
    ...
    int ensure_installed()
    {
        ...
        if (stack_mem == MAP_FAILED) {
            std::cerr << "Failed to allocate alt stack. Memory copying may not work." << " Error: " << strerror(errno)
                      << std::endl;
            return -1;
        }
        ...
        if (sigaltstack(&ss, nullptr) != 0) {
            std::cerr << "Failed to set alt stack. Memory copying may not work." << " Error: " << strerror(errno)
                      << std::endl;
            return -1;
        }
