...
bool
ProfileSharedState::cycle_buffers()
{
...
void
ProfileSharedState::entrypoint_check()
{
    if (dirty.load()) {
        std::lock_guard<std::mutex> lock(profile_mtx);
        cycle_buffers();
...
void
ProfileSharedState::setup_samplers()
{
...
size_t
ProfileSharedState::get_sample_type_length()
{
    return samplers.size();
}

long
ProfileSharedState::get_profile_seq()
{
    return g_profile_seq.load();
}

ddog_prof_Profile&
ProfileSharedState::get_current_profile()
{
    return cur_profile;
}

unsigned int
ProfileSharedState::get_max_nframes()
{
    return max_nframes;
}

ProfileType
ProfileSharedState::get_type_mask()
{
    return type_mask;
}

static inline bool
make_profile(ddog_prof_Slice_ValueType& sample_types, const struct ddog_prof_Period* period, ddog_prof_Profile& profile)
{
...
void
ProfileSharedState::one_time_init(ProfileType type, unsigned int _max_nframes)
{
    ...
    // Set the type mask
    unsigned int mask_as_int = type & ProfileType::All;
...
std::string_view
ProfileSharedState::insert_or_get(std::string_view sv)
{
...
const ValueIndex&
ProfileSharedState::val()
{
    return val_idx;
}

bool
ProfileSharedState::flush_sample(ddog_prof_Sample& sample)
{
