...
bool
ddup_push_trace_endpoint(uint64_t local_root_span_id, std::string_view trace_endpoint, int64_t count)
{
    ...
    if (!res.ok) {
        auto err = res.err;
        const std::string errmsg = Datadog::err_to_msg(&err, "Error setting trace endpoint");
    ...
    res = ddog_prof_Profile_add_endpoint_count(&profile, trace_endpoint_slice, count);
    if (!res.ok) {
        auto err = res.err;
