...
Datadog::LibraryConfig::generate_config(bool debug_logs)
{
    ...
    for (size_t i = 0; i < configs.len; i++) {
        ...
        ddog_LibraryConfigValue value = cfg->value;

        Datadog::ConfigEntry* new_entry = new Datadog::ConfigEntry{
            .key = name.ptr,
            .config_type = value.tag,
            .num_val = 0,
            .bool_val = false,
            .str_val = ""
        };

        if (value.tag == DDOG_LIBRARY_CONFIG_VALUE_STR_VAL) {
            new_entry->str_val = value.str_val.ptr;
        } else if (value.tag == DDOG_LIBRARY_CONFIG_VALUE_BOOL_VAL) {
            new_entry->bool_val = value.bool_val;
        } else if (value.tag == DDOG_LIBRARY_CONFIG_VALUE_NUM_VAL) {
            new_entry->num_val = value.num_val;
        }
