...
Datadog::Sample::push_pyframes(PyFrameObject* frame)
{
    ...
    for (PyFrameObject* f = frame; f != nullptr;) {
        ...
        if (code != nullptr) {
            // Fast path: look up the cached function_id for this code object.
            // Cache key is the code object's address; weak reference detects GC reuse.
            ...
            // code == nullptr: preserve original behaviour of pushing an unknown frame.
...
{
    // Note: CodeObjectFunctionCache::instance().postfork_child() is called from
...
{
    // Note: the CodeObjectFunctionCache is not cleared here.
    // The ProfilesDictionary persists across profile uploads by design ("common
    // to multiple profiles through time"), so function_id values remain valid
    // for the entire process lifetime.  The cache is only invalidated on fork
    // (via ProfilerState::postfork_child()) when the dictionary is recreated.
