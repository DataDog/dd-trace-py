...
// Returns true if the weak reference's referent is still alive (not GC'd).
// Handles the CPython API deprecation timeline:
//   < 3.13: PyWeakref_GetObject() is the stable API (borrowed ref, not deprecated)
//  >= 3.13: PyWeakref_GetObject() and PyWeakref_GET_OBJECT() are deprecated;
//           use PyWeakref_GetRef() (new in 3.13, returns strong ref via out-param)
inline bool
...
CodeObjectFunctionCache::get(PyObject* code)
{
    ...
    if (!weakref_is_alive(it->second.weak_ref)) {
        // Stale entry: code object was freed. Evict and signal cache miss.
...
CodeObjectFunctionCache::postfork_child()
{
    ...
    //
    // Re-initialise the mutex via placement-new — the inherited mutex may be in
    // an inconsistent state if the parent held it at fork time.
