...
class Uploader
{
  private:
    ...
    std::string errmsg;
    static inline std::atomic<ddog_CancellationToken> cancel{ ddog_CancellationToken{ .inner = nullptr } };
  ...
  public:
    ...
    ~Uploader()
    {
        ...
        // Use exchange to safely get ownership of the token
        ddog_CancellationToken current_cancel = cancel.exchange(ddog_CancellationToken{ .inner = nullptr });
