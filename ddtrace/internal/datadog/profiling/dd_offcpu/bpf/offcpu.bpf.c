...
/* tid -> timestamp (ns) when that thread left the CPU. */
struct
{
...
/* Stamp the moment a thread leaves the CPU. Keyed by the global tid (unique and
 * cheap); the namespace-local id is only needed when we emit the event. */
static __always_inline void
record_off_cpu(struct task_struct* t)
{
    if (!is_target(t))
        return;
    ...
    bpf_map_update_elem(&start, &gtid, &ts, BPF_ANY);
}

/* Emit a completed interval when a thread returns to the CPU. */
static __always_inline void
record_on_cpu(void* ctx, struct task_struct* t)
{
    ...
    __u32 gtid = BPF_CORE_READ(t, pid);
    __u64* tsp = bpf_map_lookup_elem(&start, &gtid);
    if (!tsp)
        return;
