...
TEST_F(StackRendererSampleLifecycle, StartingNextCycleRecoversAbandonedSample)
{
    ...
    // Simulate sampling cycles that return after render_thread_begin() without reaching render_stack_end(). Running
    // beyond the pool capacity detects each overwritten raw pointer rather than only the first abandoned Sample.
