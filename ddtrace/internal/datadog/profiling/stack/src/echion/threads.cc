...
ThreadInfo::unwind_tasks(PyThreadState* tstate)
{
    ...
    // AIDEV-NOTE: We use separate cache keys for asyncio and uvloop because switching between them
    // (though unlikely at runtime) would cause incorrect boundary detection otherwise.
    static std::optional<Frame::Key> asyncio_frame_cache_key;
    static std::optional<Frame::Key> uvloop_frame_cache_key;
        ...
        for (auto current_task = leaf_task;;) {
            ...
            if (task.is_on_cpu) {
                ...
                for (size_t i = 0; i < frames_to_push; i++) {
                    ...
                    if (using_uvloop && is_uvloop_wrapper_frame(python_frame.get())) {
                        continue;
                    }
