...
ThreadInfo::unwind(EchionSampler& echion, PyThreadState* tstate, microsecond_t wall_time_us)
{
    ...
    const size_t max_frames = asyncio_loop ? MAX_STACK_DISCOVERY_DEPTH : echion.stack_max_frames();
    ...
    for (auto& leaf_task : leaf_tasks) {
        ...
        for (auto current_task = leaf_task;;) {
            ...
            if (auto it = task_coro_stacks.find(task.origin); it != task_coro_stacks.end()) {
                ...
                if (stack.size() < MAX_STACK_DISCOVERY_DEPTH) {
                    task_frames_to_push = std::min(task_stack_size, MAX_STACK_DISCOVERY_DEPTH - stack.size());
                }
            }
            if (task.is_on_cpu) {
