...
TaskInfo::unwind(EchionSampler& echion, FrameStack& stack, bool using_uvloop)
{
    ...
    while (!coro_frames.empty()) {
        ...
        auto unwind_result = unwind_frame(echion, frame, stack, echion.seen_frames_scratch(), 1);
        assert(unwind_result.frames_added <= 1 && "expected exactly 1 frame to be unwound (or 0 in case of an error)");
        ...
        if (unwind_result.frames_added == 0) {
            break;
        }
