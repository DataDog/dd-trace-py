...
def start() -> None:
    # SAMPLE_TYPE_GC is opt-in (not in SampleType::All). Register it before the
    # native profiler starts when the GC collector is enabled.
    from ddtrace.internal.settings.profiling import config as profiling_config

    gc_enabled: bool = profiling_config.gc.enabled
    if gc_enabled:
        ddup_config_sample_type(SAMPLE_TYPE_GC)
    ddup_start()
