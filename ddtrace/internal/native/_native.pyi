...
# -----------------------------------------------------------------------------
# libddwaf bindings (appsec WAF), built on the high-level `libddwaf` crate. Object building/reading
# happens in Rust; Python keeps orchestration. libddwaf is embedded in the extension.
# -----------------------------------------------------------------------------

class ddwaf:
    """Native libddwaf bindings; libddwaf is statically embedded in the extension."""

    class Builder:
        """A libddwaf config builder. The optional obfuscation key/value regexes are applied as the
        builder's base config.
        """

        def __new__(cls, key_regex: Optional[bytes] = ..., value_regex: Optional[bytes] = ...) -> "ddwaf.Builder": ...
        def add_or_update_config(self, path: str, ruleset: Any) -> tuple[bool, dict]:
            """Add/update a config; returns (ok, diagnostics) with diagnostics as a dict."""
            ...
        def add_or_update_config_json(self, path: str, json: bytes) -> tuple[bool, dict]:
            """Like add_or_update_config but parses the ruleset from JSON bytes natively."""
            ...
        def remove_config(self, path: str) -> bool: ...
        def config_paths_count(self, filter_re: str) -> int: ...
        def build_instance(self) -> Optional["ddwaf.Handle"]: ...

    class Handle:
        """A built WAF instance."""

        def known_addresses(self) -> list[str]: ...
        def new_context(self) -> "ddwaf.Context": ...

    class Context:
        """A per-request WAF evaluation context."""

        def run(
            self, data: Any, max_objects: int, max_depth: int, max_string_length: int, timeout_us: int
        ) -> "ddwaf.WafResult": ...
        def new_subcontext(self) -> Optional["ddwaf.Subcontext"]: ...

    class Subcontext:
        """A subcontext for non-persisting (RASP) evaluation."""
    ...
    class WafResult:
        """The structured outcome of a WAF run."""
        ...
        truncation: tuple[Optional[int], Optional[int], Optional[int]]

    @staticmethod
    def version() -> str: ...
    @staticmethod
    def ddwaf_object_roundtrip(
        value: Any, max_objects: int, max_depth: int, max_string_length: int
    ) -> tuple[Any, tuple[Optional[int], Optional[int], Optional[int]]]:
        """Build a WafObject from a Python value and read it back (object marshalling test helper)."""
        ...
