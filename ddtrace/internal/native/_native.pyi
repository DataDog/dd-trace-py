...
def profile_uploader_before_fork() -> None:
    """os.register_at_fork(before=...) hook: see profiling_uploader.rs's fork-safety comment."""
    ...

def profile_uploader_after_fork_parent() -> None:
    """os.register_at_fork(after_in_parent=...) hook: releases the lock held by before_fork."""
    ...

def profile_uploader_after_fork_child() -> None:
    """os.register_at_fork(after_in_child=...) hook: releases the lock held by before_fork."""
    ...

# Experimental, unwired PyO3 reimplementation of dd_wrapper's C++ sample-collection machinery
# (sample.cpp/profile.cpp) -- see OPTION_C_ONE_HOP_PLAN.md. Not reachable from any production
# code path; only used by tests/tracer/test_native_profile_dualwrite.py and
# test_native_profile_fork_safety.py, and only present when the native extension is built with
# the `profiling` Cargo feature.

SAMPLE_TYPE_ALL: int

def set_timeline(enabled: bool) -> None: ...

class SampleHandle:
    """
    Mirrors dd_wrapper's Cython `SampleHandle`/C++ `Datadog::Sample`: a single profiling
    sample being built up one push at a time, created via `DdProfile.start_sample()` and
    added to its owning profile with `DdProfile.add_sample()`.
    """
    ...
    def push_alloc(self, value: int, count: int) -> bool: ...
    def push_heap(self, value: int, count: int) -> bool: ...
    ...
    def incr_dropped_frames(self, count: Optional[int] = None) -> None: ...
    ...
    def push_span_id(self, span_id: int) -> None: ...
    def push_local_root_span_id(self, local_root_span_id: int) -> None: ...
    def push_trace_type(self, trace_type: str) -> None: ...
    def push_exceptioninfo(self, exception_type: str, count: int) -> None: ...
    def push_exception_message(self, exception_message: str) -> None: ...
    def push_class_name(self, class_name: str) -> None: ...
    def push_gpu_device_name(self, device_name: str) -> None: ...
    def push_monotonic_ns(self, monotonic_ns: int) -> bool: ...
    def push_absolute_ns(self, timestamp_ns: int) -> None: ...

class DdProfile:
    """Mirrors dd_wrapper's C++ `Datadog::Profile`: owns the aggregated sample set for one
    export interval, built directly on libdd-profiling without dd_wrapper/the C ABI."""

    def __init__(self, type_mask: int, max_nframes: int): ...
    def start_sample(self) -> SampleHandle: ...
    def add_sample(self, handle: SampleHandle) -> None: ...
    def add_endpoint(self, local_root_span_id: int, endpoint: str) -> None: ...
    def add_endpoint_count(self, endpoint: str, value: int) -> None: ...
    def serialize(self, end_time_ns: Optional[int] = None) -> tuple[bytes, int, int, list[tuple[str, int]]]: ...
    def max_nframes(self) -> int: ...
