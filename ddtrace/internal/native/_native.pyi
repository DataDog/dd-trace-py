...
class TelemetryAggregatorInternalError(Exception):
    """Raised when the native OTel telemetry aggregator fails (e.g. force_flush/shutdown)."""

class TelemetryAggregatorBuilder:
    """Builds a TelemetryAggregator wrapping libdatadog's OTel metrics aggregation + OTLP export."""

    def __init__(self) -> None: ...
    def set_resource_service(self, service: str) -> TelemetryAggregatorBuilder: ...
    def set_resource_env(self, env: str) -> TelemetryAggregatorBuilder: ...
    def set_resource_version(self, version: str) -> TelemetryAggregatorBuilder: ...
    def set_resource_attribute(self, key: str, value: str) -> TelemetryAggregatorBuilder: ...
    def set_metrics_exporter(
        self, endpoint: str, protocol: str, timeout_ms: int, headers: list[tuple[str, str]]
    ) -> TelemetryAggregatorBuilder:
        """Configure the OTLP metrics exporter. ``protocol`` is ``"grpc"`` or ``"http/protobuf"``."""
        ...
    def set_metrics_temporality(self, temporality: str) -> TelemetryAggregatorBuilder:
        """``temporality`` is ``"delta"`` or ``"cumulative"``."""
        ...
    def set_export_interval(self, interval_ms: int) -> TelemetryAggregatorBuilder: ...
    def build(self, shared_runtime: SharedRuntime) -> tuple[TelemetryAggregator, list[str]]:
...
class TelemetryAggregator:
    ...
    def register_instrument(
        ...
        """
        ...
    def record_counter(self, id: int, value: float, attrs: list[tuple[str, str]]) -> None: ...
    def record_up_down_counter(self, id: int, value: float, attrs: list[tuple[str, str]]) -> None: ...
    def record_histogram(self, id: int, value: float, attrs: list[tuple[str, str]]) -> None: ...
    def observe_gauge(self, id: int, value: float, attrs: list[tuple[str, str]]) -> None: ...
    def observe_counter(self, id: int, value: float, attrs: list[tuple[str, str]]) -> None: ...
