...
def redact_query_string(query_string: str, query_string_obfuscation_pattern: Pattern) -> str:
    return query_string_obfuscation_pattern.sub("<redacted>", query_string)


def redact_url(url: str, query_string_obfuscation_pattern: Pattern, query_string: Optional[str] = None) -> str:
    ...
    if redacted_query is not None and len(parts) >= 5:
        ...
        return urlunsplit(redacted_parts, url)

    # If no obfuscation is performed, return original url
    return url


def urlunsplit(components: Tuple[str, ...], original_url: str) -> str:
    """
