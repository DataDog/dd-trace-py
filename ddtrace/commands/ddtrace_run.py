...
def _add_bootstrap_to_pythonpath(bootstrap_dir):
    ...
    """
    python_path = _env.environ.get("PYTHONPATH", "")

    if python_path:
        new_path = "%s%s%s" % (bootstrap_dir, os.path.pathsep, _env.environ["PYTHONPATH"])
        _env.environ["PYTHONPATH"] = new_path
    else:
        _env.environ["PYTHONPATH"] = bootstrap_dir
...
def _prepare_env(parser: argparse.ArgumentParser):
    ...
    if args.profiling:
        _env.environ["DD_PROFILING_ENABLED"] = "true"

    if args.debug or ddtrace.config._debug_mode:
        logging.basicConfig(level=logging.DEBUG)
        _env.environ["DD_TRACE_DEBUG"] = "true"
    ...
    log.debug("PYTHONPATH: %s", _env.environ["PYTHONPATH"])
