...
class IAST_Aspects_do_rsplit_maxsplit(bm.Scenario):
    iast_enabled: bool
        ...
        def _(loops):
            for _ in range(loops):
                if self.iast_enabled:
                    mod_patched_methods.do_rsplit_maxsplit(*("foo bar baz", 1))
                else:
                    mod_unpatched_str_methods.do_rsplit_maxsplit(*("foo bar baz", 1))
