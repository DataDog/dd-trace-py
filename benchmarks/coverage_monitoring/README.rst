...
* ``line_level_*`` and ``file_level_*``: run the corresponding coverage mode
  while Error Tracking and exception profiling own their global events. The
  ``exceptions_first`` and ``coverage_first`` suffixes select startup order.
