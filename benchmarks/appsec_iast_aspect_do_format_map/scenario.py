...
class IAST_Aspects_do_format_map(bm.Scenario):
    iast_enabled: bool
        ...
        def _(loops):
            for _ in range(loops):
                if self.iast_enabled:
                    mod_patched_methods.do_format_map(*("foobar{baz}", {"baz": "bar"}))
                else:
                    mod_unpatched_str_methods.do_format_map(*("foobar{baz}", {"baz": "bar"}))
