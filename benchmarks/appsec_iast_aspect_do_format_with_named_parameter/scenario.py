...
class IAST_Aspects_do_format_with_named_parameter(bm.Scenario):
    iast_enabled: bool
        ...
        def _(loops):
            for _ in range(loops):
                if self.iast_enabled:
                    mod_patched_methods.do_format_with_named_parameter(*("foobar{key}", "baz"))
                else:
                    mod_unpatched_str_methods.do_format_with_named_parameter(*("foobar{key}", "baz"))
