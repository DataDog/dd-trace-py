...
def main() -> None:
    ...
    modules: list[Any] = [importlib.import_module("bench_module_{}".format(index)) for index in range(nmodules)]
            ...
            with ModuleCodeCollector.CollectInContext() as collector:
                ...
                if missing_paths := expected_paths.difference(covered_paths):
                    raise AssertionError(
                        "file-level coverage missed executed modules: {}".format(sorted(missing_paths))
                    )
        else:
            checksum ^= exercise_context(modules, context_index, modules_per_context)
