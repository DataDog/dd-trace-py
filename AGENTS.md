...
15. **Keep tool output concise** — Avoid flooding the conversation with verbose command output. For noisy builds/tests, redirect output to a log file and print only a short pass/fail summary; on failure, show the relevant tail or error excerpt. Prefer quiet flags such as `-q`, `--success-output never`, or equivalent when available. Do not paste full logs, huge diffs, or long skipped-test lists unless explicitly requested.
