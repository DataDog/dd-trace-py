...
**Vintage:** 2026-09-24. Profiling-only; see ``scripts/py315-stack/PROFILING_STACK.md``.
...
.. list-table::
   ...
   * - 3.14
     ...
     - Last version in ``requires-python`` (``>=3.9,<3.15``) until the official
       3.15 bump. Echion frame/task/asyncio, Riot splits, tests done.
   * - 3.15
     - Natives + asyncio on ``main``; tooling in review
     - **Merged:** native ABI `#19269`__, asyncio ``sys.monitoring`` `#19272`__,
       prof-correctness gate `#19207`__, Cython pin / optional wheels `#19861`__.
       **Open:** dev tooling (this runbook) `#19273`__, required wheels
       `#20450`__, PEP 440 hermetic pip `#20474`__, ADR `#20478`__.
       Echion: ``FRAME_OWNED_BY_CSTACK`` removed; ``PyFrameState`` renumbered;
       ``FRAME_SUSPENDED_YIELD_FROM_LOCKED`` for free-threaded builds.
       ``_asyncio.py``: ``sys.monitoring`` on 3.15+ because wrapping is
       unavailable (API itself since 3.12). Layout contracts in
       ``test_cpython_layout_contracts.cpp``. Guards: ``0x030f0000``.
       Registry: ``scripts/profiles/profiling_versions.json``.
...
PEP timeline (phase → work)
---------------------------

Anchor work to the CPython / PEP release schedule (alpha → beta → RC → final).
Prefer the ``track-cpython-release-schedule`` flow when available. Agent entry:
skill ``migrate-profiling-new-cpython``.

* **Alpha:** natives + layout contract tests + a gated CI job. Scaffold the
  version registry (``verify_profiler_compatibility.py --scaffold 3.X``).
  Automate: ``find-cpython-usage`` → ``compare-cpython-versions`` (later
  ``cpython_delta`` stacked PR). Artifact: `#19269`__.
* **Beta:** collectors compile path + asyncio hook. Probe ``wrap()``; if it
  fails, ``sys.monitoring`` on the new minor only. Automate: compat script
  PASS. Artifact: `#19272`__.
* **RC (within days of each tag):** engraver ``python/3.X.YrcN{,-fips}`` →
  dd-source digests → language-tools seed → ``IMAGE_TAG`` / manylinux mirror →
  optional then required cp3XX wheels. Pin the hermetic interpreter to the
  **exact** prerelease. Automate: mirror/engraver scripts + registry pins.
* **Final:** SSI/OCI; release note; ADR. Do **not** publish SSI from the wheel
  PR (`#17977`__ lesson).
...
Expected changes (order)
------------------------

#. Native ABI / build path — `#19269`__
#. Asyncio hook path — `#19272`__
#. Dev tooling / runbook / registry — this PR `#19273`__
#. prof-correctness gating — `#19207`__
#. Required wheels + lib_injection schedule — `#20450`__
#. Release note + ADR — `#20478`__

Catalog of every py-315 profiling PR (merged/open/closed): 
...
Unexpected changes and gotchas (what bit us)
--------------------------------------------

* **``wrap()`` broke on 3.15.** Asyncio moved to ``sys.monitoring`` on 3.15+
  while keeping ``wrap`` below 3.15 (`#19272`__). A wrong fail-closed gate had
  to be fixed (do not gate monitoring on ``NEXT_PY``). Automate: skill gotcha
  table + compat script.
* **Prerelease ABI mismatch (a2 vs rc2).** CrashLoop / ``_native`` import
  errors looked like profiler bugs. Pin hermetic interpreter to the exact
  prerelease tag. Automate: registry ``image_tags`` + checklist.
* **Cython** pin ``<3.3`` required on 3.15; cp3XX wheels started **optional**
  (`#19861`__). Automate: registry ``cython_pin`` / ``wheels_required``.
* **Base-image / engraver flow.** Mirror manylinux (`images#9814`__ /
  `#11355`__), bump dd-trace-py bases (`#17959`__, `#19936`__), engraver
  ``python/3.X.YrcN{,-fips}``, dd-source digests, language-tools seed,
  whl_installer host-pip / ``locate_file`` fix. Automate: script/template.
* **Staging A/B failures were auth/signing/wheels**, not the profiler:
  ``BUILD_WEDGED``, AppGate/vault expiry, SSH ``unknown_key``, passphrase
  prompt hanging in detached tmux. Preflight each. Automate: skill hard stop.
* **Memory:** RSS about +15% with profiler on (~53% runtime / ~47% profiler).
  memalloc is not the lever. Do **not** gate the functional claim on memory
  parity.
* **SSI/OCI** stays ``when: never`` until final. Do not publish SSI from the
  wheel PR (`#17977`__).
* **Hermetic pip / PEP 440 local versions** needed a fix (`#20474`__).

__ https://github.com/DataDog/dd-trace-py/pull/19272
__ https://github.com/DataDog/dd-trace-py/pull/19861
__ https://github.com/DataDog/images/pull/9814
__ https://github.com/DataDog/images/pull/11355
__ https://github.com/DataDog/dd-trace-py/pull/17959
__ https://github.com/DataDog/dd-trace-py/pull/19936
__ https://github.com/DataDog/dd-trace-py/pull/17977
__ https://github.com/DataDog/dd-trace-py/pull/20474
...
.. code-block:: text
   ...
   Python 3.14  →  0x030e0000   ← last fully released in requires-python
   Python 3.15  →  0x030f0000   ← natives + asyncio on main; tooling here
   Python 3.16  →  0x03100000   ← next: --scaffold 3.16
...
Baselines for Python 3.9–3.15 live in ``scripts/profiles/compatibility_baselines.json``.
Version metadata (hex, asyncio hook, Cython pin, open checklist) lives in
``scripts/profiles/profiling_versions.json``. Scaffold the next minor with::
...
* **Local compat:** ``verify_profiler_compatibility.py --python 3.X`` (and
  ``--compare``). Asyncio guards PASS; wall samples ≥ registry
  ``min_wall_time_samples``; named tasks present.
* **Local suites:** ``scripts/run-profiling-tests --python 3.X`` (profile +
  profile-memalloc riot; fail-closed if venvs missing).
* **Local A/B harness** (314 vs 315 smoke / async / ``PROFILING=0`` /
  ``hook_path``): lives on branch ``vlad/chore-local-ab-314v315`` today —
  parameterize to ``3.X-1 vs 3.X`` as a follow-up; manual until then.
* **prof-correctness:** ``python_*_3.X`` jobs green on the S3 wheel path.
* **Staging:** smoke A/B then ai_gateway A/B (experimental staging_ab playbook).
  Preflight auth/signing/wheels. Do not require memory parity for functional
  PASS.

Automation checklist for 3.16 (copy-paste)
------------------------------------------

Ordered by phase. Check off in the PR / tracking issue. Agent driver:
``migrate-profiling-new-cpython``.

**Alpha**

* [ ] ``verify_profiler_compatibility.py --scaffold 3.16``
* [ ] ``find-cpython-usage`` + ``compare-cpython-versions`` (v3.15.0 → v3.16.0aN)
* [ ] Native ABI / layout contracts PR (pattern `#19269`__) — gated CI job
* [ ] Registry ``checklist_open`` updated

**Beta**

* [ ] Collectors + ``setup.py`` un-gate (pattern `#19270`__)
* [ ] Asyncio hook probe (``wrap`` vs ``sys.monitoring``) — pattern `#19272`__
* [ ] ``verify_profiler_compatibility.py --python 3.16`` PASS + ``--baseline``
* [ ] ``run-profiling-tests --python 3.16`` PASS

**RC**

* [ ] Engraver ``python/3.16.0rcN{,-fips}`` within days of tag
* [ ] dd-source digests + language-tools seed
* [ ] manylinux mirror + ``IMAGE_TAG`` bump
* [ ] Hermetic interpreter pinned to exact prerelease
* [ ] Cython pin if needed; wheels optional then required
* [ ] prof-correctness ``python_*_3.16`` jobs

**Final**

* [ ] SSI/OCI enabled only now
* [ ] Release note (``releasenote`` skill)
* [ ] ADR / readiness doc
* [ ] Staging smoke → ai_gateway A/B with preflight

**Still manual / follow-ups**

* Scheduled CI job that opens a tracking issue on new CPython prerelease tags.
* Promote the local 314v315 A/B harness to a version-parameterized script on
  ``main``.
* ``scripts/cpython_delta/`` inventory→diff→worklist pipeline (stacked PR on
  this tooling).
...
#. CPython header/asyncio diff + in-repo grep for the previous release's ``PY_VERSION_HEX`` /
   version tuples (use the **migrate-profiling-new-cpython** orchestrator, which
   calls **compare-cpython-versions** and **find-cpython-usage**).
...
* ADR: `PR #20478`__
* PR catalog: ``docs/cpython-diffs/py315_pr_catalog.md``
* Header analysis: ``docs/cpython-diffs/analysis_314_to_315.md``
* Stack / status map: ``scripts/py315-stack/PROFILING_STACK.md``
* Version registry: ``scripts/profiles/profiling_versions.json``
* Staging A/B playbook: DataDog/experimental ``staging_ab`` (path only; not in this repo)
