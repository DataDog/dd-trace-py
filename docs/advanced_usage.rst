...
``ddtrace`` only supports `uWSGI <https://uwsgi-docs.readthedocs.io/>`__ when configured as follows:
...
- For automatic instrumentation (like ``ddtrace-run``) set the `import <https://uwsgi-docs.readthedocs.io/en/latest/Options.html#import>`__ option to ``ddtrace.bootstrap.customize``.

Example with CLI arguments:

.. code-block:: bash

  uwsgi --enable-threads --lazy-apps --import=ddtrace.bootstrap.customize --master --processes=5 --http 127.0.0.1:8000 --module wsgi:application
