...
3. Make sure a suite with a dependency matrix exists in ``tests/contrib/suitespec.yml`` and references your
   ``contrib`` subdirectory. Its key is the test suite name.
...
9. List the suite's environments and run the new test. The runner starts the test agent and any declared services:
