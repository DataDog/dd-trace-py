...
BUILD_PROFILING_NATIVE_TESTS: bool = _env_truthy("DD_PROFILING_NATIVE_TESTS")
    ...
    def run(self) -> None:
        try:
            ...
            for ext in chain(dist.ext_modules, getattr(dist, "rust_extensions", [])):
                ...
                # For profiling, these CXX bridge artifacts are generated by the Rust extension
...
class CustomBuildRust(build_rust):
    """Custom build_rust command that copies generated CXX bridge artifacts."""
    ...
    def run(self) -> None:
        ...
        # Check if profiling is enabled and copy generated CXX bridge artifacts.
...
class CustomBuildExt(build_ext):
    ...
    def build_rust(self):
        ...
        if library.exists():
            ...
            should_build = newest_source_time > library_mtime or not generated_files_exist
    ...
    def build_extension_cmake(self, ext: "CMakeExtension") -> None:
        ...
        if BUILD_PROFILING_NATIVE_TESTS and ext.name.startswith("ddtrace.internal.datadog.profiling."):
            cmake_args += ["-DBUILD_TESTING=ON"]
        else:
            cmake_args += ["-DBUILD_TESTING=OFF"]
