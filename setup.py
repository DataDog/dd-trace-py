...
class CustomBuildExt(build_ext):
    ...
    def build_rust(self):
        ...
        # Get all Rust source files for cache checking
        ...
        if ExtensionCache.try_restore_from_cache("ddtrace.internal.native._native", source_files, library):
            # Cache hit - skip build
            print(f"Skipping Rust extension build (restored from cache): {native_name}")
        ...
        if should_build:
            ...
            ExtensionCache.save_to_cache("ddtrace.internal.native._native", source_files, library)
        else:
    ...
    def build_libdd_wrapper(self):
        ...
        # Try to restore from cache first
        if ExtensionCache.try_restore_from_cache("libdd_wrapper", source_files, wrapper_library):
            # Cache hit - skip build
            print(f"Skipping libdd_wrapper build (restored from cache): {wrapper_name}")
            return
        ...
        if should_build:
            ...
            # Save to cache after successful build
            ExtensionCache.save_to_cache("libdd_wrapper", source_files, wrapper_library)
        else:
    ...
    def build_extension(self, ext):
        if isinstance(ext, CMakeExtension):
            ...
            # For non-CMake extensions, check cache before building
            target_path = Path(self.get_ext_fullpath(ext.name))
            sources = [Path(s) for s in ext.sources]

            # Try to restore from cache
            if ExtensionCache.try_restore_from_cache(ext.name, sources, target_path):
                # Cache hit - skip build
                pass
            else:
                # Cache miss - build and save to cache
                super().build_extension(ext)
                ExtensionCache.save_to_cache(ext.name, sources, target_path)
    ...
    def build_extension_cmake(self, ext: "CMakeExtension") -> None:
        ...
        if ExtensionCache.try_restore_from_cache(ext.name, sources, full_path):
            # Cache hit - skip build
            return
        ...
        if IS_EDITABLE and self.INCREMENTAL:
            ...
            if not (
                ...
                ExtensionCache.save_to_cache(ext.name, sources, full_path)
                return
            else:
...
class ExtensionCache:
    """Utility class for caching compiled extensions."""

    @staticmethod
    def compute_hash(sources: t.List[Path]) -> str:
        """Compute SHA256 hash of source files."""
        sources_hash = hashlib.sha256()
        for source in sorted(sources):
            if source.exists():
                sources_hash.update(source.read_bytes())
        return sources_hash.hexdigest()

    @staticmethod
    def get_cache_path(ext_name: str, ext_hash: str, binary_name: str) -> Path:
        """Get the cache directory path for an extension."""
        cache_dir = EXT_CACHE_DIR / ext_name / ext_hash
        return cache_dir / binary_name

    @staticmethod
    def try_restore_from_cache(ext_name: str, sources: t.List[Path], target: Path) -> bool:
        """
        Try to restore extension from cache.
        Returns True if restoration succeeded, False otherwise.
        """
        ext_hash = ExtensionCache.compute_hash(sources)
        cache_path = ExtensionCache.get_cache_path(ext_name, ext_hash, target.name)

        if cache_path.exists():
            target.parent.mkdir(parents=True, exist_ok=True)
            shutil.copy(cache_path, target)
            print(f"Restored {ext_name} from cache (hash: {ext_hash[:8]})")
            return True
        return False

    @staticmethod
    def save_to_cache(ext_name: str, sources: t.List[Path], source_binary: Path) -> None:
        """Save built extension to cache."""
        if not source_binary.exists():
            return

        ext_hash = ExtensionCache.compute_hash(sources)
        cache_path = ExtensionCache.get_cache_path(ext_name, ext_hash, source_binary.name)

        cache_path.parent.mkdir(parents=True, exist_ok=True)
        shutil.copy(source_binary, cache_path)
        print(f"Saved {ext_name} to cache (hash: {ext_hash[:8]})")
