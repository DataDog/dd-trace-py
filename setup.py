    ...
    def run(self):
        try:
            ...
            for ext in chain(dist.ext_modules, getattr(dist, "rust_extensions", [])):
                ...
                # C/C++ steps (build_libdd_wrapper, ProfilingExtension).  Cache
...
class CustomBuildExt(build_ext):
    ...
    _cmake_deps_install_dir: "t.Optional[Path]" = None
    ...
    def run(self):
        ...
        self._cmake_deps_install_dir = self._prebuild_cmake_deps()
    ...
    def build_rust(self):
        ...
        if library.exists():
            ...
            source_files = []
            # Find all .rs files in the crate, excluding Cargo's target/ output trees.
            source_files.extend(
                f for f in NATIVE_CRATE.glob("**/*.rs") if not f.relative_to(NATIVE_CRATE).parts[0].startswith("target")
            )
            # Add cargo files
            ...
            include_dir = CARGO_TARGET_DIR / "include" / "datadog"
            headers_exist = include_dir.is_dir() and any(include_dir.glob("*.h"))
    ...
    _cython_dep_tree_cache: "t.Optional[t.Any]" = None

    @classmethod
    def _pyx_source_deps(cls, pyx_path: str) -> "list[str]":
        ...
        """
        if cls._cython_dep_tree_cache is None:
            cls._cython_dep_tree_cache = _cython_dep_tree()
        dep_tree = cls._cython_dep_tree_cache
        return [
            str((HERE / d).resolve())
    ...
    def _prebuild_cmake_deps(self) -> Path:
        ...
        Extensions compiled by setuptools (AbslExtension / ProfilingExtension)
...
class ProfilingExtension(AbslExtension):
    """Profiling extension that links against Abseil and libdd_wrapper."""
...
if not IS_PYSTON:
    ...
    if platform.system() not in ("Windows", ""):
        ...
        for _subdir in _iast_subdirs:
            _iast_sources.extend(str(p.relative_to(HERE)) for p in IAST_DIR.glob(f"{_subdir}*.cpp"))
            _iast_depends.extend(str(p.relative_to(HERE)) for p in IAST_DIR.glob(f"{_subdir}*.h"))
        ext_modules.append(
            AbslExtension(
