...
class BuildExtWithUPX(BuildExtCommand):
    ...
    def download_upx(self):
        UPXDownload.run()
        base_path = UPXDownload.get_download_dir()
...
class LibraryDownload:
    ...
    @classmethod
    def untar_gz(cls, filename, suffixes, archive_dir, arch_dir):
    ...
    @classmethod
    def untar_xz(cls, filename, suffixes, archive_dir, arch_dir):
    ...
    def download_artifacts(cls):
        # In the average case, the caller wants to match a set of file suffixes.
        # Passing the empty string will match everything.
        suffixes = cls.translate_suffix[CURRENT_OS]

        # If the directory exists and it is not empty, assume the right files are there.
        # Every downloader should register in CleanLibraries so that `python setup.py clean` can remove it.
        ...
        for arch in cls.available_releases[CURRENT_OS]:
            ...
            archive_dir, archive_name = cls.get_package_names(arch, CURRENT_OS)
            ...
            # Extract contents (if compressed)
            if archive_name.endswith(".tar.gz"):
                cls.untar_gz(filename, suffixes, archive_dir, arch_dir)
            elif archive_name.endswith(".tar.xz"):
                cls.untar_xz(filename, suffixes, archive_dir, arch_dir)
...
class LibDDWafDownload(LibraryDownload):
    ...
    @classmethod
    def get_package_names(cls, arch, os):
...
class UPXDownload(LibraryDownload):
    name = "upx"
    download_dir = "/tmp/datadog/upx"
    version = UPX_VERSION
    url_root = "https://github.com/upx/upx/releases/download"
    expected_checksums = {
        "Linux": {
    ...
    translate_suffix = {"Windows": (), "Darwin": (), "Linux": ("upx",)}
    ...
    def get_package_names(cls, arch, os):
        ...
        # upx file has leading 'v' in version name, but directory does not; strip here to conform with
        # Downloader
        archive_dir = "%s-%s-%s_%s" % (cls.name, cls.version.lstrip("v"), archnames[arch], os.lower())
