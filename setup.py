...
class CustomBuildExt(build_ext):
    ...
    def build_extension(self, ext):
        if isinstance(ext, CMakeExtension):
            ...
            # and add include directory for generated headers (profiling.h, common.h)
            if ext.name == "ddtrace.profiling.collector._memalloc" and CURRENT_OS in ("Linux", "Darwin"):
                ...
                # Add include directory for generated headers (profiling.h, common.h)
                # These are generated by the Rust extension in CARGO_TARGET_DIR/include
                generated_headers_dir = str(CARGO_TARGET_DIR / "include")
                if generated_headers_dir not in ext.include_dirs:
                    ext.include_dirs.append(generated_headers_dir)
