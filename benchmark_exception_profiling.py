...
def create_deep_exception(depth):
    """Create exception at specified call depth."""
    if depth <= 0:
...
def benchmark_exceptions(count, depth):
    """Run benchmark with specified exception count and stack depth."""
    exceptions_raised = 0
...
def print_samples(samples, max_samples=5):
    """Print captured exception samples with stack traces."""
